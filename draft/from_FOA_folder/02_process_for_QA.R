...
Path.Tmp         <- paste(Path.Project,"02_process_for_QA",                    sep="/")			
...
FL.LOG <- paste(Path.log, paste("02_process_for_QA_",timeStamp.string,".log",sep=""),sep="/")	
...
cat(paste("Log file for data processing script [02_process_for_QA.R]!\n",sep=""),file=FL.LOG, append=TRUE)
cat(paste("\n***************************************************************************",
            "*                    [02_process_for_QA.R]                                *",
            "***************************************************************************",sep="\n"),file=FL.LOG, append=TRUE)

# list of key code items: did not used in the script.
...
for (this.state in array.states)
{
	...
	if (dim(myData.thisState)[1] > 0)
	{
		...
		cat(paste("8. ",string.thisState,": Add CZ infront of CZ.\n",sep=""))
		cat(paste("8. ",string.thisState,": Add CZ infront of CZ.\n",sep=""),file=FL.LOG, append=TRUE)	

		# =======================================================================
		# F. save the cleaned data for this state
		# =======================================================================
		cat(paste("Cleaned Data of State (",this.state,"),",sep=""),file=FL.OUT.CSV,append=TRUE)
		write.table(myData.clean,file=FL.OUT.CSV,sep=",",row.names=TRUE,col.names=TRUE,append=TRUE)
		saveRDS(myData.clean,file=FL.OUT.RDS)
		...
		if (dim(myData.clean)[1] > 0)
		{
			...
			for (this.field in c("CIA_codeitem_complies","CIA_codeitem_code"))
			{
				...
				if (this.field == "CIA_codeitem_code")
				{
					...
					# ----------------------------------------------------------------------------
					myData.clean.lvldropped <- droplevels(myData.clean)
					...
					cat(paste("12B. ",string.thisField,": remove the summary data frames if they exist from previous calculation.\n",sep=""),file=FL.LOG, append=TRUE)									
					      ...
					      myTmp    <- aggregate(myData.clean.lvldropped[,this.field],list(myData.clean.lvldropped[,this.field],myData.clean.lvldropped[,"SPL_state"]),length)
					...
					cat(paste("13A. ",string.thisField,": plotting.\n",sep=""),file=FL.LOG, append=TRUE)									
					      ...
					      myTmp    <- aggregate(myData.clean.lvldropped[,this.field],list(myData.clean.lvldropped[,this.field],myData.clean.lvldropped[,"ClimateZone"]),length)
					...
					cat(paste("13B. ",string.thisField,": plotting.\n",sep=""),file=FL.LOG, append=TRUE)									
					      ...
					      myTmp    <- aggregate(myData.clean.lvldropped[,this.field],list(myData.clean.lvldropped[,this.field],myData.clean.lvldropped[,"SPL_county"]),length)
					...
					cat(paste("13C. ",string.thisField,": plotting.\n",sep=""),file=FL.LOG, append=TRUE)									
					      ...
					      myTmp    <- aggregate(myData.clean.lvldropped[,this.field],list(myData.clean.lvldropped[,this.field],myData.clean.lvldropped[,"SPL_bldg_name"]),length)
					...
					cat(paste("13D. ",string.thisField,": plotting.\n",sep=""),file=FL.LOG, append=TRUE)									
					      ...
					      myTmp    <- aggregate(myData.clean.lvldropped[,this.field],list(myData.clean.lvldropped[,this.field],myData.clean.lvldropped[,"CIA_codeitem_complies"]),length)
					...
					cat(paste("13E. ",string.thisField,": plotting.\n",sep=""),file=FL.LOG, append=TRUE)									
					      ...
					      myTmp    <- aggregate(myData.clean.lvldropped[,this.field],list(myData.clean.lvldropped[,this.field],myData.clean.lvldropped[,"CIA_codeitem_comments"]),length)
					...
					# sum.by.State   <- do.call(rbind , by(myData.clean.lvldropped[,this.field], myData.clean.lvldropped[,"SPL_state"],            summary))
					# sum.by.CZ      <- do.call(rbind , by(myData.clean.lvldropped[,this.field], myData.clean.lvldropped[,"ClimateZone"],          summary))
					# sum.by.County  <- do.call(rbind , by(myData.clean.lvldropped[,this.field], myData.clean.lvldropped[,"SPL_county"],           summary))
					# sum.by.Bldg    <- do.call(rbind , by(myData.clean.lvldropped[,this.field], myData.clean.lvldropped[,"SPL_bldg_name"],        summary))
					# sum.by.Complie <- do.call(rbind , by(myData.clean.lvldropped[,this.field], myData.clean.lvldropped[,"CIA_codeitem_complies"],summary,na.rm=TRUE))
					...
					### # create summary table
					### sum.by.State   <- do.call(rbind , by(myData.clean.lvldropped[,this.field], myData.clean.lvldropped[,"SPL_state"],            summary))
					### sum.by.CZ      <- do.call(rbind , by(myData.clean.lvldropped[,this.field], myData.clean.lvldropped[,"SPL_CZ"],               summary))
					### sum.by.County  <- do.call(rbind , by(myData.clean.lvldropped[,this.field], myData.clean.lvldropped[,"SPL_county"],           summary))
					### sum.by.Bldg    <- do.call(rbind , by(myData.clean.lvldropped[,this.field], myData.clean.lvldropped[,"SPL_bldg_name"],        summary))
					### sum.by.Complie <- do.call(rbind , by(myData.clean.lvldropped[,this.field], myData.clean.lvldropped[,"CIA_codeitem_complies"],summary))
					...
					# ----------------------------------------------------------------------------
					# ----------------------------------------------------------------------------
					list.codeItem <- sort(unique(myData.clean[,"CIA_codeitem_code"]))
					count <- 0
					...
					for (this.codeItem in list.codeItem)	
					{
						...
						# subsetting data
						mySubset <- subset(myData.clean,subset = (CIA_codeitem_code == this.codeItem))
						...
						if (no.data >= 1)
						{
							...
							if ((length(grep("[^0-9\\.]",mySubset[,"CodeItemValue"]))) | (length(grep("(\\d*\\.+)(\\d*\\.+)(.*)$",mySubset[,"CodeItemValue"]))))	# if any non-numeric value has been found in the CodeItemValue field, treat it as a character field otherwise a numeric field
							{
								...
								if (this.state == "AllData")
								{
									...
									p.plot2 <- p.plot2 + labs(x=paste(this.codeItem,":(",this.codeItem.desp,")",sep=""),y="count",title=paste(string.thisCodeitem,": ",no.Subset," observations by [County]",sep=""))
									plot(p.plot2)
									cat(paste("23F. ",string.thisField,": plotting.\n",sep=""))
									cat(paste("23F. ",string.thisField,": plotting.\n",sep=""),file=FL.LOG, append=TRUE)	
									...
									p.plot3 <- p.plot3 + labs(x=paste(this.codeItem,":(",this.codeItem.desp,")",sep=""),y="count",title=paste(string.thisCodeitem,": ",no.Subset," observations by [County]",sep=""))
							              # p.plot3 <- p.plot3 + geom_vline(xintercept=c(this.codeItem.UL,this.codeItem.LL),colour="blue",linetype="longdash")
									plot(p.plot3)
									cat(paste("23G. ",string.thisField,": plotting.\n",sep=""))
									cat(paste("23G. ",string.thisField,": plotting.\n",sep=""),file=FL.LOG, append=TRUE)	
									...
									plot(p.plot4)
									cat(paste("23H. ",string.thisField,": plotting.\n",sep=""))
									cat(paste("23H. ",string.thisField,": plotting.\n",sep=""),file=FL.LOG, append=TRUE)	
									...
									plot(p.plot5)
									cat(paste("23I. ",string.thisField,": plotting.\n",sep=""))
									cat(paste("23I. ",string.thisField,": plotting.\n",sep=""),file=FL.LOG, append=TRUE)	

								}
								cat(paste("23J. ",string.thisCodeitem,": is a character field and has ",dim(mySubset)[1]," data points and has been plotted.\n",sep=""))
								cat(paste("23J. ",string.thisCodeitem,": is a character field and has ",dim(mySubset)[1]," data points and has been plotted.\n",sep=""),file=FL.LOG, append=TRUE)									
							#
							# Numeric Field
							#							
								...
								cat(paste("31. ",string.thisCodeitem,": change lower case to upper case.\n",sep=""))
								cat(paste("31. ",string.thisCodeitem,": change lower case to upper case.\n",sep=""),file=FL.LOG, append=TRUE)
								...
								if(exists("sum.by.Complie")){rm(sum.by.Complie)}
								if(exists("sum.by.Comment")){rm(sum.by.Comment)}
								cat(paste("32. ",string.thisCodeitem,": remove the summary data frames if they exist from previous calculation.\n",sep=""))
								cat(paste("32. ",string.thisCodeitem,": remove the summary data frames if they exist from previous calculation.\n",sep=""),file=FL.LOG, append=TRUE)									
								...
								cat(paste("33A. ",string.thisField,": summarizing.\n",sep=""))
								cat(paste("33A. ",string.thisField,": summarizing.\n",sep=""),file=FL.LOG, append=TRUE)	
								...
								cat(paste("33B. ",string.thisField,": summarizing.\n",sep=""))
								cat(paste("33B. ",string.thisField,": summarizing.\n",sep=""),file=FL.LOG, append=TRUE)	
								...
								cat(paste("33C. ",string.thisField,": summarizing.\n",sep=""))
								cat(paste("33C. ",string.thisField,": summarizing.\n",sep=""),file=FL.LOG, append=TRUE)	
								...
								cat(paste("33D. ",string.thisField,": summarizing.\n",sep=""))
								cat(paste("33D. ",string.thisField,": summarizing.\n",sep=""),file=FL.LOG, append=TRUE)	
								...
								cat(paste("33E. ",string.thisField,": summarizing.\n",sep=""))
								cat(paste("33E. ",string.thisField,": summarizing.\n",sep=""),file=FL.LOG, append=TRUE)	
								...
								cat(paste("33F. ",string.thisField,": summarizing.\n",sep=""))
								cat(paste("33F. ",string.thisField,": summarizing.\n",sep=""),file=FL.LOG, append=TRUE)	
								...
								# sum.by.Bldg    <- do.call(rbind , by(mySubset[,"CodeItemValue"], mySubset[,"Building"],        summary))
								# sum.by.Complie <- do.call(rbind , by(mySubset[,"CodeItemValue"], mySubset[,"CIA_codeitem_complies"],summary,na.rm=TRUE))
								cat(paste("33G. ",string.thisCodeitem,": summarize.\n",sep=""))
								cat(paste("33G. ",string.thisCodeitem,": summarize.\n",sep=""),file=FL.LOG, append=TRUE)									
								...
								cat(paste("\n\n",sep=""),file=FL.SUM.ALL.CSV,append=TRUE)
								# ---------------------------------------------------------------------------------------------------------------------
								cat(paste("34. ",string.thisCodeitem,": output the summary.\n",sep=""))
								cat(paste("34. ",string.thisCodeitem,": output the summary.\n",sep=""),file=FL.LOG, append=TRUE)									

								no.unique.lvl <- length(unique(mySubset[,"CodeItemValue"])) 
								if (no.unique.lvl >= 10){xtick.size = 5}else{xtick.size = 10}
								cat(paste("35. ",string.thisCodeitem,": get the unique levels.\n",sep=""))
								cat(paste("35. ",string.thisCodeitem,": get the unique levels.\n",sep=""),file=FL.LOG, append=TRUE)									
								...
								if (this.state == "AllData")
								{
									...
									plot(p.plot1)	
									cat(paste("36A. ",string.thisField,": plotting.\n",sep=""))
									cat(paste("36A. ",string.thisField,": plotting.\n",sep=""),file=FL.LOG, append=TRUE)
									...
									p.plot2 <- p.plot2 + geom_vline(xintercept=c(this.codeItem.UL,this.codeItem.LL),colour="blue",linetype="longdash")									
									plot(p.plot2)	
									cat(paste("36B. ",string.thisField,": plotting.\n",sep=""))
									cat(paste("36B. ",string.thisField,": plotting.\n",sep=""),file=FL.LOG, append=TRUE)
								}else{
									...
									p.plot1 <- p.plot1 + geom_vline(xintercept=c(this.codeItem.UL,this.codeItem.LL),colour="blue",linetype="longdash")
									plot(p.plot1)	
									cat(paste("36C. ",string.thisField,": plotting.\n",sep=""))
									cat(paste("36C. ",string.thisField,": plotting.\n",sep=""),file=FL.LOG, append=TRUE)
									...
									p.plot2 <- p.plot2 + geom_vline(xintercept=c(this.codeItem.UL,this.codeItem.LL),colour="blue",linetype="longdash")									
									plot(p.plot2)	
									cat(paste("36D. ",string.thisField,": plotting.\n",sep=""))
									cat(paste("36D. ",string.thisField,": plotting.\n",sep=""),file=FL.LOG, append=TRUE)
									...
									p.plot3 <- p.plot3 + geom_vline(xintercept=c(this.codeItem.UL,this.codeItem.LL),colour="blue",linetype="longdash")
									plot(p.plot3)							
									cat(paste("36E. ",string.thisField,": plotting.\n",sep=""))
									cat(paste("36E. ",string.thisField,": plotting.\n",sep=""),file=FL.LOG, append=TRUE)
								}
								cat(paste("36F. ",string.thisCodeitem,": is a numeric field and has ",dim(mySubset)[1]," data points and has been plotted.\n",sep=""))
								cat(paste("36F. ",string.thisCodeitem,": is a numeric field and has ",dim(mySubset)[1]," data points and has been plotted.\n",sep=""),file=FL.LOG, append=TRUE)					
							}
				...
				}else if (this.field == "CIA_codeitem_complies")
				{
					...
					cat(paste("44A. ",string.thisField,": summarize.\n",sep=""))
					cat(paste("44A. ",string.thisField,": summarize.\n",sep=""),file=FL.LOG, append=TRUE)									
					...
					cat(paste("44B. ",string.thisField,": summarize.\n",sep=""))
					cat(paste("44B. ",string.thisField,": summarize.\n",sep=""),file=FL.LOG, append=TRUE)									
					...
					cat(paste("44C. ",string.thisField,": summarize.\n",sep=""))
					cat(paste("44C. ",string.thisField,": summarize.\n",sep=""),file=FL.LOG, append=TRUE)									
					...
					cat(paste("44D. ",string.thisField,": summarize.\n",sep=""))
					cat(paste("44D. ",string.thisField,": summarize.\n",sep=""),file=FL.LOG, append=TRUE)									
					...
					cat(paste("44E. ",string.thisField,": summarize.\n",sep=""))
					cat(paste("44E. ",string.thisField,": summarize.\n",sep=""),file=FL.LOG, append=TRUE)									
					...
					cat(paste("44F. ",string.thisField,": summarize.\n",sep=""))
					cat(paste("44F. ",string.thisField,": summarize.\n",sep=""),file=FL.LOG, append=TRUE)									
					...
					no.unique.lvl <- length(unique(myData.clean[,this.field])) 
					if (no.unique.lvl >= 10){xtick.size = 5}else{xtick.size = 10}	
					...
					{			
						...
						p.plot1 <- ggplot(myData.clean, aes(x=factor(1), fill=factor(CIA_codeitem_complies))) + geom_bar(width=1) + coord_polar(theta="y") + facet_grid(ClimateZone ~ SPL_state,scales="fixed",space="fixed",as.table=TRUE)
						p.plot1 <- p.plot1 + theme(axis.text.x = element_text(angle=90,color="black",size=xtick.size),axis.text.y = element_text(color="black"),legend.position="right") 
						p.plot1 <- p.plot1 + labs(x=paste("Levels of (",this.field,")",sep=""),y="Count",title=paste(string.thisField,": Responses to (complies) in term of [Climate Zone] and [State]\n",sep=""))
						plot(p.plot1)
						...
						# pie chart in term of both state and climate zone			
						 p.plot4 <- qplot(data=myData.clean,CIA_codeitem_complies,facets=SPL_state~ClimateZone,colour=SPL_state,fill=SPL_state,geom="bar") 
						 p.plot4 <- p.plot4 + theme(axis.text.x = element_text(angle=90,color="black",size=xtick.size),axis.text.y = element_text(color="black"),legend.position="right") 
						...
						p.plot5 <- qplot(data=myData.clean,CIA_codeitem_complies,colour=SPL_county,fill=SPL_county,geom="bar",facets=ClimateZone~SPL_state) 
						...
						p.plot6 <- qplot(data=myData.clean,CIA_codeitem_complies,colour=SPL_bldg_name,fill=SPL_bldg_name,geom="bar",facets=ClimateZone~.) 
						...
						p.plot7 <- qplot(data=myData.clean,CIA_codeitem_complies,colour=CIA_codeitem_code,fill=CIA_codeitem_code,geom="bar",facets=ClimateZone~.) 
						...
						# complies response in term of [climate zone] and [state]
						p.plot1 <- ggplot(myData.clean, aes(x=factor(1), fill=factor(CIA_codeitem_complies))) + geom_bar(width=1) + coord_polar(theta="y") + facet_grid(ClimateZone ~ .,scales="fixed",space="fixed",as.table=TRUE)
						p.plot1 <- p.plot1 + theme(axis.text.x = element_text(angle=90,color="black",size=xtick.size),axis.text.y = element_text(color="black"),legend.position="right") 
						...
						# pie chart in term of both state and climate zone			
						 p.plot4 <- qplot(data=myData.clean,CIA_codeitem_complies,colour=ClimateZone,fill=ClimateZone,geom="bar") 
						 p.plot4 <- p.plot4 + theme(axis.text.x = element_text(angle=90,color="black",size=xtick.size),axis.text.y = element_text(color="black"),legend.position="right") 
						...
						# bar chart in terms of [CZ] and [counties]
						p.plot5 <- qplot(data=myData.clean,CIA_codeitem_complies,colour=SPL_county,fill=SPL_county,geom="bar",facets=ClimateZone~.) 
						p.plot5 <- p.plot5 + theme(axis.text.x = element_text(angle=90,color="black",size=xtick.size),axis.text.y = element_text(color="black"),legend.position="right") 
						p.plot5 <- p.plot5 + labs(x=paste("Levels of (",this.field,")",sep=""),y="Count",title=paste(string.thisField,": Responses to (complies) in term of [Climate Zone] and [County]\n",sep=""))
						plot(p.plot5)
					...
					cat(paste("47. ",string.thisField,": done.\n\n",sep=""))
					cat(paste("47. ",string.thisField,": done.\n\n",sep=""),file=FL.LOG, append=TRUE)		
			...
			dev.off(2)
			dev.off(3)	
		}else{		# end of non-zero [myData.clean]   data condition
	...
	}else{			# end of non-zero data condition
				cat(paste("48. there is no data retrieved for ",this.state," at ",timeStamp.string,".\n\n",sep=""))
				cat(paste("48. there is no data retriveed for ",this.state," at ",timeStamp.string,".\n\n",sep=""),file=FL.LOG, append=TRUE)				
	}
}				# end of state loop
...
cat(paste("\n02_process_for_QA.R is finished successfully at ",End.time,"!\n",sep=" "))
cat(paste("\n02_process_for_QA.R is finished successfully at ",End.time,"!\n",sep=" "),file=FL.LOG,append=TRUE)

cat(paste("\nProcessing time for [02_process_for_QA.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "))
cat(paste("\nProcessing time for [02_process_for_QA.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "),file=FL.LOG,append=TRUE)
