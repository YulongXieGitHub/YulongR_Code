#
# 04B_prepare_keyItem_for_simulation_getPlots.R 
#
...
Path.TMP         <- paste(Path.Project,"04B_prepare_keyItem_for_simulation_getPlots",    sep="/")		# Output file directory			
...
# define files
FL.LOG          <- paste(Path.LOG,  paste("04B_prepare_keyItem_for_simulation_getPlots_",timeStamp.string,".log",sep=""),sep="/")	
...
cat(paste("Log file for data processing script [04B_prepare_keyItem_for_simulation_getPlots.R]!\n",sep=""),file=FL.LOG, append=TRUE)
cat(paste("\n***************************************************************************",
            "*                    [04B_prepare_keyItem_for_simulation_getPlots.R]                *",
...
for (this.state in array.states)
{
	...
	if (this.state == "AllData")
	{
		...
		FL.KeyItem.PDF                  <- paste(Path.OUT ,paste(paste("AllData",timeStamp.string,sep="_"),"_KeyItem_After.PDF",sep=""),sep="/")
		...
		FL.KeyItem.PDF                  <- paste(Path.OUT ,paste(paste("State",this.state,timeStamp.string,sep="_"),"_KeyItem_After.PDF",sep=""),sep="/")	
	...
	FL.KeyItem.Before.PDF       <- paste(Path.OUT ,paste(paste("State",this.state,timeStamp.string,sep="_"),"_KeyItem_Before.PDF",      sep=""),sep="/")	
	FL.KeyItem.Before.1page.PDF <- paste(Path.OUT ,paste(paste("State",this.state,timeStamp.string,sep="_"),"_KeyItem_Before_1page.PDF",sep=""),sep="/")	
	FL.KeyItem.After.1page.PDF  <- paste(Path.OUT ,paste(paste("State",this.state,timeStamp.string,sep="_"),"_KeyItem_After_1page.PDF", sep=""),sep="/")	
	if  (file.exists(FL.KeyItem.Before.PDF))      {print(paste(FL.KeyItem.Before.PDF," exist. Delete it!"))      ;file.remove(FL.KeyItem.Before.PDF)}
	if  (file.exists(FL.KeyItem.Before.1page.PDF)){print(paste(FL.KeyItem.Before.1page.PDF," exist. Delete it!"));file.remove(FL.KeyItem.Before.1page.PDF)}
	if  (file.exists(FL.KeyItem.After.1page.PDF)) {print(paste(FL.KeyItem.After.1page.PDF, " exist. Delete it!"));file.remove(FL.KeyItem.After.1page.PDF)}
	...
	pdf(file = FL.KeyItem.Before.PDF,      paper="special", width=17, height=11,bg = "transparent")	# device 3
	pdf(file = FL.KeyItem.Before.1page.PDF,paper="special", width=17, height=11,bg = "transparent")	# device 4
	pdf(file = FL.KeyItem.After.1page.PDF, paper="special", width=17, height=11,bg = "transparent")	# device 5
	...
	if (dim(myData.thisState.cleaned)[1] > 0)
	{
		...
		# &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
			...
			# -----------------------------------------------------------------------------------------------
			list.codeItem.raw <- c("FI17","FI1","FI4","FI6","FO1","FO4A","FO4B","FR2","FR3","IN1a","IN1b","IN3a","IN3b","FO4a","FO4b")	# do not do all the KeyCode in the KeyCodeName field because some are raw and some ware combined.  Need customized as follow:
			for (this.codeItem.raw in list.codeItem.raw[grep("^Comp",list.codeItem.raw,invert=TRUE)])
			{
				this.codeitem.name <- name.keyItem[this.codeItem.raw]		# MAPPING!!!!!!!!!!!



				
				# find the corresponding short description and range if it is a numeric code item
				idx.matched <- match(this.codeItem.raw,myMap[,"ID"])
				this.codeItem.raw.desp <- myMap[idx.matched,"Short.Name"]
				this.codeItem.raw.UL   <- myMap[idx.matched,"Upper.Bound"]
				this.codeItem.raw.LL   <- myMap[idx.matched,"Lower.Bound"]
				cat(paste("660. ",string.thisState,": description and range of [",this.codeitem.name,"].\n",sep=""))
				cat(paste("660. ",string.thisState,": description and range of [",this.codeitem.name,"].\n",sep=""),file=FL.LOG, append=TRUE)	



				mySubset.raw <- subset(myData.notClean,subset=(CIA_codeitem_code == this.codeItem.raw))
				cat(paste("661. ",string.thisState,": subsetting the data for [",this.codeitem.name,"].\n",sep=""))
				cat(paste("661. ",string.thisState,": subsetting the data for [",this.codeitem.name,"].\n",sep=""),file=FL.LOG, append=TRUE)
				
				# remove NA rows
				mySubset.raw <- mySubset.raw[!(is.na(mySubset.raw[,"PV_codeitem_value"])),]
				mySubset.raw <- mySubset.raw[mySubset.raw[,"PV_codeitem_value"] != "NA" & mySubset.raw[,"PV_codeitem_value"] != "NR" & mySubset.raw[,"PV_codeitem_value"] != "",]							
				cat(paste("662. ",string.thisState,": [",this.codeItem.raw,"]:  detele the NA rows.\n",sep=""))
				cat(paste("662. ",string.thisState,": [",this.codeItem.raw,"]:  detele the NA rows.\n",sep=""),file=FL.LOG, append=TRUE)
				
				#
				# June 10, 2015
				# 
				if (this.codeItem.raw == "FI1")
				{
					mySubset.raw[mySubset.raw[,"PV_codeitem_value"] == "30,11","PV_codeitem_value"] <- "30"
					mySubset.raw[,"PV_codeitem_value"] <- as.numeric(as.character(mySubset.raw[,"PV_codeitem_value"])) 
				}
				
				
				mySubset.raw.lvldropped <- droplevels(mySubset.raw)
				

				# out put the record to a summary file
				no.records.nonNA.raw <- length(!(is.na(mySubset.raw.lvldropped[,"PV_codeitem_value"])))
				cat(paste("CodeItem in Database",this.state,this.codeItem.raw,this.codeitem.name,no.records.nonNA.raw,"\n",sep=","),file=FL.allData.SUM, append=TRUE)	
				cat(paste("663. ",string.thisState,": [",this.codeitem.name,"]: number of non-NA records has been saved to [",FL.allData.SUM,"].\n",sep=""))
				cat(paste("663. ",string.thisState,": [",this.codeitem.name,"]: number of non-NA records has been saved to [",FL.allData.SUM,"].\n",sep=""),file=FL.LOG, append=TRUE)	
				...
				# -------------------------------------------------------------------------------------------------
				if (this.codeItem.raw == "FI4")
				{
					# # May 15: assume multiple observation sof the same system, so take means
					# mySubset.raw.lvldropped[,"PV_codeitem_value.raw"]  <- mySubset.raw.lvldropped[,"PV_codeitem_value"]
					# mySubset.raw.lvldropped[,"PV_codeitem_value.form"] <- paste("mean(c(",mySubset.raw.lvldropped[,"PV_codeitem_value.raw"],"),na.rm=TRUE)",sep="")
					# 
					# for (index in seq(1:dim(mySubset.raw.lvldropped)[1]))
					# {
					# 	mySubset.raw.lvldropped[index,"PV_codeitem_value"] <- eval(parse(text=mySubset.raw.lvldropped[index,"PV_codeitem_value.form"]))
					# }
					...
					#
					if (is.factor(mySubset.raw.lvldropped[,"PV_codeitem_value"]))
					{
						mySubset.raw.lvldropped[,"PV_codeitem_value"]     <- as.character(mySubset.raw.lvldropped[,"PV_codeitem_value"])
						mySubset.raw.lvldropped[,"PV_codeitem_value.raw"] <- mySubset.raw.lvldropped[,"PV_codeitem_value"]
					}else{
						mySubset.raw.lvldropped[,"PV_codeitem_value.raw"] <- mySubset.raw.lvldropped[,"PV_codeitem_value"]
					}

					idx.new.row                                        <- dim(mySubset.raw.lvldropped)[1]					
					index.multiple                                     <- grep(",",mySubset.raw.lvldropped[,"PV_codeitem_value.raw"])	# the index where FI4 has multiple entries
					...
					{
						multiple.array <- unlist(strsplit(mySubset.raw.lvldropped[index,"PV_codeitem_value.raw"],","))
						mySubset.raw.lvldropped[index,"PV_codeitem_value"] <- multiple.array[1]				# the original row take the first of the multiple entries
						for (tmp.idx in seq(from=2,to=length(multiple.array)))
						{	
							idx.new.row <- idx.new.row + 1							# the rest of the multiple entries is added to the end of the [mySubset.raw.lvldropped]
							mySubset.raw.lvldropped[idx.new.row,]               <- mySubset.raw.lvldropped[index,]	# everything else in the new row has the same value as the row of the the multiple entry
							mySubset.raw.lvldropped[idx.new.row,"PV_codeitem_value"] <- multiple.array[tmp.idx]	# replace the "PV_codeitem_value" with the rest of the multplie entries
						}
					}
					
					# May 29, 2015: convert the observed leakage rate (cfm/100sf) into leakage ratio
					index.withValue <- (!(is.na(mySubset.raw.lvldropped[,"PV_codeitem_value"]))       & 
					                           (mySubset.raw.lvldropped[,"PV_codeitem_value"] !="NA") & 
					                           (mySubset.raw.lvldropped[,"PV_codeitem_value"] !="")    )
					# mySubset.raw.lvldropped[index.withValue,"PV_codeitem_value"] <- (as.numeric(mySubset.raw.lvldropped[index.withValue,"PV_codeitem_value"]) * 2400 / 100) / maximum_supply_cfm
					mySubset.raw.lvldropped[index.withValue,"PV_codeitem_value"] <- as.numeric(mySubset.raw.lvldropped[index.withValue,"PV_codeitem_value"])	# june 9, 2015: showing the measure unit cfm/100 sf instead of the simulation unit the leakage ratio 
					
				}else{
					string.multiple <- ""
				}
				cat(paste("664. ",string.thisState,": [",this.codeitem.name,"]: Handling multiple entries as in duct tightness in FI4.\n",sep=""))
				cat(paste("664. ",string.thisState,": [",this.codeitem.name,"]: Handling multiple entries as in duct tightness in FI4.\n",sep=""),file=FL.LOG, append=TRUE)	


				no.data.raw   <- dim(mySubset.raw.lvldropped)[1] 
			      # no.Subset.raw <- dim(mySubset.raw.lvldropped)[1]
				no.Subset.raw <- sum(!(is.na(mySubset.raw.lvldropped[,"PV_codeitem_value"])))	
				cat(paste("664B. ",string.thisState,": [",this.codeitem.name,"]: number of observations.\n",sep=""))
				cat(paste("664B. ",string.thisState,": [",this.codeitem.name,"]: number of observations.\n",sep=""),file=FL.LOG, append=TRUE)	
				...
				if (no.Subset.raw > 0)
				{
					...
					if ((length(grep("[^0-9\\.]",mySubset.raw.lvldropped[,"PV_codeitem_value"]))) | (length(grep("(\\d*\\.+)(\\d*\\.+)(.*)$",mySubset.raw.lvldropped[,"PV_codeitem_value"]))))	
					{
						cat(paste("**** ",string.thisState,": [",this.codeItem.raw,"]: is a character field.  No plotting at this time!\n",sep=""))
						cat(paste("**** ",string.thisState,": [",this.codeItem.raw,"]: is a character field.  No plotting at this time!\n",sep=""),file=FL.LOG, append=TRUE)
					}else{
						# convert the numeric fiel first
						if(is.character(mySubset.raw.lvldropped[,"PV_codeitem_value"])){mySubset.raw.lvldropped[,"PV_codeitem_value"] <- as.numeric(mySubset.raw.lvldropped[,"PV_codeitem_value"])}
						if(is.factor(mySubset.raw.lvldropped[,"PV_codeitem_value"])){mySubset.raw.lvldropped[,"PV_codeitem_value"] <- as.numeric(as.character(mySubset.raw.lvldropped[,"PV_codeitem_value"]))}
					
	
						
						p.plot.before2A <- ggplot(data=mySubset.raw.lvldropped,aes(x=PV_codeitem_value,colour=factor(PV_codeitem_value),fill=factor(PV_codeitem_value))) + geom_histogram()
						p.plot.before2A <- p.plot.before2A + theme(axis.text.x = element_text(angle=90,color="black",size=10),axis.text.y = element_text(color="black"),legend.position="")
						p.plot.before2A <- p.plot.before2A + labs(x=paste(this.codeItem.raw,"(",this.codeitem.name,")",sep=" "),y="count",title=paste(string.thisState,": ( ",this.codeItem.raw,")-(",this.codeitem.name,"\nNo. Observations: ",no.Subset.raw,sep=""))
					      #p.plot.before2A <- p.plot.before2A + geom_vline(aes(xintercept=CodeCompliance), data = mySubset.raw.lvldropped) 
						# plot(p.plot.before2A)

						p.plot.before2B <- ggplot(data=mySubset.raw.lvldropped,aes(x=PV_codeitem_value,colour=factor(SPL_CZ),fill=factor(SPL_CZ))) + geom_histogram()
						p.plot.before2B <- p.plot.before2B + theme(axis.text.x = element_text(angle=90,color="black",size=10),axis.text.y = element_text(color="black"),legend.position="") 
						p.plot.before2B <- p.plot.before2B + labs(x=paste(this.codeItem.raw,"(",this.codeitem.name,")",sep=" "),y="count",title=paste(string.thisState,": ( ",this.codeItem.raw,")-(",this.codeitem.name,"\nNo. Observations: ",no.Subset.raw,sep=""))
					      #p.plot.before2B <- p.plot.before2B + geom_vline(aes(xintercept=CodeCompliance), data = mySubset.raw.lvldropped)
						# plot(p.plot.before2B)

						p.plot.before2C <- ggplot(data=mySubset.raw.lvldropped,aes(x=PV_codeitem_value,colour=factor(SPL_county),fill=factor(SPL_county))) + geom_histogram()
						p.plot.before2C <- p.plot.before2C + theme(axis.text.x = element_text(angle=90,color="black",size=10),axis.text.y = element_text(color="black"),legend.position="") 
						p.plot.before2C <- p.plot.before2C + labs(x=paste(this.codeItem.raw,"(",this.codeitem.name,")",sep=" "),y="count",title=paste(string.thisState,": ( ",this.codeItem.raw,")-(",this.codeitem.name,"\nNo. Observations: ",no.Subset.raw,sep=""))
					      #p.plot.before2C <- p.plot.before2C + geom_vline(aes(xintercept=CodeCompliance), data = mySubset.raw.lvldropped)
						# plot(p.plot.before2C)

						p.plot.before2D <- ggplot(data=mySubset.raw.lvldropped,aes(x=PV_codeitem_value,colour=factor(SPL_bldg_name),fill=factor(SPL_bldg_name))) + geom_histogram()
						p.plot.before2D <- p.plot.before2D + theme(axis.text.x = element_text(angle=90,color="black",size=10),axis.text.y = element_text(color="black"),legend.position="") 
						p.plot.before2D <- p.plot.before2D + labs(x=paste(this.codeItem.raw,"(",this.codeitem.name,")",sep=" "),y="count",title=paste(string.thisState,": ( ",this.codeItem.raw,")-(",this.codeitem.name,"\nNo. Observations: ",no.Subset.raw,sep=""))
					      #p.plot.before2D <- p.plot.before2D + geom_vline(aes(xintercept=CodeCompliance), data = mySubset.raw.lvldropped)
						# plot(p.plot.before2D)	

						dev.set(3)
						multiplot(p.plot.before2A,p.plot.before2B,p.plot.before2C,p.plot.before2D,cols=2)




						p.plot.before22A <- ggplot(data=mySubset.raw.lvldropped,aes(x=PV_codeitem_value,colour=factor(PV_codeitem_value),fill=factor(PV_codeitem_value))) + geom_histogram() + facet_wrap(~ SPL_CZ,ncol = 1)
						p.plot.before22A <- p.plot.before22A + theme(axis.text.x = element_text(angle=90,color="black",size=10),axis.text.y = element_text(color="black"),legend.position="") 
						p.plot.before22A <- p.plot.before22A + labs(x=paste(this.codeItem.raw,"(",this.codeitem.name,")",sep=" "),y="count",title=paste(string.thisState,": ( ",this.codeItem.raw,")-(",this.codeitem.name,"\nNo. Observations: ",no.Subset.raw,sep=""))
					      #p.plot.before22A <- p.plot.before22A + geom_vline(aes(xintercept=CodeCompliance), data = mySubset.raw.lvldropped)
						# plot(p.plot.before22A)

					      # p.plot.before22B <- qplot(data=mySubset.raw.lvldropped,PV_codeitem_value,colour=SPL_CZ,fill=SPL_CZ,geom="histogram",facets=SPL_CZ~.) 
						p.plot.before22B <- ggplot(data=mySubset.raw.lvldropped,aes(x=PV_codeitem_value,colour=factor(SPL_CZ),fill=factor(SPL_CZ))) + geom_histogram() + facet_wrap(~ SPL_CZ,ncol = 1)
						p.plot.before22B <- p.plot.before22B + theme(axis.text.x = element_text(angle=90,color="black",size=10),axis.text.y = element_text(color="black"),legend.position="") 
						p.plot.before22B <- p.plot.before22B + labs(x=paste(this.codeItem.raw,"(",this.codeitem.name,")",sep=" "),y="count",title=paste(string.thisState,": ( ",this.codeItem.raw,")-(",this.codeitem.name,"\nNo. Observations: ",no.Subset.raw,sep=""))
					      #p.plot.before22B <- p.plot.before22B + geom_vline(aes(xintercept=CodeCompliance), data = mySubset.raw.lvldropped)
						# plot(p.plot.before22B)

					      # p.plot.before22C <- qplot(data=mySubset.raw.lvldropped,PV_codeitem_value,colour=SPL_bldg_name,fill=SPL_bldg_name,geom="histogram",facets=SPL_CZ~.) 
						p.plot.before22C <- ggplot(data=mySubset.raw.lvldropped,aes(x=PV_codeitem_value,colour=factor(SPL_bldg_name),fill=factor(SPL_bldg_name))) + geom_histogram() + facet_wrap(~ SPL_CZ,ncol = 1)
						p.plot.before22C <- p.plot.before22C + theme(axis.text.x = element_text(angle=90,color="black",size=10),axis.text.y = element_text(color="black"),legend.position="",legend.text=element_text(colour="black",size=10))	# ,legend.direction = "horizontal") 
						p.plot.before22C <- p.plot.before22C + labs(x=paste(this.codeItem.raw,"(",this.codeitem.name,")",sep=" "),y="count",title=paste(string.thisState,": ( ",this.codeItem.raw,")-(",this.codeitem.name,"\nNo. Observations: ",no.Subset.raw,sep=""))
					      #p.plot.before22C <- p.plot.before22C + geom_vline(aes(xintercept=CodeCompliance), data = mySubset.raw.lvldropped)
						# plot(p.plot.before22C)

					      # p.plot.before22D <- qplot(data=mySubset.raw.lvldropped,PV_codeitem_value,colour=SPL_county,fill=SPL_county,geom="histogram",facets=SPL_CZ~.) 
						p.plot.before22D <- ggplot(data=mySubset.raw.lvldropped,aes(x=PV_codeitem_value,colour=factor(SPL_county),fill=factor(SPL_county))) + geom_histogram() + facet_wrap(~ SPL_CZ,ncol = 1)
						p.plot.before22D <- p.plot.before22D + theme(axis.text.x = element_text(angle=90,color="black",size=10),axis.text.y = element_text(color="black"),legend.position="") 
						p.plot.before22D <- p.plot.before22D + labs(x=paste(this.codeItem.raw,"(",this.codeitem.name,")",sep=" "),y="count",title=paste(string.thisState,": ( ",this.codeItem.raw,")-(",this.codeitem.name,"\nNo. Observations: ",no.Subset.raw,sep=""))   
					      # p.plot.before22D <- p.plot.before22D + geom_vline(aes(xintercept=CodeCompliance), data = mySubset.raw.lvldropped)
						# plot(p.plot.before22D)			


						dev.set(3)
						multiplot(p.plot.before22A,p.plot.before22B,p.plot.before22C,p.plot.before22D,cols=2)	
			...
			cat(paste("665. ",string.thisState,": [plotting the original 18 key code items in the database.\n",sep=""))
			cat(paste("665. ",string.thisState,": [plotting the original 18 key code items in the database.\n",sep=""),file=FL.LOG, append=TRUE)	
			...
			multiplot(mplot.before.ach50,           mplot.before.highefflump,mplot.before.bsmtwallCavityR,mplot.before.bsmtwallContR,
			          mplot.before.ceilingR,        mplot.before.FlooRCavity,mplot.before.slabedgeR,
			          mplot.before.FrameWallRCavity,mplot.before.FrameWallRCont,mplot.before.windowU,    mplot.before.windowSHGC,     mplot.before.leakageratio,cols=4)
			         }else{
			multiplot(mplot.before.ach50,           mplot.before.highefflump,mplot.before.bsmtwallCavityR,mplot.before.bsmtwallContR,
			          mplot.before.ceilingR,        mplot.before.FlooRCavity,mplot.before.slabedgeR,
		...
		if (dim(myData.keyCode)[1] > 0)
		{
		  ...
		  names(myTable.bldg.wide.rev) <-                    c(var.other.more,var.keyCode.more.rev)
			...
			cat(paste("56. ",string.thisState,": add code name field after combing cavity and continuous insulation [KeyCode] in [myTable.bldg.long].\n",sep=""))
			cat(paste("56. ",string.thisState,": add code name field after combing cavity and continuous insulation [KeyCode] in [myTable.bldg.long].\n",sep=""),file=FL.LOG, append=TRUE)	
			...
			for (this.codeItem in list.codeItem[grep("^Comp",list.codeItem,invert=TRUE)])
			{
				...
				if (no.Subset > 0)
				{
					...
					if ((length(grep("[^0-9\\.]",mySubset.lvldropped[,"KeyCodeValue"]))) | (length(grep("(\\d*\\.+)(\\d*\\.+)(.*)$",mySubset.lvldropped[,"KeyCodeValue"]))))	
					{
						...
						if (this.state == "AllData")
						{
							p.plot.after1A <- qplot(data=mySubset.lvldropped,KeyCodeValue,facets=State~.,colour=factor(KeyCodeValue),fill=factor(KeyCodeValue),geom="histogram") 
							p.plot.after1A <- p.plot.after1A + theme(axis.text.x = element_text(angle=90,color="black",size=10),axis.text.y = element_text(color="black"),legend.position="") 
							p.plot.after1A <- p.plot.after1A + labs(x=paste(this.codeItem,"(",this.codeitem.name,")",sep=" "),y="count",title=paste(string.thisState,": ( ",this.codeItem,")-(",this.codeitem.name,")\n",vertical.line.string,"\nNo. Observations: ",no.Subset,string.multiple,sep=""))			
							p.plot.after1A <- p.plot.after1A + geom_vline(aes(xintercept=CodeCompliance), data = mySubset.lvldropped)
							p.plot.after1A <- p.plot.after1A + geom_vline(xintercept=c(this.codeItem.UL,this.codeItem.LL),colour="blue",linetype="longdash")
							# plot(p.plot.after1A)	

							p.plot.after1B <- qplot(data=mySubset.lvldropped,KeyCodeValue,facets=State~.,colour=ClimateZone,fill=ClimateZone,geom="histogram") 
							p.plot.after1B <- p.plot.after1B + theme(axis.text.x = element_text(angle=90,color="black",size=10),axis.text.y = element_text(color="black"),legend.position="") 
							p.plot.after1B <- p.plot.after1B + labs(x=paste(this.codeItem,"(",this.codeitem.name,")",sep=" "),y="count",title=paste(string.thisState,": ( ",this.codeItem,")-(",this.codeitem.name,")\n",vertical.line.string,"\nNo. Observations: ",no.Subset,string.multiple,sep=""))			
							p.plot.after1B <- p.plot.after1B + geom_vline(aes(xintercept=CodeCompliance), data = mySubset.lvldropped)
							# plot(p.plot.after1B)	

							p.plot.after1C <- qplot(data=mySubset.lvldropped,KeyCodeValue,facets=State~.,colour=SPL_bldg_name,fill=SPL_bldg_name,geom="histogram") 
							p.plot.after1C <- p.plot.after1C + theme(axis.text.x = element_text(angle=90,color="black",size=10),axis.text.y = element_text(color="black"),legend.position="") 
							p.plot.after1C <- p.plot.after1C + labs(x=paste(this.codeItem,"(",this.codeitem.name,")",sep=" "),y="count",title=paste(string.thisState,": ( ",this.codeItem,")-(",this.codeitem.name,")\n",vertical.line.string,"\nNo. Observations: ",no.Subset,string.multiple,sep=""))			
							p.plot.after1C <- p.plot.after1C + geom_vline(aes(xintercept=CodeCompliance), data = mySubset.lvldropped)
							# plot(p.plot.after1C)	

							p.plot.after1D <- qplot(data=mySubset.lvldropped,KeyCodeValue,facets=State~.,colour=County,fill=County,geom="histogram") 
							p.plot.after1D <- p.plot.after1D + theme(axis.text.x = element_text(angle=90,color="black",size=10),axis.text.y = element_text(color="black"),legend.position="") 
							p.plot.after1D <- p.plot.after1D + labs(x=paste(this.codeItem,"(",this.codeitem.name,")",sep=" "),y="count",title=paste(string.thisState,": ( ",this.codeItem,")-(",this.codeitem.name,")\n",vertical.line.string,"\nNo. Observations: ",no.Subset,string.multiple,sep=""))			
							p.plot.after1D <- p.plot.after1D + geom_vline(aes(xintercept=CodeCompliance), data = mySubset.lvldropped)
							# plot(p.plot.after1D)	
							
							
							dev.set(2)
							multiplot(p.plot.after1A,p.plot.after1B,p.plot.after1C,p.plot.after1D,cols=2)

							p.plot.after11A <- qplot(data=mySubset.lvldropped,KeyCodeValue,facets=ClimateZone~State,colour=factor(KeyCodeValue),fill=factor(KeyCodeValue),geom="histogram") 
							p.plot.after11A <- p.plot.after11A + theme(axis.text.x = element_text(angle=90,color="black",size=10),axis.text.y = element_text(color="black"),legend.position="") 
							p.plot.after11A <- p.plot.after11A + labs(x=paste(this.codeItem,"(",this.codeitem.name,")",sep=" "),y="count",title=paste(string.thisState,": ( ",this.codeItem,")-(",this.codeitem.name,")\n",vertical.line.string,"\nNo. Observations: ",no.Subset,string.multiple,sep=""))			
							p.plot.after11A <- p.plot.after11A + geom_vline(aes(xintercept=CodeCompliance), data = mySubset.lvldropped)
							# plot(p.plot.after11A)	

							p.plot.after11B <- qplot(data=mySubset.lvldropped,KeyCodeValue,facets=ClimateZone~State,colour=ClimateZone,fill=ClimateZone,geom="histogram") 
							p.plot.after11B <- p.plot.after11B + theme(axis.text.x = element_text(angle=90,color="black",size=10),axis.text.y = element_text(color="black"),legend.position="") 
							p.plot.after11B <- p.plot.after11B + labs(x=paste(this.codeItem,"(",this.codeitem.name,")",sep=" "),y="count",title=paste(string.thisState,": ( ",this.codeItem,")-(",this.codeitem.name,")\n",vertical.line.string,"\nNo. Observations: ",no.Subset,string.multiple,sep=""))			
							p.plot.after11B <- p.plot.after11B + geom_vline(aes(xintercept=CodeCompliance), data = mySubset.lvldropped)
							# plot(p.plot.after11B)	

							p.plot.after11C <- qplot(data=mySubset.lvldropped,KeyCodeValue,facets=ClimateZone~State,colour=SPL_bldg_name,fill=SPL_bldg_name,geom="histogram") 
							p.plot.after11C <- p.plot.after11C + theme(axis.text.x = element_text(angle=90,color="black",size=10),axis.text.y = element_text(color="black"),legend.position="") 
							p.plot.after11C <- p.plot.after11C + labs(x=paste(this.codeItem,"(",this.codeitem.name,")",sep=" "),y="count",title=paste(string.thisState,": ( ",this.codeItem,")-(",this.codeitem.name,")\n",vertical.line.string,"\nNo. Observations: ",no.Subset,string.multiple,sep=""))			
							p.plot.after11C <- p.plot.after11C + geom_vline(aes(xintercept=CodeCompliance), data = mySubset.lvldropped)
							# plot(p.plot.after11C)	

							p.plot.after11D <- qplot(data=mySubset.lvldropped,KeyCodeValue,facets=ClimateZone~State,colour=County,fill=County,geom="histogram") 
							p.plot.after11D <- p.plot.after11D + theme(axis.text.x = element_text(angle=90,color="black",size=10),axis.text.y = element_text(color="black"),legend.position="") 
							p.plot.after11D <- p.plot.after11D + labs(x=paste(this.codeItem,"(",this.codeitem.name,")",sep=" "),y="count",title=paste(string.thisState,": ( ",this.codeItem,")-(",this.codeitem.name,")\n",vertical.line.string,"\nNo. Observations: ",no.Subset,string.multiple,sep=""))			
							p.plot.after11D <- p.plot.after11D + geom_vline(aes(xintercept=CodeCompliance), data = mySubset.lvldropped)
							# plot(p.plot.after11D)


							dev.set(2)
							multiplot(p.plot.after11A,p.plot.after11B,p.plot.after11C,p.plot.after11D,cols=2)
						}else{
							p.plot.after2A <- ggplot(data=mySubset.lvldropped,aes(x=KeyCodeValue,colour=factor(KeyCodeValue),fill=factor(KeyCodeValue))) + geom_histogram()
							p.plot.after2A <- p.plot.after2A + theme(axis.text.x = element_text(angle=90,color="black",size=10),axis.text.y = element_text(color="black"),legend.position="")
							p.plot.after2A <- p.plot.after2A + labs(x=paste(this.codeItem,"(",this.codeitem.name,")",sep=" "),y="count",title=paste(string.thisState,": ( ",this.codeItem,")-(",this.codeitem.name,")\n",vertical.line.string,"\nNo. Observations: ",no.Subset,string.multiple,sep=""))
							p.plot.after2A <- p.plot.after2A + geom_vline(aes(xintercept=CodeCompliance), data = mySubset.lvldropped) 
							# plot(p.plot.after2A)
							
							p.plot.after2B <- ggplot(data=mySubset.lvldropped,aes(x=KeyCodeValue,colour=factor(ClimateZone),fill=factor(ClimateZone))) + geom_histogram()
							p.plot.after2B <- p.plot.after2B + theme(axis.text.x = element_text(angle=90,color="black",size=10),axis.text.y = element_text(color="black"),legend.position="") 
							p.plot.after2B <- p.plot.after2B + labs(x=paste(this.codeItem,"(",this.codeitem.name,")",sep=" "),y="count",title=paste(string.thisState,": ( ",this.codeItem,")-(",this.codeitem.name,")\n",vertical.line.string,"\nNo. Observations: ",no.Subset,string.multiple,sep=""))
							p.plot.after2B <- p.plot.after2B + geom_vline(aes(xintercept=CodeCompliance), data = mySubset.lvldropped)
							# plot(p.plot.after2B)

							p.plot.after2C <- ggplot(data=mySubset.lvldropped,aes(x=KeyCodeValue,colour=factor(County),fill=factor(County))) + geom_histogram()
							p.plot.after2C <- p.plot.after2C + theme(axis.text.x = element_text(angle=90,color="black",size=10),axis.text.y = element_text(color="black"),legend.position="") 
							p.plot.after2C <- p.plot.after2C + labs(x=paste(this.codeItem,"(",this.codeitem.name,")",sep=" "),y="count",title=paste(string.thisState,": ( ",this.codeItem,")-(",this.codeitem.name,")\n",vertical.line.string,"\nNo. Observations: ",no.Subset,string.multiple,sep=""))
							p.plot.after2C <- p.plot.after2C + geom_vline(aes(xintercept=CodeCompliance), data = mySubset.lvldropped)
							# plot(p.plot.after2C)

							p.plot.after2D <- ggplot(data=mySubset.lvldropped,aes(x=KeyCodeValue,colour=factor(SPL_bldg_name),fill=factor(SPL_bldg_name))) + geom_histogram()
							p.plot.after2D <- p.plot.after2D + theme(axis.text.x = element_text(angle=90,color="black",size=10),axis.text.y = element_text(color="black"),legend.position="") 
							p.plot.after2D <- p.plot.after2D + labs(x=paste(this.codeItem,"(",this.codeitem.name,")",sep=" "),y="count",title=paste(string.thisState,": ( ",this.codeItem,")-(",this.codeitem.name,")\n",vertical.line.string,"\nNo. Observations: ",no.Subset,string.multiple,sep=""))
							p.plot.after2D <- p.plot.after2D + geom_vline(aes(xintercept=CodeCompliance), data = mySubset.lvldropped)
							# plot(p.plot.after2D)	
							
							dev.set(2)
							multiplot(p.plot.after2A,p.plot.after2B,p.plot.after2C,p.plot.after2D,cols=2)
							



							p.plot.after22A <- ggplot(data=mySubset.lvldropped,aes(x=KeyCodeValue,colour=factor(KeyCodeValue),fill=factor(KeyCodeValue))) + geom_histogram() + facet_wrap(~ ClimateZone,ncol = 1)
							p.plot.after22A <- p.plot.after22A + theme(axis.text.x = element_text(angle=90,color="black",size=10),axis.text.y = element_text(color="black"),legend.position="") 
							p.plot.after22A <- p.plot.after22A + labs(x=paste(this.codeItem,"(",this.codeitem.name,")",sep=" "),y="count",title=paste(string.thisState,": ( ",this.codeItem,")-(",this.codeitem.name,")\n",vertical.line.string,"\nNo. Observations: ",no.Subset,string.multiple,sep=""))
							p.plot.after22A <- p.plot.after22A + geom_vline(aes(xintercept=CodeCompliance), data = mySubset.lvldropped)
							# plot(p.plot.after22A)

					              # p.plot.after22B <- qplot(data=mySubset.lvldropped,KeyCodeValue,colour=ClimateZone,fill=ClimateZone,geom="histogram",facets=ClimateZone~.) 
							p.plot.after22B <- ggplot(data=mySubset.lvldropped,aes(x=KeyCodeValue,colour=factor(ClimateZone),fill=factor(ClimateZone))) + geom_histogram() + facet_wrap(~ ClimateZone,ncol = 1)
							p.plot.after22B <- p.plot.after22B + theme(axis.text.x = element_text(angle=90,color="black",size=10),axis.text.y = element_text(color="black"),legend.position="") 
							p.plot.after22B <- p.plot.after22B + labs(x=paste(this.codeItem,"(",this.codeitem.name,")",sep=" "),y="count",title=paste(string.thisState,": ( ",this.codeItem,")-(",this.codeitem.name,")\n",vertical.line.string,"\nNo. Observations: ",no.Subset,string.multiple,sep=""))
							p.plot.after22B <- p.plot.after22B + geom_vline(aes(xintercept=CodeCompliance), data = mySubset.lvldropped)
							# plot(p.plot.after22B)

					              # p.plot.after22C <- qplot(data=mySubset.lvldropped,KeyCodeValue,colour=SPL_bldg_name,fill=SPL_bldg_name,geom="histogram",facets=ClimateZone~.) 
							p.plot.after22C <- ggplot(data=mySubset.lvldropped,aes(x=KeyCodeValue,colour=factor(SPL_bldg_name),fill=factor(SPL_bldg_name))) + geom_histogram() + facet_wrap(~ ClimateZone,ncol = 1)
							p.plot.after22C <- p.plot.after22C + theme(axis.text.x = element_text(angle=90,color="black",size=10),axis.text.y = element_text(color="black"),legend.position="",legend.text=element_text(colour="black",size=10))	# ,legend.direction = "horizontal") 
							p.plot.after22C <- p.plot.after22C + labs(x=paste(this.codeItem,"(",this.codeitem.name,")",sep=" "),y="count",title=paste(string.thisState,": ( ",this.codeItem,")-(",this.codeitem.name,")\n",vertical.line.string,"\nNo. Observations: ",no.Subset,string.multiple,sep=""))
							p.plot.after22C <- p.plot.after22C + geom_vline(aes(xintercept=CodeCompliance), data = mySubset.lvldropped)
							# plot(p.plot.after22C)

					              # p.plot.after22D <- qplot(data=mySubset.lvldropped,KeyCodeValue,colour=County,fill=County,geom="histogram",facets=ClimateZone~.) 
							p.plot.after22D <- ggplot(data=mySubset.lvldropped,aes(x=KeyCodeValue,colour=factor(County),fill=factor(County))) + geom_histogram() + facet_wrap(~ ClimateZone,ncol = 1)
							p.plot.after22D <- p.plot.after22D + theme(axis.text.x = element_text(angle=90,color="black",size=10),axis.text.y = element_text(color="black"),legend.position="") 
							p.plot.after22D <- p.plot.after22D + labs(x=paste(this.codeItem,"(",this.codeitem.name,")",sep=" "),y="count",title=paste(string.thisState,": ( ",this.codeItem,")-(",this.codeitem.name,")\n",vertical.line.string,"\nNo. Observations: ",no.Subset,string.multiple,sep=""))
							p.plot.after22D <- p.plot.after22D + geom_vline(aes(xintercept=CodeCompliance), data = mySubset.lvldropped)
							# plot(p.plot.after22D)			


							dev.set(2)
							multiplot(p.plot.after22A,p.plot.after22B,p.plot.after22C,p.plot.after22D,cols=2)	
						...
						if (this.codeItem == "FI17")
						{
							mplot.after.ach50 <- p.plot.after22A
						}else if(this.codeItem == "FI1")
						{
							mplot.after.ceilingR <- p.plot.after22A
						}else if(this.codeItem == "FI4")
						{
							mplot.after.leakageratio <- p.plot.after22A
						}else if(this.codeItem == "FI6")
						{
							mplot.after.highefflump <- p.plot.after22A
						}else if(this.codeItem == "FO1")
						{
							mplot.after.slabedgeR <- p.plot.after22A
						}else if(this.codeItem == "FO4a")
						{
							mplot.after.bsmtwallCavityR <- p.plot.after22A
						}else if(this.codeItem == "FO4b")
						{
							mplot.after.bsmtwallContR <- p.plot.after22A
						}else if(this.codeItem == "FR2")
						{
							mplot.after.windowU <- p.plot.after22A
						}else if(this.codeItem == "FR3")
						{
							mplot.after.windowSHGC <- p.plot.after22A
						}else if(this.codeItem == "IN1a")
						{
							mplot.after.FlooRCavity <- p.plot.after22A
						}else if(this.codeItem == "IN1b")
						{
							mplot.after.FlooRCont <- p.plot.after22A
						}else if(this.codeItem == "IN3a")
						{
							mplot.after.FrameWallRCavity <- p.plot.after22A
						}else if(this.codeItem == "IN3b")
						{
							mplot.after.FrameWallRCont <- p.plot.after22A
						}else if(this.codeItem == "FO4a")
						{
							mplot.after.BsmtWallRCavity <- p.plot.after22A
						}else if(this.codeItem == "FO4b")
						{
							mplot.after.BsmtWallRCont <- p.plot.after22A
						}
					}		# end of judging if it is a numeric field
				}
			}
			...
			{
				dev.set(5)
				multiplot(mplot.after.ach50,           mplot.after.highefflump,mplot.after.ceilingR,        
				          mplot.after.FrameWallRCavity,mplot.after.windowU,    mplot.after.windowSHGC,     mplot.after.leakageratio,cols=3)
			}else{
				dev.set(5)
						multiplot(mplot.after.ach50,           mplot.after.highefflump,mplot.after.bsmtwallCavityR,mplot.after.bsmtwallContR,
						          mplot.after.ceilingR,        mplot.after.FlooRCavity,mplot.after.FlooRCont,      mplot.after.slabedgeR,
						          mplot.after.FrameWallRCavity,mplot.after.windowU,    mplot.after.windowSHGC,     mplot.after.leakageratio,cols=4)
			}

			# multiplot(mplot.after.ach50,mplot.after.ceilingR,mplot.after.leakageratio,mplot.after.highefflump,mplot.after.slabedgeR,mplot.after.bsmtwallCavityR,mplot.after.bsmtwallContR,mplot.after.windowU,mplot.after.windowSHGC,mplot.after.FlooRCavity,mplot.after.FlooRCont,mplot.after.FrameWallRCavity,mplot.after.FrameWallRCont,mplot.after.BsmtWallRCavity,mplot.after.BsmtWallRCont,cols=2)
			# &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
			# &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
			# &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
			# PLOT AFTER CLEANNING  [myData.clean.bldg.wide]
			...
			cat(paste("99. there is no keyitem data retrieved for ",this.state," at ",timeStamp.string,".\n\n",sep=""))
			cat(paste("99. there is no keyitem data retriveed for ",this.state," at ",timeStamp.string,".\n\n",sep=""),file=FL.LOG, append=TRUE)								
		}

	}else{		# end of non-zero [myData.thisState.cleaned] data condition
		cat(paste("100. there is no data retrieved for ",this.state," at ",timeStamp.string,".\n\n",sep=""))
		cat(paste("100. there is no data retriveed for ",this.state," at ",timeStamp.string,".\n\n",sep=""),file=FL.LOG, append=TRUE)					
	}
...
cat(paste("\n04B_prepare_keyItem_for_simulation_getPlots.R is finished successfully at ",End.time,"!\n",sep=" "))
cat(paste("\n04B_prepare_keyItem_for_simulation_getPlots.R is finished successfully at ",End.time,"!\n",sep=" "),file=FL.LOG,append=TRUE)

cat(paste("\nProcessing time for [04B_prepare_keyItem_for_simulation_getPlots.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "))
cat(paste("\nProcessing time for [04B_prepare_keyItem_for_simulation_getPlots.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "),file=FL.LOG,append=TRUE)
