#
# 01_extraction_for_QA_stateOnly.R 
#
...
# -------------------------------------------------------------------------------------------------
Path.TMP   <- paste(Path.Project,"01_extraction_for_QA_stateOnly",  sep="/")				# OUTPUT processed result directory
...
# ------------------------------------------------------------------------------------------------- 
FL.LOG          <- paste(Path.LOG,paste("01_extraction_for_QA_stateOnly_",timeStamp.string,".LOG",    sep=""),sep="/")		
...
cat(paste("Log file for data processing script [01_extraction_for_QA_stateOnly.R]!\n",sep=""))
...
for (this.state in array.states)
{
	...
	command.string1 <- paste("myQuery.command  <- ",	                                              
		...
		paste("\"SELECT  USER_PROFILE.mail                                       as UP_mail,                   \n",
                                ...
                                "PRESCRIPTIVE_VALUES.checklist_item_answer               as PV_codeitem_ID             \n",
                                "	                                                                               \n",
                          paste("FROM  (select state,id,user_id,climate_zone,county,id_code,last_modified from SAMPLE where state = '",this.state,"') as SPL     \n",sep=""),
                                "             JOIN CHECKLIST_ITEM_ANSWER ON SPL.id = CHECKLIST_ITEM_ANSWER.sample                                                \n",
                                "        LEFT JOIN PRESCRIPTIVE_VALUES   ON CHECKLIST_ITEM_ANSWER.id  = PRESCRIPTIVE_VALUES.checklist_item_answer                \n",
                                "        LEFT JOIN USER_PROFILE          ON SPL.user_id = USER_PROFILE.id                                                     ;\"\n",
                                sep=" "),
                                sep="")
...
cat(paste("\n01_extraction_for_QA_stateOnly.R is finished successfully at ",End.time,"!\n",sep=" "))
cat(paste("\n01_extraction_for_QA_stateOnly.R is finished successfully at ",End.time,"!\n",sep=" "),file=FL.LOG,append=TRUE)

cat(paste("\nProcessing time for [01_extraction_for_QA_stateOnly.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "))
cat(paste("\nProcessing time for [01_extraction_for_QA_stateOnly.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "),file=FL.LOG,append=TRUE)
