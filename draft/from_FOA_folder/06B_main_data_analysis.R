...
Path.main.TMP <- paste(Path.Project,"06B_main_data_analysis",sep="/")
Path.main.OUT <- paste(Path.main.TMP,timeStamp.string,sep="/")
...
if (!file.exists(Path.main.TMP)){print(paste("NOT existing:",Path.main.TMP));dir.create(Path.main.OUT,showWarnings=TRUE,recursive=TRUE)}
...
FL.LOG <- paste(Path.LOG, "06B_main_data_analysis_revised.log",sep="/")
...
for (this.state in array.states[1])
{
	...
	cat(paste("10A: [",this.state,"]: the arrays for CZ, found and htgsys shares are updated based on field observation.\n",sep=""))
	cat(paste("10A: [",this.state,"]: the arrays for CZ, found and htgsys shares are updated based on field observation.\n",sep=""),file=FL.LOG,append=TRUE)
	...
	cat(paste("10B: [",this.state,"]: get the CZ shares in the state.\n",sep=""))
	cat(paste("10B: [",this.state,"]: get the CZ shares in the state.\n",sep=""),file=FL.LOG,append=TRUE)
	...
	Path.duct.In.State   <- paste(Path.duct.OUT, this.state,sep="/")
	Path.main.Out.State  <- paste(Path.main.OUT, this.state,sep="/")
	# Path.duct.Simu.State <- paste(Path.Simu,this.state,"duct.leakage",sep="/")	
	# Path.main.Simu.State <- paste(Path.Simu,this.state,sep="/")
	Path.duct.Simu.State <- paste(Path.Simu,"AL_June29_2015_Final","duct.leakage",sep="/")	
	Path.main.Simu.State <- paste(Path.Simu,"AL_June29_2015_Final",sep="/")	
	if (!file.exists(Path.main.Out.State)) {print(paste("NOT existing:",Path.main.Out.State)); dir.create(Path.main.Out.State, showWarnings=TRUE,recursive=TRUE)}
	if (!file.exists(Path.main.Simu.State)){print(paste("NOT existing:",Path.main.Simu.State,". Check Why!\n"));die}
	cat(paste("11A: [",this.state,"]: specified an output and a simulation folder for the state.\n",sep=""))
	cat(paste("11A: [",this.state,"]: specified an output and a simulation folder for the state.\n",sep=""),file=FL.LOG,append=TRUE)
	
	# define simulation subfolders
	Path.main.summary <- paste(Path.main.Simu.State,"summary.nobackup",sep="/")
	Path.duct.summary <- paste(Path.duct.Simu.State,"summary.nobackup",sep="/")
	if (!file.exists(Path.main.summary)){print(paste("NOT existing:",Path.main.summary),". Check Why!\n",sep="");die}
	if (!file.exists(Path.duct.summary)){print(paste("NOT existing:",Path.duct.summary),". Check Why!\n",sep="");die}
	cat(paste("11B: [",this.state,"]: summary file.\n",sep=""))
	cat(paste("11B: [",this.state,"]: summary file.\n",sep=""),file=FL.LOG,append=TRUE)
	
	# parm csv file of main and cut
	FL.main.parm <- paste(Path.main.Simu.State,"phase1_main_parm.csv",sep="/")
	FL.duct.parm <- paste(Path.duct.Simu.State,"phase1_duct_parm.csv",sep="/")
	if (!file.exists(FL.main.parm)){print(paste("NOT existing:",FL.main.parm),". Check Why!\n",sep="");die}
	if (!file.exists(FL.duct.parm)){print(paste("NOT existing:",FL.duct.parm),". Check Why!\n",sep="");die}
	cat(paste("11C: [",this.state,"]: parm file.\n",sep=""))
	cat(paste("11C: [",this.state,"]: parm file.\n",sep=""),file=FL.LOG,append=TRUE)
	
	
	
	# define E+ summary file for input
	FL.main.summary <- paste(Path.main.summary,"summary.csv",sep="/")
	if (!file.exists(FL.main.summary)){print(paste("NOT existing:",FL.main.summary),". Check Why!\n",sep="");die}
	cat(paste("12A: [",this.state,"]: define the E+ summary file name and path.\n",sep=""))
	cat(paste("12A: [",this.state,"]: define the E+ summary file name and path.\n",sep=""),file=FL.LOG,append=TRUE)
	
	# define duct leakage EUI change percentage file processed by "6A_duct_analysis.R"
	FL.duct.EUI.Change <- paste(Path.duct.In.State,paste("FOA_duct_EUI_Change.rds",      sep=""),sep="/")
	if (!file.exists(FL.duct.EUI.Change)){print(paste("NOT existing:",FL.duct.EUI.Change),". Check Why!\n",sep="");die}
	cat(paste("12B: [",this.state,"]: define the EUI change file from duct leakage simulation.\n",sep=""))
	cat(paste("12B: [",this.state,"]: define the EUI change file from duct leakage simulation.\n",sep=""),file=FL.LOG,append=TRUE)
	

	# define output file names
	FL.Rdata             <- paste(Path.main.Out.State,paste("FOA_data.Rdata",               sep=""),sep="/")
	FL.share.csv         <- paste(Path.main.Out.State,paste("FOA_Shares.csv",               sep=""),sep="/")	
	FL.RawData.csv       <- paste(Path.main.Out.State,paste("FOA_RawData.csv",              sep=""),sep="/")	
	FL.RawData.rds       <- paste(Path.main.Out.State,paste("FOA_RawData.rds",              sep=""),sep="/")	
	FL.Proc.Comp.csv     <- paste(Path.main.Out.State,paste("FOA_ProcData_Complete.csv",    sep=""),sep="/")	
	FL.Proc.Comp.rds     <- paste(Path.main.Out.State,paste("FOA_ProcData_Complete.rds",    sep=""),sep="/")	
	FL.Proc.Conc.csv     <- paste(Path.main.Out.State,paste("FOA_ProcData_Concise.csv",     sep=""),sep="/")	
	FL.Proc.Conc.rds     <- paste(Path.main.Out.State,paste("FOA_ProcData_Concise.rds",     sep=""),sep="/")	
	FL.Base.Advn.csv     <- paste(Path.main.Out.State,paste("FOA_Base_Advn.csv",            sep=""),sep="/")	
	FL.Base.Advn.obj     <- paste(Path.main.Out.State,paste("FOA_Base_Advn.Rdata",          sep=""),sep="/")	
	
	FL.EUI_dEUI.Raw.csv  <- paste(Path.main.Out.State,paste("FOA_EUI_dEUI_RawModel.csv",    sep=""),sep="/")	
	FL.EUI_dEUI.Raw.rds  <- paste(Path.main.Out.State,paste("FOA_EUI_dEUI_RawModel.rds",    sep=""),sep="/")	
	FL.EUI_dEUI.Raw.pdf  <- paste(Path.main.Out.State,paste("FOA_EUI_dEUI_RawModel.pdf",    sep=""),sep="/")	
	
	FL.EUI_dEUI.Agg.csv  <- paste(Path.main.Out.State,paste("FOA_EUI_dEUI_aggModel.csv",    sep=""),sep="/")	
	FL.EUI_dEUI.Agg.rds  <- paste(Path.main.Out.State,paste("FOA_EUI_dEUI_aggModel.rds",    sep=""),sep="/")	
	FL.EUI_dEUI.Agg.pdf  <- paste(Path.main.Out.State,paste("FOA_EUI_dEUI_aggModel.pdf",    sep=""),sep="/")	
	FL.EUI_dEUI.Agg.sum  <- paste(Path.main.Out.State,paste("FOA_EUI_dEUI_aggModel_sum.csv",sep=""),sep="/")
	
	FL.EUI_dEUI.rank.csv    <- paste(Path.main.Out.State,paste("FOA_EUI_dEUI_rankedModel.csv", sep=""),sep="/")

	FL.EUI.ductAdj.csv      <- paste(Path.main.Out.State,paste("FOA_EUI_ductAdj.csv",          sep=""),sep="/")
	FL.EUI.ductAdj.rds      <- paste(Path.main.Out.State,paste("FOA_EUI_ductAdj.rds",          sep=""),sep="/")	
	FL.EUI.ductAdj.pdf      <- paste(Path.main.Out.State,paste("FOA_EUI_ductAdj.pdf",          sep=""),sep="/")
	FL.EUI.ductAdj.rank.csv <- paste(Path.main.Out.State,paste("FOA_EUI_ductAdj_ranked.csv",   sep=""),sep="/")
	...
	if (file.exists(FL.EUI.ductAdj.csv)){print(paste(FL.EUI.ductAdj.csv,  "exist.Delete it!"));file.remove(FL.EUI.ductAdj.csv)}
	if (file.exists(FL.EUI.ductAdj.rds)){print(paste(FL.EUI.ductAdj.rds,  "exist.Delete it!"));file.remove(FL.EUI.ductAdj.rds)}
	if (file.exists(FL.EUI.ductAdj.pdf)){print(paste(FL.EUI.ductAdj.pdf,  "exist.Delete it!"));file.remove(FL.EUI.ductAdj.pdf)}
	if (file.exists(FL.EUI.ductAdj.rank.csv)){print(paste(FL.EUI.ductAdj.rank.csv,  "exist.Delete it!"));file.remove(FL.EUI.ductAdj.rank.csv)}
	...
	pdf(file = FL.EUI.ductAdj.pdf, paper="special", width=17, height=11,bg = "transparent")	
	...
	# June 29, 2015: the output fileds changes due to recent template modification.
 	fields.raw.common <- c("idx",  "ID","Total.Site.Energy.kBtu","Total.Source.Energy.kBtu",         "Site.EUI.kBtu.sqft",           "Source.EUI.kBtu.sqft","Total.Electricity.kWh","Total.Natural.Gas.therm",  "Heating.kWh",  "Heating.therm",  "Cooling.kWh",  "Cooling.therm",  "Interior.Lighting.kWh",  "Interior.Lighting.therm","Interior.Equipment.kWh","Interior.Equipment.therm","Fans.kWh","Fans.therm",  "Water.Systems.kWh",  "Water.Systems.therm",  "Fan_CoolingOp",      "Fan_HeatingOp",    "Location",   "ach50","afn_control","aspect_ratio","cfa_total",  "code",  "dhw_type",  "doe_climate_zone",  "doe_moisture_regime",   "f_inc_hw",  "fndn_type",  "heating_fuel",   "ins_qual_bsmtwall",   "ins_qual_ceiling",   "ins_qual_floor",   "ins_qual_wall",   "leakage_ratio",  "location","n_stories_per_unit","n_units",  "prototype",   "r_bsmtwall",   "r_ceiling",   "r_floor",   "r_sheathing",   "r_slab",   "r_wall",  "runbsmt",  "runslab",   "shgc_window",  "system",  "tag",   "u_window",  "vent_sch",  "weatherfile",  "wfr","  cfa_per_unit","ela","f_cfl_hw","f_inc_hw.1","f_led_hw","f_lf_hw")
	fields.common     <- c("idx","S.ID","Total.Site.Energy.kBtu","Total.Source.Energy.kBtu",         "Site.EUI.kBtu.sqft",           "Source.EUI.kBtu.sqft","Total.Electricity.kWh","Total.Natural.Gas.therm","S.Heating.kWh","S.Heating.therm","S.Cooling.kWh","S.Cooling.therm","S.Interior.Lighting.kWh","S.Interior.Lighting.therm","Interior.Equipment.kWh","Interior.Equipment.therm","Fans.kWh","Fans.therm","S.Water.Systems.kWh","S.Water.Systems.therm","S.Fan_CoolingOp.kWh","S.Fan_HeatingOp.kWh","Location","SS.ach50","afn_control","aspect_ratio","cfa_total","S.code","S.dhw_type","S.doe_climate_zone","S.doe_moisture_regime","SS.f_inc_hw","S.fndn_type","S.heating_fuel","SS.ins_qual_bsmtwall","SS.ins_qual_ceiling","SS.ins_qual_floor","SS.ins_qual_wall","SS.leakage_ratio","S.Location","n_stories_per_unit","n_units","S.prototype","SS.r_bsmtwall","SS.r_ceiling","SS.r_floor","SS.r_sheathing","SS.r_slab","SS.r_wall","S.runbsmt","S.runslab","SS.shgc_window","S.system","S.tag","SS.u_window","S.vent_sch","S.weatherfile","S.wfr","S.cfa_per_unit","ela","f_cfl_hw","f_inc_hw.1","f_led_hw","f_lf_hw") 
	...
	# ---------------------------------------------------------------------------------
	myData.raw <- read.table(file=FL.main.summary,header=TRUE,sep=",",stringsAsFactors=FALSE)
  ...
  names(Part.Diff) <- paste("DIFF",names(Part.Diff),sep="_")	
	...
	mySubset.4plot <- subset(myEUI.raw.long,subset=(Variable == "PRO_SS.ach50"))
	this.codeItem  <- "ACH50"
	this.TRB       <- "TRB_SS.ach50"
	this.CZ        <- "PRO_SS.CZ"	
	...
	# 2. high eff lump
	if(exists("mySubset.4plot")  && is.data.frame(get("mySubset.4plot"))) {rm(mySubset.4plot)}
	mySubset.4plot <- subset(myEUI.raw.long,subset=(Variable == "PRO_SS.f_inc_hw"))
	this.codeItem  <- "HighEff Lump"
	this.TRB       <- "TRB_SS.f_inc_hw"
	this.CZ        <- "PRO_SS.CZ"	
	names(mySubset.4plot) <- sub(this.CZ,"CZ",sub(this.TRB,"CodeCompliance",sub("value","KeyCodeValue",names(mySubset.4plot))))
	p.plot02 <- ggplot(data=mySubset.4plot,aes(x=KeyCodeValue,colour=factor(CZ),fill=factor(CZ))) + geom_histogram() + facet_wrap(~ CZ,ncol = 1)
	p.plot02 <- p.plot02 + theme(axis.text.x = element_text(angle=90,color="black",size=10),axis.text.y = element_text(color="black"),legend.position="")
	p.plot02 <- p.plot02 + labs(x=this.codeItem,y="count",title=paste(paste("Sampled values of [",this.codeItem,"] in simulation at [",this.state,"]",sep=""),"\n",sep=""))
	p.plot02 <- p.plot02 + geom_vline(aes(xintercept=CodeCompliance), data =mySubset.4plot) 
	
	# 3. r_bsmtwall
	if(exists("mySubset.4plot")  && is.data.frame(get("mySubset.4plot"))) {rm(mySubset.4plot)}
	mySubset.4plot <- subset(myEUI.raw.long,subset=(Variable == "PRO_SS.r_bsmtwall"))
	this.codeItem  <- "BsmtWallR"
	this.TRB       <- "TRB_SS.r_bsmtwall"
	this.CZ        <- "PRO_SS.CZ"	
	names(mySubset.4plot) <- sub(this.CZ,"CZ",sub(this.TRB,"CodeCompliance",sub("value","KeyCodeValue",names(mySubset.4plot))))
	p.plot03 <- ggplot(data=mySubset.4plot,aes(x=KeyCodeValue,colour=factor(CZ),fill=factor(CZ))) + geom_histogram() + facet_wrap(~ CZ,ncol = 1)
	p.plot03 <- p.plot03 + theme(axis.text.x = element_text(angle=90,color="black",size=10),axis.text.y = element_text(color="black"),legend.position="")
	p.plot03 <- p.plot03 + labs(x=this.codeItem,y="count",title=paste(paste("Sampled values of [",this.codeItem,"] in simulation at [",this.state,"]",sep=""),"\n",sep=""))
	p.plot03 <- p.plot03 + geom_vline(aes(xintercept=CodeCompliance), data =mySubset.4plot) 
	
	# 4. CeilingR
	if(exists("mySubset.4plot")  && is.data.frame(get("mySubset.4plot"))) {rm(mySubset.4plot)}
	mySubset.4plot <- subset(myEUI.raw.long,subset=(Variable == "PRO_SS.r_ceiling"))
	this.codeItem  <- "CeilingR"
	this.TRB       <- "TRB_SS.r_ceiling"
	this.CZ        <- "PRO_SS.CZ"	
	names(mySubset.4plot) <- sub(this.CZ,"CZ",sub(this.TRB,"CodeCompliance",sub("value","KeyCodeValue",names(mySubset.4plot))))
	p.plot04 <- ggplot(data=mySubset.4plot,aes(x=KeyCodeValue,colour=factor(CZ),fill=factor(CZ))) + geom_histogram() + facet_wrap(~ CZ,ncol = 1)
	p.plot04 <- p.plot04 + theme(axis.text.x = element_text(angle=90,color="black",size=10),axis.text.y = element_text(color="black"),legend.position="")
	p.plot04 <- p.plot04 + labs(x=this.codeItem,y="count",title=paste(paste("Sampled values of [",this.codeItem,"] in simulation at [",this.state,"]",sep=""),"\n",sep=""))
	p.plot04 <- p.plot04 + geom_vline(aes(xintercept=CodeCompliance), data =mySubset.4plot) 
	
	# 5. FloorR
	if(exists("mySubset.4plot")  && is.data.frame(get("mySubset.4plot"))) {rm(mySubset.4plot)}
	mySubset.4plot <- subset(myEUI.raw.long,subset=(Variable == "PRO_SS.r_floor"))
	this.codeItem  <- "FloorR"
	this.TRB       <- "TRB_SS.r_floor"
	this.CZ        <- "PRO_SS.CZ"	
	names(mySubset.4plot) <- sub(this.CZ,"CZ",sub(this.TRB,"CodeCompliance",sub("value","KeyCodeValue",names(mySubset.4plot))))
	p.plot05 <- ggplot(data=mySubset.4plot,aes(x=KeyCodeValue,colour=factor(CZ),fill=factor(CZ))) + geom_histogram() + facet_wrap(~ CZ,ncol = 1)
	p.plot05 <- p.plot05 + theme(axis.text.x = element_text(angle=90,color="black",size=10),axis.text.y = element_text(color="black"),legend.position="")
	p.plot05 <- p.plot05 + labs(x=this.codeItem,y="count",title=paste(paste("Sampled values of [",this.codeItem,"] in simulation at [",this.state,"]",sep=""),"\n",sep=""))
	p.plot05 <- p.plot05 + geom_vline(aes(xintercept=CodeCompliance), data =mySubset.4plot) 
	
	# 6. SlabEdgeR
	if(exists("mySubset.4plot")  && is.data.frame(get("mySubset.4plot"))) {rm(mySubset.4plot)}
	mySubset.4plot <- subset(myEUI.raw.long,subset=(Variable == "PRO_SS.r_slab"))
	this.codeItem  <- "SlabEdgeR"
	this.TRB       <- "TRB_SS.r_slab"
	this.CZ        <- "PRO_SS.CZ"	
	names(mySubset.4plot) <- sub(this.CZ,"CZ",sub(this.TRB,"CodeCompliance",sub("value","KeyCodeValue",names(mySubset.4plot))))
	p.plot06 <- ggplot(data=mySubset.4plot,aes(x=KeyCodeValue,colour=factor(CZ),fill=factor(CZ))) + geom_histogram() + facet_wrap(~ CZ,ncol = 1)
	p.plot06 <- p.plot06 + theme(axis.text.x = element_text(angle=90,color="black",size=10),axis.text.y = element_text(color="black"),legend.position="")
	p.plot06 <- p.plot06 + labs(x=this.codeItem,y="count",title=paste(paste("Sampled values of [",this.codeItem,"] in simulation at [",this.state,"]",sep=""),"\n",sep=""))
	p.plot06 <- p.plot06 + geom_vline(aes(xintercept=CodeCompliance), data =mySubset.4plot) 
	
	# 7. FrameWallR
	if(exists("mySubset.4plot")  && is.data.frame(get("mySubset.4plot"))) {rm(mySubset.4plot)}
	mySubset.4plot <- subset(myEUI.raw.long,subset=(Variable == "PRO_SS.r_wall"))
	this.codeItem  <- "FrameWallR"
	this.TRB       <- "TRB_SS.r_wall"
	this.CZ        <- "PRO_SS.CZ"	
	names(mySubset.4plot) <- sub(this.CZ,"CZ",sub(this.TRB,"CodeCompliance",sub("value","KeyCodeValue",names(mySubset.4plot))))
	p.plot07 <- ggplot(data=mySubset.4plot,aes(x=KeyCodeValue,colour=factor(CZ),fill=factor(CZ))) + geom_histogram() + facet_wrap(~ CZ,ncol = 1)
	p.plot07 <- p.plot07 + theme(axis.text.x = element_text(angle=90,color="black",size=10),axis.text.y = element_text(color="black"),legend.position="")
	p.plot07 <- p.plot07 + labs(x=this.codeItem,y="count",title=paste(paste("Sampled values of [",this.codeItem,"] in simulation at [",this.state,"]",sep=""),"\n",sep=""))
	p.plot07 <- p.plot07 + geom_vline(aes(xintercept=CodeCompliance), data =mySubset.4plot) 
	
	# 8. WindowU
	if(exists("mySubset.4plot")  && is.data.frame(get("mySubset.4plot"))) {rm(mySubset.4plot)}
	mySubset.4plot <- subset(myEUI.raw.long,subset=(Variable == "PRO_SS.u_window"))
	this.codeItem  <- "WindowU"
	this.TRB       <- "TRB_SS.u_window"
	this.CZ        <- "PRO_SS.CZ"	
	names(mySubset.4plot) <- sub(this.CZ,"CZ",sub(this.TRB,"CodeCompliance",sub("value","KeyCodeValue",names(mySubset.4plot))))
	p.plot08 <- ggplot(data=mySubset.4plot,aes(x=KeyCodeValue,colour=factor(CZ),fill=factor(CZ))) + geom_histogram() + facet_wrap(~ CZ,ncol = 1)
	p.plot08 <- p.plot08 + theme(axis.text.x = element_text(angle=90,color="black",size=10),axis.text.y = element_text(color="black"),legend.position="")
	p.plot08 <- p.plot08 + labs(x=this.codeItem,y="count",title=paste(paste("Sampled values of [",this.codeItem,"] in simulation at [",this.state,"]",sep=""),"\n",sep=""))
	p.plot08 <- p.plot08 + geom_vline(aes(xintercept=CodeCompliance), data =mySubset.4plot) 
	

	# 9. WindowSHGC
	if(exists("mySubset.4plot")  && is.data.frame(get("mySubset.4plot"))) {rm(mySubset.4plot)}
	mySubset.4plot <- subset(myEUI.raw.long,subset=(Variable == "PRO_SS.shgc_window"))
	this.codeItem  <- "WindowSHGC"
	this.TRB       <- "TRB_SS.shgc_window"
	this.CZ        <- "PRO_SS.CZ"	
	names(mySubset.4plot) <- sub(this.CZ,"CZ",sub(this.TRB,"CodeCompliance",sub("value","KeyCodeValue",names(mySubset.4plot))))
	p.plot09 <- ggplot(data=mySubset.4plot,aes(x=KeyCodeValue,colour=factor(CZ),fill=factor(CZ))) + geom_histogram() + facet_wrap(~ CZ,ncol = 1)
	p.plot09 <- p.plot09 + theme(axis.text.x = element_text(angle=90,color="black",size=10),axis.text.y = element_text(color="black"),legend.position="")
	p.plot09 <- p.plot09 + labs(x=this.codeItem,y="count",title=paste(paste("Sampled values of [",this.codeItem,"] in simulation at [",this.state,"]",sep=""),"\n",sep=""))
	p.plot09 <- p.plot09 + geom_vline(aes(xintercept=CodeCompliance), data =mySubset.4plot) 
	

	# 10. LeakageRatio
	if(exists("mySubset.4plot")  && is.data.frame(get("mySubset.4plot"))) {rm(mySubset.4plot)}
	mySubset.4plot <- subset(myEUI.raw.long,subset=(Variable == "PRO_SS.leakage_ratio"))
	this.codeItem  <- "LeakageRatio"
	this.TRB       <- "TRB_SS.leakage_ratio"
	this.CZ        <- "PRO_SS.CZ"	
	names(mySubset.4plot) <- sub(this.CZ,"CZ",sub(this.TRB,"CodeCompliance",sub("value","KeyCodeValue",names(mySubset.4plot))))
	p.plot10 <- ggplot(data=mySubset.4plot,aes(x=KeyCodeValue,colour=factor(CZ),fill=factor(CZ))) + geom_histogram() + facet_wrap(~ CZ,ncol = 1)
	p.plot10 <- p.plot10 + theme(axis.text.x = element_text(angle=90,color="black",size=10),axis.text.y = element_text(color="black"),legend.position="")
	p.plot10 <- p.plot10 + labs(x=this.codeItem,y="count",title=paste(paste("Sampled values of [",this.codeItem,"] in simulation at [",this.state,"]",sep=""),"\n",sep=""))
	p.plot10 <- p.plot10 + geom_vline(aes(xintercept=CodeCompliance), data =mySubset.4plot) 
	

	# 11. IECC Regulated end-use Gas EUI
	if(exists("mySubset.4plot")  && is.data.frame(get("mySubset.4plot"))) {rm(mySubset.4plot)}
	mySubset.4plot <- subset(myEUI.raw.long,subset=(Variable == "PRO_SS.tot.EUI.gas"))
	this.codeItem  <- "IECC Regulated end-use Gas EUI"
	this.TRB       <- "TRB_SS.tot.EUI.gas"
	this.CZ        <- "PRO_SS.CZ"	
	names(mySubset.4plot) <- sub(this.CZ,"CZ",sub(this.TRB,"CodeCompliance",sub("value","KeyCodeValue",names(mySubset.4plot))))
	p.plot11 <- ggplot(data=mySubset.4plot,aes(x=KeyCodeValue,colour=factor(CZ),fill=factor(CZ))) + geom_histogram() + facet_wrap(~ CZ,ncol = 1)
	p.plot11 <- p.plot11 + theme(axis.text.x = element_text(angle=90,color="black",size=10),axis.text.y = element_text(color="black"),legend.position="")
	p.plot11 <- p.plot11 + labs(x=this.codeItem,y="count",title=paste(paste(this.codeItem," at [",this.state,"]",sep=""),"\n",sep=""))
	p.plot11 <- p.plot11 + geom_vline(aes(xintercept=CodeCompliance), data =mySubset.4plot) 
	

	# 12. IECC Regulated end-use Electricity EUI
	if(exists("mySubset.4plot")  && is.data.frame(get("mySubset.4plot"))) {rm(mySubset.4plot)}
	mySubset.4plot <- subset(myEUI.raw.long,subset=(Variable == "PRO_SS.tot.EUI.elec"))
	this.codeItem  <- "IECC Regulated end-use Elec EUI"
	this.TRB       <- "TRB_SS.tot.EUI.elec"
	this.CZ        <- "PRO_SS.CZ"	
	names(mySubset.4plot) <- sub(this.CZ,"CZ",sub(this.TRB,"CodeCompliance",sub("value","KeyCodeValue",names(mySubset.4plot))))
	p.plot12 <- ggplot(data=mySubset.4plot,aes(x=KeyCodeValue,colour=factor(CZ),fill=factor(CZ))) + geom_histogram() + facet_wrap(~ CZ,ncol = 1)
	p.plot12 <- p.plot12 + theme(axis.text.x = element_text(angle=90,color="black",size=10),axis.text.y = element_text(color="black"),legend.position="")
	p.plot12 <- p.plot12 + labs(x=this.codeItem,y="count",title=paste(paste(this.codeItem," at [",this.state,"]",sep=""),"\n",sep=""))
	p.plot12 <- p.plot12 + geom_vline(aes(xintercept=CodeCompliance), data =mySubset.4plot) 
	
	# 13. IECC Regulated end-use Total EUI
	if(exists("mySubset.4plot")  && is.data.frame(get("mySubset.4plot"))) {rm(mySubset.4plot)}
	mySubset.4plot <- subset(myEUI.raw.long,subset=(Variable == "PRO_SS.tot.EUI.sum"))
	this.codeItem  <- "IECC Regulated end-use Total EUI"
	this.TRB       <- "TRB_SS.tot.EUI.sum"
	this.CZ        <- "PRO_SS.CZ"	
	names(mySubset.4plot) <- sub(this.CZ,"CZ",sub(this.TRB,"CodeCompliance",sub("value","KeyCodeValue",names(mySubset.4plot))))
	p.plot13 <- ggplot(data=mySubset.4plot,aes(x=KeyCodeValue,colour=factor(CZ),fill=factor(CZ))) + geom_histogram() + facet_wrap(~ CZ,ncol = 1)
	p.plot13 <- p.plot13 + theme(axis.text.x = element_text(angle=90,color="black",size=10),axis.text.y = element_text(color="black"),legend.position="")
	p.plot13 <- p.plot13 + labs(x=this.codeItem,y="count",title=paste(paste(this.codeItem," at [",this.state,"]",sep=""),"\n",sep=""))
	p.plot13 <- p.plot13 + geom_vline(aes(xintercept=CodeCompliance), data =mySubset.4plot) 	

	dev.set(2)
	multiplot(p.plot01,p.plot02,p.plot03,p.plot04,p.plot05,p.plot06,p.plot07,p.plot08,p.plot09,p.plot10,cols=4)
	multiplot(p.plot11,p.plot12,p.plot13,cols=1)
	...
	myEUI.agg[,"SS.PRO.NUM"] <- as.numeric(sub("PRO","",sub("(.*)-(.*)-(.*)","\\2",row.names(myEUI.agg),perl=TRUE)))
	...
	# ******************************************************************************************************************************************************
	# June 29, 2015: re-simulated using 1500 random sample for each CZ with some changes in the templates to better handling insulation quality
	# add the average simulated EUI at each CZ in the state
	      myTmp.CZmeanEUI  <- data.frame(tapply(myEUI.agg[,"EUI.tot.PRO"],myEUI.agg[,"SS.CZ"],mean,na.rm=TRUE))
	names(myTmp.CZmeanEUI) <- "CZmeanEUI"
	myEUI.agg[,"EUI.tot.CZ.mean"] <-  myTmp.CZmeanEUI[myEUI.agg[,"SS.CZ"],"CZmeanEUI"]			# put the CZ mean EUI into a new field
	cat(paste("35C: [",this.state,"]: added [CZmeanEUI] to [myEUI.agg].\n",sep=""))
	cat(paste("35C: [",this.state,"]: added [CZmeanEUI] to [myEUI.agg].\n",sep=""),file=FL.LOG,append=TRUE)
	
	# add the CZ shares to [myEUI.agg]
	myEUI.agg[,"CZ.share"] <-  thisState.CZ.shares[myEUI.agg[,"SS.CZ"],this.state]				# put the CZ shares ad a new field
	cat(paste("35D: [",this.state,"]: added [CZ.share] to [myEUI.agg].\n",sep=""))
	cat(paste("35D: [",this.state,"]: added [CZ.share] to [myEUI.agg].\n",sep=""),file=FL.LOG,append=TRUE)

	# add the CZ shares weighted state mean EUI to [myEUI.agg]
	thisState.WgtEUI <- sum(myTmp.CZmeanEUI * thisState.CZ.shares)

	myEUI.agg[,"EUI.State.WtgMean"] <-  thisState.WgtEUI							# put the CZ wighted EUI into a new field
	cat(paste("35E: [",this.state,"]: added [EUI.State.WtgMean] to [myEUI.agg].\n",sep=""))
	cat(paste("35E: [",this.state,"]: added [EUI.State.WtgMean] to [myEUI.agg].\n",sep=""),file=FL.LOG,append=TRUE)
	# ******************************************************************************************************************************************************	
	...
	for (label.CZ in sort(unique(myEUI.agg[,"SS.CZ"])))
	{
		...
		CZ.mean.EUI <- myEUI.agg[myEUI.agg[,"SS.CZ"] == label.CZ,"EUI.tot.CZ.mean"][1]	# the corresponding CZ average EUI
		...
		command.string <- paste(paste("p.plot00",count.plot,sep="")," <- plot.obj.cdf",sep="") 
		...
		# assign a plot number of the plot
		command.string <- paste(paste("p.plot00",count.plot,sep="")," <- plot.obj.pdf",sep="") 
		eval(parse(text=command.string))		
	...
	##### 
	##### 	# assign a plot number of the plot
	##### 	command.string <- paste(paste("p.plot00000",count.plot,sep="")," <- plot.obj",sep="") 
	##### 	eval(parse(text=command.string))		
	##### 
	##### 	# plotting
	##### 	dev.set(3)
	...
	#
	# do the duct leakage adjustment
	#
	myEUI.duct.change <- readRDS(file = FL.duct.EUI.Change)
	cat(paste("44: [",this.state,"]: loaded the duct leakage EUI change file.\n",sep=""))
	cat(paste("44: [",this.state,"]: loaded the duct leakage EUI change file.\n",sep=""),file=FL.LOG,append=TRUE)

	
	# add auxilary fields in order to match the case from duct leakage simulation
	...
	#
	# June 6, 2015: the leakage ratio in the summary file was manipulated invisially and show difference in many decimal places, so cannot use the leakage ratio in the summary file to match the duct EUI change.
	#               need to go from parm csv files of both main and duct leakage simulation.
	#
	myParm.main <- read.table(FL.main.parm,header=TRUE,stringsAsFactors=FALSE,sep=",")
	myParm.duct <- read.table(FL.duct.parm,header=TRUE,,stringsAsFactors=FALSE,sep=",")
	cat(paste("46: [",this.state,"]: parm csv has been read in.\n",sep=""))
	cat(paste("46: [",this.state,"]: parm csv has been read in.\n",sep=""),file=FL.LOG,append=TRUE)

	
	myParm.duct[,"part1"] <- sub("(.*)_(.*)_(.*)_(.*)_(.*)","\\1",myParm.duct[,"ID"])
	myParm.duct[,"part2"] <- sub("(.*)_(.*)_(.*)_(.*)_(.*)","\\2",myParm.duct[,"ID"])
	myParm.duct[,"part3"] <- sub("(.*)_(.*)_(.*)_(.*)_(.*)","\\3",myParm.duct[,"ID"])
	myParm.duct[,"part4"] <- sub("(.*)_(.*)_(.*)_(.*)_(.*)","\\4",myParm.duct[,"ID"])
	myParm.duct[,"part5"] <- sub("(.*)_(.*)_(.*)_(.*)_(.*)","\\5",myParm.duct[,"ID"])
	myParm.duct[,"ID.leakage"] <- paste(myParm.duct[,"location"],sub("\\.duct","",myParm.duct[,"part5"]),sep="-")

	
	myParm.main[,"part1"] <- sub("(.*)_(.*)_(.*)_(.*)_(.*)","\\1",myParm.main[,"ID"])
	myParm.main[,"part2"] <- sub("(.*)_(.*)_(.*)_(.*)_(.*)","\\2",myParm.main[,"ID"])
	myParm.main[,"part3"] <- sub("(.*)_(.*)_(.*)_(.*)_(.*)","\\3",myParm.main[,"ID"])
	myParm.main[,"part4"] <- sub("(.*)_(.*)_(.*)_(.*)_(.*)","\\4",myParm.main[,"ID"])
	myParm.main[,"part5"] <- sub("(.*)_(.*)_(.*)_(.*)_(.*)","\\5",myParm.main[,"ID"])
	myParm.main[,"ID.leakage"] <- paste(myParm.main[,"location"],myParm.main[,"part5"],sep="-")
	cat(paste("47: [",this.state,"]: add [ID.leakage] to parm file for matching.\n",sep=""))
	cat(paste("47: [",this.state,"]: add [ID.leakage] to parm file for matching..\n",sep=""),file=FL.LOG,append=TRUE)

	
	# to check if this ID.leakage is unique to leakage_ratio.  If each only have one value, yes!
	for (idx in unique(myParm.duct[,"ID.leakage"]))
	{
		# cat(paste(idx,"\t",length(unique(myParm.duct[myParm.duct["ID.leakage"]==idx,"leakage_ratio"])),"\n",sep=""))
		if (length(unique(myParm.duct[myParm.duct["ID.leakage"]==idx,"leakage_ratio"]))!= 1){cat("wring!\n");die}
	}
	
	for (idx in unique(myParm.main[,"ID.leakage"]))
	{
		# cat(paste(idx,"\t",length(unique(myParm.main[myParm.main["ID.leakage"]==idx,"leakage_ratio"])),"\n",sep=""))
		if (length(unique(myParm.main[myParm.main["ID.leakage"]==idx,"leakage_ratio"]))!= 1){cat("wring!\n");die}
	}	
	cat(paste("48: [",this.state,"]: ensure the ID.leakage is unique to the leakage ratio values.\n",sep=""))
	cat(paste("48: [",this.state,"]: ensure the ID.leakage is unique to the leakage ratio values.\n",sep=""),file=FL.LOG,append=TRUE)


	myEUI.agg[,"ID.leakage"]         <- paste(sub("(.*)-(.*)-(.*)","\\1",myEUI.agg[,"ID"]),sub("(.*)-(.*)-(.*)","\\2",myEUI.agg[,"ID"]),sep="-")
	myEUI.duct.change[,"ID.leakage"] <- paste(myEUI.duct.change[,"City"],sub("\\.duct","",myEUI.duct.change[,"INDEX"]),sep="-")
	cat(paste("49: [",this.state,"]: add [ID.leakage] to [myEUI.agg] from main simulation and [myEUI.duct.change] from duct simulation for matching.\n",sep=""))
	cat(paste("49: [",this.state,"]: add [ID.leakage] to [myEUI.agg] from main simulation and [myEUI.duct.change] from duct simulation for matching.\n",sep=""),file=FL.LOG,append=TRUE)

	#
	# based on "ID.leakage" field, add leakage.ratio in the parm to [myEUI.agg] or [myEUI.duct.change]
	#
	myEUI.agg[,"parm.leakage.ratio"] <- NA
	myEUI.duct.change[,"parm.leakage.ratio"] <- NA
	
	myEUI.agg[,"parm.leakage.ratio"]         <- myParm.main[match(myEUI.agg[,"ID.leakage"],        myParm.main[,"ID.leakage"]),"leakage_ratio"]
	myEUI.duct.change[,"parm.leakage.ratio"] <- myParm.duct[match(myEUI.duct.change[,"ID.leakage"],myParm.duct[,"ID.leakage"]),"leakage_ratio"]
	cat(paste("50: [",this.state,"]: add leakage ratio from parm file to [myEUI.agg] or [myEUI.duct.change].\n",sep=""))
	cat(paste("50: [",this.state,"]: add leakage ratio from parm file to [myEUI.agg] or [myEUI.duct.change].\n",sep=""),file=FL.LOG,append=TRUE)
	
	# the combination of "CZ" and "parm.leakage.ratio" unique match the two sets of [myEUI.agg] and [myEUI.duct.change]
	myEUI.agg.ductAdj <- merge(myEUI.agg,myEUI.duct.change[,c("SS.City","SS.CZ","parm.leakage.ratio","SS.STATE","EUI.elec.PntChange","EUI.gas.PntChange","EUI.tot.PntChange","SS.PRO.IDX")],by =c("SS.City","SS.CZ","parm.leakage.ratio"),all.x=TRUE,all.y=FALSE)
	cat(paste("51: [",this.state,"]: [myEUI.agg.ductAdj]: containsing EUI change from duct leakage in [myEUI.duct.change] merged with the main EUI in [myEUI.agg].\n",sep=""))
	cat(paste("51: [",this.state,"]: [myEUI.agg.ductAdj]: containsing EUI change from duct leakage in [myEUI.duct.change] merged with the main EUI in [myEUI.agg].\n",sep=""),file=FL.LOG,append=TRUE)
	
	#
	# calculate duct leakage adjusted EUI
	#
	myEUI.agg.ductAdj[,"EUI.elec.PRO.ductAdj"] <- myEUI.agg.ductAdj[,"EUI.elec.PRO"] * (1 - myEUI.agg.ductAdj[,"EUI.elec.PntChange"]/100)
	myEUI.agg.ductAdj[,"EUI.gas.PRO.ductAdj"]  <- myEUI.agg.ductAdj[,"EUI.gas.PRO"]  * (1 - myEUI.agg.ductAdj[,"EUI.gas.PntChange"]/100)
	myEUI.agg.ductAdj[,"EUI.tot.PRO.ductAdj"]  <- myEUI.agg.ductAdj[,"EUI.tot.PRO"]  * (1 - myEUI.agg.ductAdj[,"EUI.tot.PntChange"]/100)
	cat(paste("52: [",this.state,"]: duct leakage adjusted EUI has been calculated.\n",sep=""))
	cat(paste("52: [",this.state,"]: duct leakage adjusted EUI has been calculated.\n",sep=""),file=FL.LOG,append=TRUE)
	...
	# June 29, 2015: re-simulated using 1500 random sample for each CZ with some changes in the templates to better handling insulation quality
	# add the average simulated EUI at each CZ in the state
	      myTmp.CZmeanEUI.ductAdj  <- data.frame(tapply(myEUI.agg.ductAdj[,"EUI.tot.PRO.ductAdj"],myEUI.agg.ductAdj[,"SS.CZ"],mean,na.rm=TRUE))
	names(myTmp.CZmeanEUI.ductAdj) <- "CZmeanEUI.ductAdj"
	myEUI.agg.ductAdj[,"EUI.tot.CZ.mean.ductAdj"] <-  myTmp.CZmeanEUI.ductAdj[myEUI.agg.ductAdj[,"SS.CZ"],"CZmeanEUI.ductAdj"]			# put the duct adjusted CZ mean EUI into a new field
	cat(paste("53A: [",this.state,"]: added [CZmeanEUI.ductAdj] to [myEUI.agg.ductAdj].\n",sep=""))
	cat(paste("53A: [",this.state,"]: added [CZmeanEUI.ductAdj] to [myEUI.agg.ductAdj].\n",sep=""),file=FL.LOG,append=TRUE)
	
	# add the CZ shares weighted state mean EUI to [myEUI.agg]
	thisState.WgtEUI.ductAdj <- sum(myTmp.CZmeanEUI.ductAdj * thisState.CZ.shares)

	myEUI.agg.ductAdj[,"EUI.State.WtgMean.ductAdj"] <-  thisState.WgtEUI.ductAdj									# put duct adjusted the CZ wighted EUI into a new field
	cat(paste("53B: [",this.state,"]: added [EUI.State.WtgMean.ductAdj] to [myEUI.agg.ductAdj].\n",sep=""))
	cat(paste("53B: [",this.state,"]: added [EUI.State.WtgMean.ductAdj] to [myEUI.agg.ductAdj].\n",sep=""),file=FL.LOG,append=TRUE)
	# ******************************************************************************************************************************************************	
	...
	# creating a long format version of [myEUI.agg.ductAdj] for plotting purpose: re-arrange the 6 variables of [EUI.elec.PRO],[EUI.gas.PRO],[EUI.tot.PRO],[EUI.elec.DIFF],[EUI.gas.DIFF],[EUI.tot.DIFF] 
	measure.vars.ductAdj   <- c(grep("EUI.gas.PRO|EUI.elec.PRO|EUI.tot.PRO",names(myEUI.agg.ductAdj),value=TRUE))
	id.vars.ductAdj        <- names(myEUI.agg.ductAdj)[-c(match(measure.vars.ductAdj,names(myEUI.agg.ductAdj)))]
	# the non-match subset of the fields: including the 6 PRO fields (elec, gas, tot EUI and DIFF)	
	myEUI.agg.ductAdj.long <- melt(myEUI.agg.ductAdj,id.vars = id.vars.ductAdj,measure.vars = measure.vars.ductAdj,variable.name = "Variable",value.name = "value")
	cat(paste("54: [",this.state,"]: have a ",dim(myEUI.agg.ductAdj)[1]," by ",dim(myEUI.agg.ductAdj)[2]," [myEUI.agg.ductAdj] and a ",dim(myEUI.agg.ductAdj.long)[1]," by ",dim(myEUI.agg.ductAdj.long)[2]," [myEUI.agg.ductAdj.long] ready.\n",sep=""))
	cat(paste("54: [",this.state,"]: have a ",dim(myEUI.agg.ductAdj)[1]," by ",dim(myEUI.agg.ductAdj)[2]," [myEUI.agg.ductAdj] and a ",dim(myEUI.agg.ductAdj.long)[1]," by ",dim(myEUI.agg.ductAdj.long)[2]," [myEUI.agg.ductAdj.long] ready.\n",sep=""),file=FL.LOG,append=TRUE)

	#
	# add mean, median, CZ weighted TRB EUI to [myEUI.agg.ductAdj]
	#
	...
	# [myEUI.agg] has 1500 models
	# ----------------------------------------------------------------------------------
	cat(paste("duct leakage adjusted EUI & Delta EUI,",sep=""),file=FL.EUI.ductAdj.csv,append=TRUE)
	write.table(myEUI.agg.ductAdj,file=FL.EUI.ductAdj.csv,sep=",",row.names=TRUE,col.names=TRUE,append=TRUE)
	
	saveRDS(myEUI.agg.ductAdj,file=FL.EUI.ductAdj.rds)
	cat(paste("55: [",this.state,"]: output ",dim(myEUI.agg.ductAdj)[1]," by ",dim(myEUI.agg.ductAdj)[2]," [myEUI.agg].\n",sep=""))
	cat(paste("55: [",this.state,"]: output ",dim(myEUI.agg.ductAdj)[1]," by ",dim(myEUI.agg.ductAdj)[2]," [myEUI.agg].\n",sep=""),file=FL.LOG,append=TRUE)
	...
	# ----------------------------------------------------------------------------------
	count.plot <- 0
	for (label.CZ in sort(unique(myEUI.agg.ductAdj[,"SS.CZ"])))
	{
		if (label.CZ == "2A")
		{
			this.color <- "red"
		...
		if(exists("mySubset.4plot")  && is.data.frame(get("mySubset.4plot"))) {rm(mySubset.4plot)}
		      ...
		      mySubset.4plot  <- subset(myEUI.agg.ductAdj,subset=(SS.CZ == label.CZ),select = EUI.tot.PRO.ductAdj,drop=FALSE)
		...
		baseline.EUI <- myEUI.agg.ductAdj[myEUI.agg.ductAdj[,"SS.CZ"] == label.CZ,"EUI.tot.TRB"][1]		# the corresponding baseline EUI
		baseline.IDX <- seq(1:dim(mySubset.4plot)[1])[mySubset.4plot[,"EUI"] > baseline.EUI][1]

		CZ.mean.EUI <- myEUI.agg.ductAdj[myEUI.agg.ductAdj[,"SS.CZ"] == label.CZ,"EUI.tot.CZ.mean.ductAdj"][1]	# the corresponding CZ average EUI
		CZ.mean.IDX <- seq(1:dim(mySubset.4plot)[1])[mySubset.4plot[,"EUI"] > CZ.mean.EUI][1]


		
		# write the ranked models out
		cat(paste("(",label.CZ,") (",this.state,")",sep=""),file=FL.EUI.ductAdj.rank.csv,append=TRUE)
		write.table(mySubset.4plot,file=FL.EUI.ductAdj.rank.csv,sep=",",row.names=TRUE,col.names=TRUE,append=TRUE)
		cat(paste("\n\n",sep=""),file=FL.EUI.ductAdj.rank.csv,append=TRUE)
		...
		FL.JPG <- paste(Path.main.Out.State,paste(this.state,"_CZ_figure",count.plot,".jpg",sep=""),sep="/")
	...
	cat(paste("139: [",this.state,"]: output ",dim(myEUI.agg.ductAdj)[1]," by ",dim(myEUI.agg.ductAdj)[2]," [myEUI.agg.ductAdj].\n",sep=""))
	cat(paste("139: [",this.state,"]: output ",dim(myEUI.agg.ductAdj)[1]," by ",dim(myEUI.agg.ductAdj)[2]," [myEUI.agg.ductAdj].\n",sep=""),file=FL.LOG,append=TRUE)
	
	##### dev.set(4)
	##### plot(p.plot0001,split=c(1,1,2,2))
	##### plot(p.plot0002,split=c(1,2,2,2),newpage=FALSE)
	##### plot(p.plot0003,split=c(2,1,2,2),newpage=FALSE)
	##### plot(p.plot0004,split=c(2,2,2,2),newpage=FALSE)	
	...
	# in [myEUI.agg.ductAdj], there is a field "EUI.tot.TRB" which has only 2 or 3 unique numbers, one for each CZ, retrieve the frst row to reach
	thisState.baselineEUI  <- myEUI.agg.ductAdj[diff(c(0,myEUI.agg.ductAdj[,"EUI.tot.TRB"])) != 0,c("SS.CZ","EUI.tot.TRB"),drop=FALSE]
	thisState.baselineEUI[,"CZ.share"] <- thisState.CZ.shares[thisState.baselineEUI[,"SS.CZ"],this.state,drop=FALSE]
	thisState.weightedEUI <- weighted.mean(thisState.baselineEUI[,"EUI.tot.TRB"], w = thisState.baselineEUI[,"CZ.share"])
	cat(paste("139A: [",this.state,"]: baseline EUI in CZs [thisState.baselineEUI] and weighted baselineEUI of the state in [thisState.weightedEUI].\n",sep=""))
	cat(paste("139A: [",this.state,"]: baseline EUI in CZs [thisState.baselineEUI] and weighted baselineEUI of the state in [thisState.weightedEUI].\n",sep=""),file=FL.LOG,append=TRUE)

	##### 
	##### 
	##### # plot the ranked EUI of the state without distinguishing the CZs
	#####       mySubset.4plot  <- myEUI.agg.ductAdj[,c("SS.CZ","EUI.tot.PRO.ductAdj"),drop=FALSE]
	#####       mySubset.4plot  <- mySubset.4plot[order(mySubset.4plot[,"EUI.tot.PRO.ductAdj"]),,drop=FALSE]
	#####       mySubset.4plot[,"Index"] <- seq(from=1,to=dim(mySubset.4plot)[1])
	##### names(mySubset.4plot) <- sub("EUI.tot.PRO.ductAdj","EUI",names(mySubset.4plot))	# note field name changed from "EUI.tot.PRO" to "EUI"
	##### 
	##### # -------------------------------------------------------------------------------------------------
	##### # add the CZ fraction weighted TRB EUI for the state
	##### # -------------------------------------------------------------------------------------------------
	...
	##### 	mySubset.4plot[,paste(this.CZ,".TRBEUI",sep="")] <- thisState.baselineEUI[thisState.baselineEUI[,"SS.CZ"] == this.CZ,"EUI.tot.TRB"]
	##### }
	##### cat(paste("139B: [",this.state,"]: added baseline EUI to [mySubset.4plot].\n",sep=""))
	##### cat(paste("139B: [",this.state,"]: added baseline EUI to [mySubset.4plot].\n",sep=""),file=FL.LOG,append=TRUE)
	##### 
	##### # -------------------------------------------------------------------------------------------------
	##### # calculate the mean and median of the 1500 EUIs
	...
	##### mySubset.4plot[,"meanEUI"]   <- thisState.meanEUI
	##### mySubset.4plot[,"medianEUI"] <- thisState.medianEUI	
	##### cat(paste("139C: [",this.state,"]: calculate the mean EUI of ",dim(myEUI.agg.ductAdj)[1]," randomly sampled houses.\n",sep=""))
	##### cat(paste("139C: [",this.state,"]: calculate the mean EUI of ",dim(myEUI.agg.ductAdj)[1]," randomly sampled houses.\n",sep=""),file=FL.LOG,append=TRUE)
	##### 
	##### 
	##### # write the ranked models out
	##### cat(paste("(",this.state,"),",sep=""),file=FL.EUI.ductAdj.rank.csv,append=TRUE)
	##### write.table(mySubset.4plot,file=FL.EUI.ductAdj.rank.csv,sep=",",row.names=TRUE,col.names=TRUE,append=TRUE)
	##### cat(paste("\n\n",sep=""),file=FL.EUI.ductAdj.rank.csv,append=TRUE)
	##### cat(paste("139D: [",this.state,"]: ranked EUI are written out.\n",sep=""))
	##### cat(paste("139D: [",this.state,"]: ranked EUI are written out.\n",sep=""),file=FL.LOG,append=TRUE)
	##### 
	##### 
	##### # find the index in the ranked EUI array for  mean, median and CZ weighted EUI
	...
	##### thisIndex.medianEUI   <- seq(1:dim(mySubset.4plot)[1])[mySubset.4plot[,"EUI"] > thisState.medianEUI][1]
	##### thisIndex.weightedEUI <- seq(1:dim(mySubset.4plot)[1])[mySubset.4plot[,"EUI"] > thisState.weightedEUI][1]
	##### cat(paste("139E: [",this.state,"]: the index in the ranked EUI df of the mean and median EUI and the CZ weighted TRB EUI.\n",sep=""))
	##### cat(paste("139E: [",this.state,"]: the index in the ranked EUI df of the mean and median EUI and the CZ weighted TRB EUI.\n",sep=""),file=FL.LOG,append=TRUE)
	##### 
	##### 
	##### plot.obj <- xyplot(EUI~Index,data=mySubset.4plot,
	...
	##### 		   xlab       = "Index of Models",
	##### 		   ylab       = "EUI (kBtu/sf)",
	##### 		   main       = paste("(",this.state,"): IECC Regulated end-use EUI (kBtu/sf) of ",dim(mySubset.4plot)[1]," pseudo houses sampled\nHorizontal Red line indicates the EUI of a 2009 IECC prescriptive code-compliant prototype",sep=""),
	##### 		   cex        = 0.5,
	##### 		   pch        = 19,
	##### 		   col        = "blue",
	##### 
	...
	##### 		   panel.abline(v=thisIndex.medianEUI,  col=c("cyan"), lty=2)			   
	##### 		   panel.text(0,thisState.weightedEUI,adj=c(-0.25,-0.25),labels=paste("CZ Weighted Code EUI = ",       round(thisState.weightedEUI,digits=2)," (kBtu/sf)",sep=""),col="red")		
	##### 		   panel.text(0,thisState.meanEUI,    adj=c(-0.75,-0.25),labels=paste("Mean of the simulated EUI = ",  round(thisState.meanEUI,    digits=2)," (kBtu/sf)",sep=""),col="green")
	##### 		   panel.text(0,thisState.medianEUI,  adj=c(0.25,-0.25),labels=paste("median of the simulated EUI = ",round(thisState.medianEUI,  digits=2)," (kBtu/sf)",sep=""),col="cyan")}
	##### 		   )
	##### 
	##### 
	##### 	# assign a plot number of the plot
	##### 	command.string <- paste(paste("p.plot0000000",count.plot,sep="")," <- plot.obj",sep="") 
	##### 	eval(parse(text=command.string))		
	##### 
	##### 	# plotting
	##### 	dev.set(4)
	##### 	plot(plot.obj)	
	##### cat(paste("139E: [",this.state,"]: ranked EUI are plotted in terms of the CZs in the state.\n",sep=""))
	##### cat(paste("139E: [",this.state,"]: ranked EUI are plotted in terms of the CZs in the state.\n",sep=""),file=FL.LOG,append=TRUE)
	...
	# 11. IECC Regulated end-use Gas EUI
	if(exists("mySubset.4plot")  && is.data.frame(get("mySubset.4plot"))) {rm(mySubset.4plot)}
	mySubset.4plot <- subset(myEUI.agg.ductAdj.long,subset=(Variable == "EUI.gas.PRO.ductAdj"))
	mySubset.4plot[,"y"] <- 0
	this.codeItem <- "IECC Regulated end-use Gas EUI"
	...
	# 12. IECC Regulated end-use Elec EUI
	if(exists("mySubset.4plot")  && is.data.frame(get("mySubset.4plot"))) {rm(mySubset.4plot)}
	mySubset.4plot <- subset(myEUI.agg.ductAdj.long,subset=(Variable == "EUI.elec.PRO.ductAdj"))
	mySubset.4plot[,"y"] <- 0
	this.codeItem <- "IECC Regulated end-use Elec EUI"
	...
	# 13. IECC Regulated end-use Total EUI
	if(exists("mySubset.4plot")  && is.data.frame(get("mySubset.4plot"))) {rm(mySubset.4plot)}
	mySubset.4plot <- subset(myEUI.agg.ductAdj.long,subset=(Variable == "EUI.tot.PRO.ductAdj"))
	mySubset.4plot[,"y"] <- 0
	this.codeItem <- "IECC Regulated end-use Total EUI"
	...
	multiplot(p.plot51,p.plot52,p.plot53,cols=3)
      # multiplot(p.plot54,p.plot55,cols=2)
      	plot(p.plot54)


	# output to a jpeg file
	FL.JPG <- paste(Path.main.Out.State,paste(this.state,"_EndUse_figure001.jpg",sep=""),sep="/")
	jpeg(file = FL.JPG,width=12,height=8,units="in",res=1200,bg = "transparent")
	dev.set(5)	
		multiplot(p.plot51,p.plot52,p.plot53,cols=3)
	dev.off(5)	
	...
	# output to a jpeg file
	FL.JPG <- paste(Path.main.Out.State,paste(this.state,"_EndUse_figure002.jpg",sep=""),sep="/")
	jpeg(file = FL.JPG,width=12,height=8,units="in",res=1200,bg = "transparent")
	dev.set(5)	
		plot(p.plot54)
	dev.off(5)			
	...
	#### July 7, 2015: abandon the establishment of a state EUI cdf based on randon drawing
	#### # ---------------------------------------------------------------------------------
	#### # sample 1500 EUI from the 3000 EUI proportional to the CZ shares
	#### # ---------------------------------------------------------------------------------
	#### no.2draw <- 1500
	####       no.2draw.CZ  <- round(array.share.CZ.state.revised[,this.state] * no.2draw,digits=0)
	#### names(no.2draw.CZ) <- row.names(array.share.CZ.state.revised)
	#### 
	#### 
	#### # DRAW 3 TIMES
	#### count.plot <- 0
	...
	#### {
	#### 	no.CZ <- 0
	#### 	for (this.CZ in names(no.2draw.CZ[no.2draw.CZ > 0]))
	#### 	{
	#### 		no.CZ <- no.CZ + 1
	#### 
	#### 		no.2draw.thisCZ <- no.2draw.CZ[this.CZ]
	#### 
	#### 
	#### 		mySub.EUI <- subset(myEUI.agg.ductAdj,subset = (SS.CZ == this.CZ))
	#### 		myIdx.EUI <- sample(seq(from=1,to=dim(mySub.EUI)[1]),no.2draw.thisCZ,replace=FALSE)
	#### 		myCZ.EUI  <- mySub.EUI[myIdx.EUI,]
	#### 
	#### 		if (no.CZ == 1)
	#### 		{
	#### 			myCombined.EUI <- myCZ.EUI
	#### 		}else{
	#### 			myCombined.EUI <- rbind(myCombined.EUI,myCZ.EUI)
	#### 		}
	#### 	}
	#### 
	#### 
	#### 	# plotting the pseudo cumulative samples
	#### 	if(exists("mySubset.4plot")  && is.data.frame(get("mySubset.4plot"))) {rm(mySubset.4plot)}
	#### 	
	#### 	      mySubset.4plot  <- myCombined.EUI
	#### 	      mySubset.4plot  <- mySubset.4plot[order(mySubset.4plot[,"EUI.tot.PRO.ductAdj"]),,drop=FALSE]
	#### 	      mySubset.4plot[,"Index"] <- seq(from=1,to=dim(mySubset.4plot)[1])
	#### 	names(mySubset.4plot) <- sub("EUI.tot.PRO.ductAdj","EUI",names(mySubset.4plot))
	#### 
	...
	#### 	# find the index in the ranked EUI array for  mean, median and CZ weighted EUI
	#### 	thisIndex.weightedEUI <- seq(1:dim(mySubset.4plot)[1])[mySubset.4plot[,"EUI"] > thisState.weightedEUI][1]
	#### 	thisIndex.StateEUI    <- seq(1:dim(mySubset.4plot)[1])[mySubset.4plot[,"EUI"] > thisState.WgtEUI.ductAdj][1]
	#### 
	#### 	
	#### 	
	#### 	cat(paste("139E: [",this.state,"]: the index in the ranked EUI df of the mean and median EUI and the CZ weighted TRB EUI.\n",sep=""))
	...
	#### 			   xlab       = "Index of Models",
	#### 			   ylab       = "EUI (kBtu/sf)",
	#### 			   main       = paste("(",this.state,"): IECC Regulated end-use EUI (kBtu/sf) of ",dim(mySubset.4plot)[1]," pseudo houses sampled\nRed line indicates the EUI of a 2009 IECC prescriptive code-compliant prototype",sep=""),
	#### 			 # main       = paste("(",this.state,"): IECC Regulated end-use EUI (kBtu/sf) of ",dim(mySubset.4plot)[1]," pseudo houses sampled\nRed line indicates the EUI of a 2009 IECC prescriptive code-compliant prototype\nCyan line indicates the weighted average EUI of the simulated models",sep=""),
	#### 			   cex        = 0.5,
	#### 			   pch        = 19,
	#### 			   col        = "blue",
	#### 
	...
	#### 			   panel.text(0,thisState.weightedEUI,adj=c(-0.25,-0.25),labels=paste("CZ Weighted Code EUI = ",       round(thisState.weightedEUI,digits=2)," (kBtu/sf)",sep=""),col="red")		
	#### 			   
	#### 			   # panel.abline(h=thisState.WgtEUI.ductAdj,col=c("cyan"))
	#### 			   # panel.abline(v=thisIndex.StateEUI,col=c("cyan"))
	#### 			   # panel.text(0,thisState.WgtEUI.ductAdj,adj=c(-0.25,-0.25),labels=paste("Weighted Average EUI of (",this.state,")=",round(thisState.WgtEUI.ductAdj,digits=2)," (kBtu/sf)",sep=""),col="cyan")	
	#### 			   
	#### 			   }
	#### 			   )
	#### 
	#### 
	#### 		# assign a plot number of the plot
	#### 		command.string <- paste(paste("p.plot0000",count.plot,sep="")," <- plot.obj.cdf",sep="") 
	#### 		eval(parse(text=command.string))		
	#### 
	#### 		# plotting
	#### 		dev.set(4)
	...
	#### 		
	#### 		# output to a jpeg file
	#### 		FL.JPG <- paste(Path.main.Out.State,paste(this.state,"_State_figure",count.plot,".jpg",sep=""),sep="/")
	#### 		jpeg(file = FL.JPG,width=12,height=8,units="in",res=1200,bg = "transparent")
	#### 		dev.set(5)	
	#### 			plot(plot.obj.cdf)
	#### 		dev.off(5)			
	...
	#### 			           between    = list(y=0.5),
	#### 			           xlab       = "EUI (kBtu/sf)",
	#### 			           main       = paste("(",this.state,"): IECC Regulated end-use EUI (kBtu/sf) of ",dim(mySubset.4plot)[1]," pseudo houses sampled at (",label.CZ,")\nRed line indicates the EUI for a 2009 IECC prescriptive code-compliant prototype",sep=""),
	#### 			           cex        = 0.5,
	#### 			           col        = "black",
	#### 
	#### 		   panel = function(x,...) {
	#### 			   panel.histogram(x,...)
	#### 			   panel.abline(v=thisState.weightedEUI,col=c("red"))
	#### 			   panel.text(thisState.weightedEUI,0,adj=c(0.25,-0.25),labels=round(thisState.weightedEUI,digits=2),col="red")}	
	#### 			   
	#### 			   )
	#### 
	#### 	# assign a plot number of the plot
	#### 	command.string <- paste(paste("p.plot0000",count.plot,sep="")," <- plot.obj.pdf",sep="") 
	#### 	eval(parse(text=command.string))		
	#### 	
	#### 	# plotting
	#### 	dev.set(4)
	...
	#### 
	#### dev.set(4)
	#### plot(p.plot00001,split=c(1,1,3,2))
	#### plot(p.plot00002,split=c(1,2,3,2),newpage=FALSE)
	#### plot(p.plot00003,split=c(2,1,3,2),newpage=FALSE)
	#### plot(p.plot00004,split=c(2,2,3,2),newpage=FALSE)	
	#### plot(p.plot00005,split=c(3,1,3,2),newpage=FALSE)
	#### plot(p.plot00006,split=c(3,2,3,2),newpage=FALSE)	
...
cat(paste("\n19. 06B_main_data_analysis_revised.R is finished successfully at ",End.time,"!\n",sep=" "))
cat(paste("\n19. 06B_main_data_analysis_revised.R is finished successfully at ",End.time,"!\n",sep=" "),file=FL.LOG,append=TRUE)

cat(paste("\n20. Processing time for [06B_main_data_analysis_revised.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "))
cat(paste("\n20. Processing time for [06B_main_data_analysis_revised.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "),file=FL.LOG,append=TRUE)
