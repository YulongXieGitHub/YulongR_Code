#
# 09_report_ggplot_Hartigan_Standardized.R 
#
# March 15, 2015:
# Data are standardized before clustering: i.e., subtracting mean and dividef the standard deviation to make the data mean-centred with unit variance
#
#
...
FL.LOG <- paste(Path.LOG,"09_report_ggplot_Hartigan_Standardized.log",sep="/")
...
for (this.year.alt in c(2011,2010,20100)[1])
{
	...
	Path.results <- paste(Path.Project,"09_report_ggplot_Hartigan_Standardized",this.year.string,sep="/")
	...
	for (this.variable in c("Sp","U"))
	{
		...
		#
		# March 15, 2015: Standardized the data before clustering
		#
		...
		for (this.no.cls in cluser.array)
		{
			...
			# use 5 classes Kmean
			model.kMeans  <- kmeans(myDATA.scaled,centers=no.cls,nstart=10,iter.max=50,algorithm = "Hartigan-Wong")
			...
			if (this.no.cls==4)
			{
				...
				}else if (this.variable == "U")
				{
					...
					p.add3 <- p0  + geom_boxplot(notch=TRUE) + labs(x="Category",y="Aqueous U (ugL-1)") + theme(axis.text.x = element_text(angle=45,size=10,vjust=0.5,color="black"),axis.title.x = element_text(color="black",size=20,vjust=2))  + scale_fill_manual(guide=FALSE,name = "Category",values=col.array2)	# ,title=paste("Year ", this.year,": Aqueous U: ",no.cls," clusters of Kmeans (before/after max WL)",sep="")
					p.add3 <- p.add3 + theme(axis.text.x = element_text(angle=90,size=15,vjust=0.5,color="black"),axis.title.x = element_text(color="black",size=20,vjust=0)) 
					p.add3 <- p.add3 + theme(axis.text.y = element_text(angle=0,size=15,vjust=0.5,color="black"),axis.title.y = element_text(color="black",size=20,vjust=1)) 
					# p.add3 <- p.add3 + theme(legend.position="right",legend.text = element_text(color="black",size=15,vjust=2),legend.title = element_text(color="black",size=20,vjust=2)) 
...
cat(paste("\n09_report_ggplot_Hartigan_Standardized.R is finished successfully at ",End.time,"!\n",sep=" "))
cat(paste("\n09_report_ggplot_Hartigan_Standardized.R is finished successfully at ",End.time,"!\n",sep=" "),file=FL.LOG,append=TRUE)

cat(paste("\nProcessing time for [09_report_ggplot_Hartigan_Standardized.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "))
cat(paste("\nProcessing time for [09_report_ggplot_Hartigan_Standardized.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "),file=FL.LOG,append=TRUE)
