#
# 07_report_ggplot_Hartigan_report.R 
#
# Feb 24-25, 2015
...
if(.Platform$OS.type == "unix") 
{
	...
	Path.Script  <- "c:/YuLong_Projects/FY2014_SFA/0_scripts"
	Path.Project <- "c:/YuLong_Projects/FY2014_SFA"
}
...
FL.LOG <- paste(Path.LOG,"07_report_ggplot_Hartigan_report.log",sep="/")
...
for (this.year.alt in c(2011,2010,20100)[1])
{
	...
	Path.results <- paste(Path.Project,"07_report_ggplot_Hartigan_report",this.year.string,sep="/")
	...
	for (this.variable in c("Sp","U"))
	{
		...
		for (this.no.cls in cluser.array)
		{
			...
			p0  <- ggplot(data=myData.4ggplot,aes(x=class.label,y=value,group=Cluster,fill=Cluster,colour=Cluster)) 
			p15 <- p0 + geom_boxplot(notch=TRUE) + labs(x="Clsuter",y=this.unit) # + scale_fill_manual(name = "Cluster",values=c("pink","cyan","green","magenta")[1:no.cls],labels=paste("Cluster",seq(1:no.cls),sep=""))
			...
			if (this.no.cls==4)
			{
				if (this.variable == "Sp")
				{
					...
					p.add2 <- p0  + geom_boxplot(notch=TRUE) + labs(x="Clusters",y=this.unit,title=paste("NOT UPDATED!!! Year ", this.year,": Sp: ",no.cls," clusters of Kmeans",sep="")) + theme(axis.text.x = element_text(angle=90,size=10,vjust=0.5,color="black"),axis.title.x = element_text(color="black",size=20,vjust=2))  + scale_fill_manual(name = "Cluster",values=c("pink","cyan","green","magenta")[1:no.cls],labels=paste("Cluster",seq(1:no.cls),sep=""))
					...
					p.add3 <- p0  + geom_boxplot(notch=TRUE) + labs(x="Categories",y=this.unit) + theme(axis.text.x = element_text(angle=45,size=10,vjust=0.5,color="black"),axis.title.x = element_text(color="black",size=20,vjust=2))  + scale_fill_manual(name = "Category",values=col.array2)   # title=paste("Year ", this.year,": Sp: ",no.cls," clusters of Kmeans (before/after max WL)",sep="")
					...
					p.add4 <- p0  + geom_boxplot(notch=TRUE) + labs(x="Categories",y=this.unit,title=paste("NOT UPDATED!!! Year ", this.year,": Sp: ",no.cls," clusters of Kmeans (before/after max WL)",sep="")) + theme(axis.text.x = element_text(angle=90,size=10,vjust=0.5,color="black"),axis.title.x = element_text(color="black",size=20,vjust=2)) + scale_fill_manual(name = "Category",values=col.array2)
				...
				}else if (this.variable == "U")
				{
					...
					p.add2 <- p0  + geom_boxplot(notch=TRUE) + labs(x="Clusters",y="Aqueous U (ugL-1)",title=paste("NOT UPDATED!!! Year ", this.year,": Aqueous U: ",no.cls," clusters of Kmeans",sep="")) + theme(axis.text.x = element_text(angle=90,size=10,vjust=0.5,color="black"),axis.title.x = element_text(color="black",size=20,vjust=2))  + scale_fill_manual(name = "Cluster",values=c("pink","cyan","green","magenta")[1:no.cls],labels=paste("Cluster",seq(1:no.cls),sep=""))

					# boxplot of all wells within cluster (split by the maximum WL)
					p0 <- ggplot(data=myAqueousU.4ggplot,aes(x=cluster.WL,y=value,fill=as.factor(cluster.WL)))
					p.add3 <- p0  + geom_boxplot(notch=TRUE) + labs(x="Categories",y="Aqueous U (ugL-1)") + theme(axis.text.x = element_text(angle=45,size=10,vjust=0.5,color="black"),axis.title.x = element_text(color="black",size=20,vjust=2))  + scale_fill_manual(name = "Category",values=col.array2)	# ,title=paste("Year ", this.year,": Aqueous U: ",no.cls," clusters of Kmeans (before/after max WL)",sep="")

					# boxplot of individual well in the clusters (split by the maximum WL)
					p0 <- ggplot(data=myAqueousU.4ggplot,aes(x=WellID.WL,y=value,fill=cluster.WL))
					p.add4 <- p0  + geom_boxplot(notch=TRUE) + labs(x="Categories",y="Aqueous U (ugL-1)",title=paste("NOT UPDATED!!! Year ", this.year,": Aqueous U: ",no.cls," clusters of Kmeans (before/after max WL)",sep="")) + theme(axis.text.x = element_text(angle=90,size=10,vjust=0.5,color="black"),axis.title.x = element_text(color="black",size=20,vjust=2)) + scale_fill_manual(name = "Category",values=col.array2)
					cat(paste("2511F. (",this.variable,") [",no.cls," classes]: We are here.\n",sep=""))
					cat(paste("2511F. (",this.variable,") [",no.cls," classes]: We are here.\n",sep=""),file=FL.LOG,append=TRUE)				
				}
...
cat(paste("\n07_report_ggplot_Hartigan_report.R is finished successfully at ",End.time,"!\n",sep=" "))
cat(paste("\n07_report_ggplot_Hartigan_report.R is finished successfully at ",End.time,"!\n",sep=" "),file=FL.LOG,append=TRUE)

cat(paste("\nProcessing time for [07_report_ggplot_Hartigan_report.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "))
cat(paste("\nProcessing time for [07_report_ggplot_Hartigan_report.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "),file=FL.LOG,append=TRUE)
