#
# 06_report_ggplot_Hartigan_report.R 
#
...
FL.LOG <- paste(Path.LOG,"06_report_ggplot_Hartigan_report.log",sep="/")
...
for (this.year.alt in c(2011,2010,20100)[1])
{
	...
	Path.results <- paste(Path.Project,"06_report_ggplot_Hartigan_report",this.year.string,sep="/")
	...
	for (this.variable in c("Sp","U"))
	{
		...
		for (this.no.cls in cluser.array)
		{
			...
			p16 <- p0 + geom_point(size=2,shape=4) + geom_text(size=3,vjust=-0.5) + scale_color_manual(name="Cluster",values=col.array) + coord_equal() + theme(legend.position="none") + labs(x="Easting",y="Northing",title=paste("Location of the wells in the clusters of ",this.variable," at Year ", this.year,": ",no.cls," clusters of Kmeans",sep=""))
			...
			p0 <- ggplot(data=myExtracU.4ggplot,aes(x=class.label,y=extractable.U,fill=as.factor(class.label)))
			p.add1 <- p0  + geom_boxplot(notch=TRUE) + scale_fill_manual(name = "Cluster",values=c("pink","cyan","green","magenta")[1:no.cls],labels=paste("Cluster",seq(1:no.cls),sep="")) + labs(x="Cluster",y="Extractable U",title=paste("Year ", this.year,": Extractable U: ",no.cls," clusters of Kmeans",sep="")) + theme(axis.text.x = element_text(angle=90,size=10,vjust=0.5,color="black"),axis.title.x = element_text(color="black",size=20,vjust=2))  
			...
			myAqueousU.4ggplot[,"cluster.WL"]  <- paste(paste("Cluster",myAqueousU.4ggplot[,"class.label"],sep=""),myAqueousU.4ggplot[,"from.WL.peak"],sep="-")
			...
			p0 <- ggplot(data=myAqueousU.4ggplot,aes(x=class.label,y=value,fill=as.factor(class.label)))
			p.add2 <- p0  + geom_boxplot(notch=TRUE) + labs(x="Clusters",y="Aqueous U",title=paste("Year ", this.year,": Aqueous U: ",no.cls," clusters of Kmeans",sep="")) + theme(axis.text.x = element_text(angle=90,size=10,vjust=0.5,color="black"),axis.title.x = element_text(color="black",size=20,vjust=2))  + scale_fill_manual(name = "Cluster",values=c("pink","cyan","green","magenta")[1:no.cls],labels=paste("Cluster",seq(1:no.cls),sep=""))
			...
			p.add3 <- p0  + geom_boxplot(notch=TRUE) + labs(x="Categories",y="Aqueous U",title=paste("Year ", this.year,": Aqueous U: ",no.cls," clusters of Kmeans (before/after max WL)",sep="")) + theme(axis.text.x = element_text(angle=45,size=10,vjust=0.5,color="black"),axis.title.x = element_text(color="black",size=20,vjust=2))  + scale_fill_manual(name = "Category",values=col.array2)
			
			# boxplot of individual well in the clusters (split by the maximum WL)
			p0 <- ggplot(data=myAqueousU.4ggplot,aes(x=WellID.WL,y=value,fill=as.factor(cluster.WL)))
			p.add4 <- p0  + geom_boxplot(notch=TRUE) + labs(x="Categories",y="Aqueous U",title=paste("Year ", this.year,": Aqueous U: ",no.cls," clusters of Kmeans (before/after max WL)",sep="")) + theme(axis.text.x = element_text(angle=90,size=10,vjust=0.5,color="black"),axis.title.x = element_text(color="black",size=20,vjust=2)) + scale_fill_manual(name = "Category",values=col.array2)
...
cat(paste("\n06_report_ggplot_Hartigan_report.R is finished successfully at ",End.time,"!\n",sep=" "))
cat(paste("\n06_report_ggplot_Hartigan_report.R is finished successfully at ",End.time,"!\n",sep=" "),file=FL.LOG,append=TRUE)

cat(paste("\nProcessing time for [06_report_ggplot_Hartigan_report.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "))
cat(paste("\nProcessing time for [06_report_ggplot_Hartigan_report.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "),file=FL.LOG,append=TRUE)
