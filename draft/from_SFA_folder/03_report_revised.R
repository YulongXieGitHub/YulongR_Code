...
FL.LOG <- paste(Path.LOG,"03_report_revised.log",sep="/")
...
Path.results <- paste(Path.Project,"3_report_revised",sep="/")
...
for (this.year.alt in c(2011,2010,20100))
{
	...
	Path.results <- paste(Path.Project,"3_report_revised",this.year.string,sep="/")
	if (!file.exists(Path.Data)){print(paste("NOT existing:",Path.Data));cat(paste(Path.Data," does not exist for [",this.subfolder,"].  Check why!\n",sep=""));die}
	if (!file.exists(Path.results)){print(paste("NOT existing:",Path.results));dir.create(Path.results,showWarnings=TRUE,recursive=TRUE)}
...
cat(paste("\n03_report_revised.R is finished successfully at ",End.time,"!\n",sep=" "))
cat(paste("\n03_report_revised.R is finished successfully at ",End.time,"!\n",sep=" "),file=FL.LOG,append=TRUE)

cat(paste("\nProcessing time for [03_report_revised.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "))
cat(paste("\nProcessing time for [03_report_revised.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "),file=FL.LOG,append=TRUE)
