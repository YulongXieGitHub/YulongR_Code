...
FL.LOG <- paste(Path.LOG,"03_report_Hartigan.log",sep="/")
...
Path.results <- paste(Path.Project,"3_report_Hartigan",sep="/")
...
for (this.year.alt in c(2011,2010,20100))
{
	...
	Path.results <- paste(Path.Project,"3_report_Hartigan",this.year.string,sep="/")
	if (!file.exists(Path.Data)){print(paste("NOT existing:",Path.Data));cat(paste(Path.Data," does not exist for [",this.subfolder,"].  Check why!\n",sep=""));die}
	if (!file.exists(Path.results)){print(paste("NOT existing:",Path.results));dir.create(Path.results,showWarnings=TRUE,recursive=TRUE)}
		...
		for (this.no.cls in cluser.array)
		{
			...
			for (this.option in c("1cls1page","allCls1page"))
			{
				...
				for (this.class in seq(1:no.cls))
				{
					...
					idx.rows <- myDATA.MERGED[,"date.chron"] >= this.chron.cut.begin & myDATA.MERGED[,"date.chron"] <= this.chron.cut.end
					plot(myDATA.MERGED[idx.rows,idx.this.cls[1]] ~ myDATA.MERGED[idx.rows,"date.chron"],type="l",lty=1,cex=1,col="red",xlab="date",ylab="U",ylim = Value.range.all,main=paste(this.variable,": Year ",this.year,": this cls has ",length(idx.this.cls)," wells:",gsub("\\.loess","",gsub("\\.loess","",paste(idx.this.cls,collapse=","))),sep=""))
					if (length(idx.this.cls) > 1)
					{
						for (count in seq(from=2,to=length(idx.this.cls)))
						{
							lines(myDATA.MERGED[idx.rows,idx.this.cls[count]] ~ myDATA.MERGED[idx.rows,"date.chron"],pch=16,col=col.array[count])
						}
					}

					# add the water table: since yDATA.MERGED] is a different data frame, we need to constrian the range to be plotted as the same as in [myDATA]
					par(new=TRUE)
					plot(myDATA.MERGED[idx.rows,"date.chron"],myDATA.MERGED[idx.rows,"WL"],type="l",lty=2,lwd=2.5,col="blue",col.lab="blue",axes=FALSE,bty="n",xlab="",ylab="")
...
cat(paste("\n03_report_Hartigan.R is finished successfully at ",End.time,"!\n",sep=" "))
cat(paste("\n03_report_Hartigan.R is finished successfully at ",End.time,"!\n",sep=" "),file=FL.LOG,append=TRUE)

cat(paste("\nProcessing time for [03_report_Hartigan.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "))
cat(paste("\nProcessing time for [03_report_Hartigan.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "),file=FL.LOG,append=TRUE)
