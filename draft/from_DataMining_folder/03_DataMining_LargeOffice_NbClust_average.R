#
# 03_DataMining_LargeOffice_NbClust_average.R 
# 
# March 4, 2015
#
...
Path.OUT <- paste(Path.Sim,"03_DataMining_LargeOffice_NbClust_average",sep="/")
...
FL.LOG <- paste(Path.OUT,"03_DataMining_LargeOffice_NbClust_average.log",sep="/")
 		  ...
 		  for (this.subset in c("artificial1","artificial2","artificial3","July","July-August","WeekDay","Sunday","Saturday","Holiday"))
 		{
	      ...
	      row.names(myData.dayMapping) <- myData.dayMapping[,"date.new"]
			...
			{ 
				...
				NbClust.model <- NbClust(myData.Work, diss=NULL,distance="euclidean",min.nc = 2,max.nc = 20,method="average",index="all")

				no.cls.best <- which.max(table(NbClust.model$Best.nc["Number_clusters",]))

				cls.Work <-  NbClust.model$Best.partition

				cat(paste("34. [",this.CZ,"]-[",this.EEM.name,"]-[",this.subset,"]: NbClust_average modeling.\n",sep=""))
				cat(paste("34. [",this.CZ,"]-[",this.EEM.name,"]-[",this.subset,"]: NbClust_average modeling.\n",sep=""),file=FL.LOG,append=TRUE)	

				cat(paste("35. [",this.CZ,"]-[",this.EEM.name,"]-[",this.subset,"]: NbClust_average predicting.\n",sep=""))
				cat(paste("35. [",this.CZ,"]-[",this.EEM.name,"]-[",this.subset,"]: NbClust_average predicting.\n",sep=""),file=FL.LOG,append=TRUE)	
...
cat(paste("\nProcessing time for [03_DataMining_LargeOffice_NbClust_average.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "))
cat(paste("\nProcessing time for [03_DataMining_LargeOffice_NbClust_average.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "),file=FL.LOG,append=TRUE)
