#
# 04_DataMining_LargeOffice_PAM.R 
# 
...
Path.OUT <- paste(Path.Sim,"04_DataMining_LargeOffice_PAM",sep="/")
...
FL.LOG <- paste(Path.OUT,"04_DataMining_LargeOffice_PAM.log",sep="/")
 		...
 		for (this.subset in c("artificial1","artificial2","artificial3","July","July-August","July-Weekday","July-August-Weekday","WeekDay","Sunday","Saturday","Holiday"))
 		{
	      ...
	      row.names(myData.dayMapping) <- myData.dayMapping[,"date.new"]
			...
			{ 
				...
				par(mfrow=c(1,2))				
				pamk.best <- pamk(myData.Work)		# Partitioning around medoids with estimation of number of clusters
...
cat(paste("\n04_DataMining_LargeOffice_PAM.R is finished successfully at ",End.time,"!\n",sep=" "))
cat(paste("\n04_DataMining_LargeOffice_PAM.R is finished successfully at ",End.time,"!\n",sep=" "),file=FL.LOG,append=TRUE)


cat(paste("\nProcessing time for [04_DataMining_LargeOffice_PAM.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "))
cat(paste("\nProcessing time for [04_DataMining_LargeOffice_PAM.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "),file=FL.LOG,append=TRUE)
