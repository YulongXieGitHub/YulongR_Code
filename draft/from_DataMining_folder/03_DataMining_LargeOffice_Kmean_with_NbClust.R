#
# 03_DataMining_LargeOffice_Kmean_with_NbClust.R 
# 
...
Path.OUT <- paste(Path.Sim,"03_DataMining_LargeOffice_Kmean_with_NbClust",sep="/")
...
FL.LOG <- paste(Path.OUT,"03_DataMining_LargeOffice_Kmean_with_NbClust.log",sep="/")
	...
	for (this.idx in seq(1,length(EEMs.name)))
	{
		...
		cat(paste("21. [",this.CZ,"]-[",this.EEM.name,"]: to avoid worngly taking existing data frame, delete them first..\n",sep=""),file=FL.LOG,append=TRUE)	
 		...
 		# for (this.subset in c("artificial1","artificial2","artificial3","July","July-August","WeekDay","All Data","Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Holiday"))
 		  for (this.subset in c("artificial1","artificial2","artificial3","July","July-August","WeekDay","Sunday","Saturday","Holiday"))
 		# for (this.subset in c("WeekDay","Sunday","Saturday","Holiday"))
 		{
	      ...
	      row.names(myData.dayMapping) <- myData.dayMapping[,"date.new"]
			...
			{ 
				...
				NbClust.model <- NbClust(myData.Work, diss=NULL,distance="euclidean",min.nc = 2,max.nc = 20,method="kmean",index="all")
...
cat(paste("\n03_DataMining_LargeOffice_Kmean_with_NbClust.R is finished successfully at ",End.time,"!\n",sep=" "))
cat(paste("\n03_DataMining_LargeOffice_Kmean_with_NbClust.R is finished successfully at ",End.time,"!\n",sep=" "),file=FL.LOG,append=TRUE)


cat(paste("\nProcessing time for [03_DataMining_LargeOffice_Kmean_with_NbClust.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "))
cat(paste("\nProcessing time for [03_DataMining_LargeOffice_Kmean_with_NbClust.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "),file=FL.LOG,append=TRUE)
