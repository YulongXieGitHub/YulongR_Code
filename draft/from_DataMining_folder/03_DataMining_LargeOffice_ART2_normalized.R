#
# 03_DataMining_LargeOffice_ART2_normalized.R 
# 
...
Path.OUT <- paste(Path.Sim,"03_DataMining_LargeOffice_ART2_normalized",sep="/")
...
FL.LOG <- paste(Path.OUT,"03_DataMining_LargeOffice_ART2_normalized.log",sep="/")
...
{	
	...
	for (this.idx in seq(1,length(EEMs.name)))
	{
		...
		cat(paste("21. [",this.CZ,"]-[",this.EEM.name,"]: to avoid worngly taking existing data frame, delete them first..\n",sep=""),file=FL.LOG,append=TRUE)	
 		...
 		for (this.subset in c("artificial1","artificial2","artificial3","July","July-August","WeekDay","All Data","Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Holiday"))
 		{
	      ...
	      row.names(myData.dayMapping) <- myData.dayMapping[,"date.new"]
			...
			# { 
				# 
				  myModel <- art2(myData.Work.norm, f2Units=12, initFunc   = "ART2_Weights", initFuncParams   = c(thisInit, 2),
										learnFunc  = "ART2",         learnFuncParams  = c(thisLearn, 10, 10, 0.1, 0),
										updateFunc = "ART2_Stable",  updateFuncParams = c(thisUpdate, 10, 10, 0.1, 0))


				cat(paste("34. [",this.CZ,"]-[",this.EEM.name,"]-[",this.subset,"]: ATR modeling.\n",sep=""))
				cat(paste("34. [",this.CZ,"]-[",this.EEM.name,"]-[",this.subset,"]: ATR modeling.\n",sep=""),file=FL.LOG,append=TRUE)	

				myPredict <- predict(myModel, myData.Work.norm)
				cat(paste("35. [",this.CZ,"]-[",this.EEM.name,"]-[",this.subset,"]: ATR predicting.\n",sep=""))
				cat(paste("35. [",this.CZ,"]-[",this.EEM.name,"]-[",this.subset,"]: ATR predicting.\n",sep=""),file=FL.LOG,append=TRUE)	

				myResults.Work <- cbind(myDate,myData.Work.norm,myPredict)
				cat(paste("36. [",this.CZ,"]-[",this.EEM.name,"]-[",this.subset,"]: combine clustering results with data.\n",sep=""))
				cat(paste("36. [",this.CZ,"]-[",this.EEM.name,"]-[",this.subset,"]: combine clustering results with data.\n",sep=""),file=FL.LOG,append=TRUE)	

				# assign cluster label
				myResults.Work[,"cls.Work"] <- encodeClassLabels(myModel$fitted.values)
		...
		# {		
			...
			cat(paste("65. [",this.CZ,"]-[",this.EEM.name,"]-[weekly]: ATR modeling.\n",sep=""))
			cat(paste("65. [",this.CZ,"]-[",this.EEM.name,"]-[weekly]: ATR modeling.\n",sep=""),file=FL.LOG,append=TRUE)	

			myPredict <- predict(myModel, myData.Work.norm)
			cat(paste("66. [",this.CZ,"]-[",this.EEM.name,"]-[weekly]: ATR predicting.\n",sep=""))
			cat(paste("66. [",this.CZ,"]-[",this.EEM.name,"]-[weekly]: ATR predicting.\n",sep=""),file=FL.LOG,append=TRUE)	

			myResults.Work <- cbind(myDate,myData.Work.norm,myPredict)
			cat(paste("67. [",this.CZ,"]-[",this.EEM.name,"]-[weekly]: combine clustering results with data.\n",sep=""))
			cat(paste("67. [",this.CZ,"]-[",this.EEM.name,"]-[weekly]: combine clustering results with data.\n",sep=""),file=FL.LOG,append=TRUE)	

			# assign cluster label
			myResults.Work[,"cls.Work"] <- encodeClassLabels(myModel$fitted.values)
			cat(paste("68. [",this.CZ,"]-[",this.EEM.name,"]-[weekly]: combine clustering results with data.\n",sep=""))
			cat(paste("68. [",this.CZ,"]-[",this.EEM.name,"]-[weekly]: combine clustering results with data.\n",sep=""),file=FL.LOG,append=TRUE)	
...
cat(paste("\n03_DataMining_LargeOffice_ART2_normalized.R is finished successfully at ",End.time,"!\n",sep=" "))
cat(paste("\n03_DataMining_LargeOffice_ART2_normalized.R is finished successfully at ",End.time,"!\n",sep=" "),file=FL.LOG,append=TRUE)


cat(paste("\nProcessing time for [03_DataMining_LargeOffice_ART2_normalized.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "))
cat(paste("\nProcessing time for [03_DataMining_LargeOffice_ART2_normalized.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "),file=FL.LOG,append=TRUE)
