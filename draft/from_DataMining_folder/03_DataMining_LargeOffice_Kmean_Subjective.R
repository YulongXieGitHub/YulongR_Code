#
# 03_DataMining_LargeOffice_Kmean_Subjective.R 
# 
...
Path.OUT <- paste(Path.Sim,"03_DataMining_LargeOffice_Kmean_Subjective",sep="/")
...
FL.LOG <- paste(Path.OUT,"03_DataMining_LargeOffice_Kmean_Subjective.log",sep="/")
...
{	
	...
	for (this.idx in seq(1,length(EEMs.name)))
	{
		...
		this.EEM.cls    = sub("\\.Rdata","_cluster.csv",this.EEM.Rdata)
		...
		FL.OUT.CLS <- paste(Path.CZ.OUT,this.EEM.cls,  sep="/")
		...
		if  (file.exists(FL.OUT.CLS)) {print(paste(FL.OUT.CLS," exist. Delete it!"));file.remove(FL.OUT.CLS)}	
		...
		cat(paste("21. [",this.CZ,"]-[",this.EEM.name,"]: to avoid worngly taking existing data frame, delete them first..\n",sep=""),file=FL.LOG,append=TRUE)	
 		...
 		for (this.subset in c("artificial1","artificial2","artificial3","July","July-August","WeekDay","All Data","Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Holiday"))
 		{
	      ...
	      row.names(myData.dayMapping) <- myData.dayMapping[,"date.new"]
			...
			# { 
				# 
				# Find the best numberr of clusters based on wWithin Sun of Squares
				#
				wss <- (nrow(myData.Work)-1) * sum(apply(myData.Work,2,var))
				for (idx.cls in seq(from=2,to=no.cls,by=1))
				{	
					wss[idx.cls] <- sum(kmeans(as.matrix(myData.Work),centers=idx.cls,nstart=10,iter.max=50,algorithm = "Hartigan-Wong")$withinss)
				}
				

				
				# the number of cluster which account for more than 99% of the data variation
				wss.cum.percnt <- 1 - wss/wss[1]
				no.cls.best    <- seq(1:no.cls)[wss.cum.percnt > 0.99][1]	# there is a number of cluster reaching 99% of WSS 
				if(is.na(no.cls.best))						# if no such a cluster was found
				{
					# find the sharpest drop of wss but that biggest drop should be larger than 90%
					tmp1 <- c(wss[1],wss[1:(no.cls-1)])
					tmp2 <- wss
					delta <- tmp1 - tmp2
					no.cls.best <- which.max(delta)				# if no cluster reach 99% of WSS, find the biggest drop but the biggest drop should be larger than 90%
					
					if (wss.cum.percnt[no.cls.best] < 0.90)
					{
						no.cls.best <- no.cls				# otherwise use the no.cls
					}
				}
				thisLearn.string <- paste(this.method,": ",no.cls.best," clusters",sep="")
				plot(1:no.cls,wss,type="b",xlab="Number of Clsuters",ylab = "Within Groups Sun of Squares",main=paste("[",thisLearn.string,"] Clustering on subset [",this.subset,"] with ",no.object," objects.\n",no.cls.best," cluster seems like the best",sep=""))
				
				# key input the best number of clusters you think
				print("enter the best number of cluster you think from the plot:\n")
				try(no.cls.best <- scan(what=integer(), nmax=1), silent=TRUE)
				if (is.nan(no.cls.best) | (no.cls.best<1) | (no.cls.best > no.cls))
				{
					print("Your input is invalid. Please re-enter the best number of cluster you think from the plot:\n",sep="")
					try(no.cls.best <- scan(what=integer(), nmax=1), silent=TRUE)
				}

				# 
				# Use the best number of cluster to do clustering
				#				
				model.kMeans   <- kmeans(as.matrix(myData.Work),centers=no.cls.best,nstart=10,iter.max=50,algorithm = "Hartigan-Wong")
				
				
				cls.Work       <- model.kMeans$cluster
				myResults.Work <- cbind(myDate,myData.Work,cls.Work)
				...
				cat(paste("clustering results on [",this.CZ,"]-[",this.EEM.name,"]-[",this.subset,"],",sep=""),file=FL.OUT.CLS,append=TRUE)
				write.table(class.days,file=FL.OUT.CLS,sep=",",row.names=TRUE,col.names=TRUE,appen=TRUE)
		...
		# {		
			...
			thisLearn.string <- paste(this.method,": ",no.cls.best," clusters",sep="")
			plot(1:no.cls,wss,type="b",xlab="Number of Clsuters",ylab = "Within Groups Sun of Squares",main=paste("[",thisLearn.string,"] Clustering on subset [weekly] with ",no.object," objects.\n",no.cls.best," cluster seems like the best",sep=""))
			...
			cat(paste("\n\nclustering results on [",this.CZ,"]-[",this.EEM.name,"]-[weekly],",sep=""),file=FL.OUT.CLS,append=TRUE)
			write.table(class.weeks,file=FL.OUT.CLS,sep=",",row.names=TRUE,col.names=TRUE,appen=TRUE)
...
cat(paste("\n03_DataMining_LargeOffice_Kmean_Subjective.R is finished successfully at ",End.time,"!\n",sep=" "))
cat(paste("\n03_DataMining_LargeOffice_Kmean_Subjective.R is finished successfully at ",End.time,"!\n",sep=" "),file=FL.LOG,append=TRUE)


cat(paste("\nProcessing time for [03_DataMining_LargeOffice_Kmean_Subjective.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "))
cat(paste("\nProcessing time for [03_DataMining_LargeOffice_Kmean_Subjective.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "),file=FL.LOG,append=TRUE)
