#
# Pepare_JPG_Figures_for_Presentation_Restart.R
...
Path.OUT <- paste(Path.Sim,"Pepare_JPG_Figures_for_Presentation_Restart",sep="/")
...
FL.LOG <- paste(Path.OUT,"Pepare_JPG_Figures_for_Presentation_Restart.log",sep="/")
...
for (this.random in c("random1","random2")[1])
{
	...
	for (this.CZ in CZ.arrays[c(7)])	#for (this.CZ in CZ.arrays)  c("SanFrancisco","Albuquerque")  c(7,9,4,8,3,2,5,6,10,11,12,13,14,15,16,1)
	{
		...
		#
		# July 10, 2015: save the weather file to a RDS and CSV file
		#
		FL.Weather.CSV <- paste(Path.CZ.OUT,paste("EPW_",this.CZ,".csv",sep=""),sep="/")
		FL.Weather.RDS <- paste(Path.CZ.OUT,paste("EPW_",this.CZ,".rds",sep=""),sep="/")
		if  (file.exists(FL.Weather.CSV))       {print(paste(FL.Weather.CSV,       "exist. Delete it!"));file.remove(FL.Weather.CSV)}
		if  (file.exists(FL.Weather.RDS))       {print(paste(FL.Weather.RDS,       "exist. Delete it!"));file.remove(FL.Weather.RDS)}
		saveRDS(myEPW.thisCZ,file=FL.Weather.RDS)
		cat(paste("EPW_",this.CZ,".csv,",sep=""),file=FL.Weather.CSV,append=TRUE)
		write.table(myEPW.thisCZ,file=FL.Weather.CSV,sep=",",row.names=TRUE,col.names=TRUE,append=TRUE)
		cat(paste("7c. [",this.CZ,"]: output the weather data.\n",sep=""))
		cat(paste("7c. [",this.CZ,"]: output the weather data.\n",sep=""),file=FL.LOG,append=TRUE)
		...
		# for (this.idx in seq(1,length(EEMs.name)))
		{
			...
			this.EEM.rds        <- sub("\\.Rdata","_fabriacted.rds",     this.EEM.Rdata)
			...
			FL.OUT.RDS        <- paste(Path.CZ.OUT,this.EEM.rds,       sep="/")
			...
			if  (file.exists(FL.OUT.RDS))       {print(paste(FL.OUT.CSV,       "exist. Delete it!"));file.remove(FL.OUT.RDS)}
			...
			#
			# add the weather variable to [myData.Fake]
			#
			myData.Fake[,"epw.T.drybulb"]       <- myEPW.thisCZ[,"epw.T.drybulb"]
			myData.Fake[,"epw.T.dewpoint"]      <- myEPW.thisCZ[,"epw.T.dewpoint"]
			myData.Fake[,"epw.rel.humidity"]    <- myEPW.thisCZ[,"epw.rel.humidity"]
			myData.Fake[,"epw.atm.pressure"]    <- myEPW.thisCZ[,"epw.atm.pressure"]
			myData.Fake[,"epw.hor.ir.sky"]      <- myEPW.thisCZ[,"epw.hor.ir.sky"]
			myData.Fake[,"epw.direct.norm.rad"] <- myEPW.thisCZ[,"epw.direct.norm.rad"]
			myData.Fake[,"epw.diffuse.hor.rad"] <- myEPW.thisCZ[,"epw.diffuse.hor.rad"]
			myData.Fake[,"epw.wind.direct"]     <- myEPW.thisCZ[,"epw.wind.direct"]
			myData.Fake[,"epw.wind.speed"]      <- myEPW.thisCZ[,"epw.wind.speed"]
			cat(paste("14D. [",this.CZ,"]-[",this.EEM.name,"]: Add all weather variable to [myData.Remain].\n\n",sep=""))
			cat(paste("14C. [",this.CZ,"]-[",this.EEM.name,"]: Add all weather variable to [myData.Remain].\n\n",sep=""),file=FL.LOG,append=TRUE)
			...
			#
			# July 13, 2015: prepare a data file for Luke Gosink
			#
			FL.4LukeGosink01.CSV <- paste(Path.CZ.OUT,"Data_Prepared_for_LukeGosink_01.csv",sep="/")
			if  (file.exists(FL.4LukeGosink01.CSV)){print(paste(FL.4LukeGosink01.CSV,"exist. Delete it!"));file.remove(FL.4LukeGosink01.CSV)}
			cat(paste("hourly format,",sep=""),file=FL.4LukeGosink01.CSV,append=TRUE)
			write.table(myData.Fake,file=FL.4LukeGosink01.CSV,sep=",",row.names=TRUE,col.names=TRUE,append=TRUE)

			
			# 
			# save as RDS format	
			#
			saveRDS(myData.Fake,FL.OUT.RDS)
		      ...
		      row.names(myData.dayMapping) <- myData.dayMapping[,"date"]
				...
				if (no.object >= no.variable)
				{
					...
					if (length(grep("artificial",this.subset,invert = TRUE)))	# for all non-artifical subsets
					{
					  ...
					  names(Tmp.RelHumid)  <- c("date","RelHumid")

						Tmp.AtmPres    <- aggregate(myEpw.Cls[,"epw.atm.pressure"],list(myEpw.Cls[,"date"]),mean,na.rm=TRUE)
					  names(Tmp.AtmPres)   <- c("date","AtmPres")

						Tmp.HorIrSky   <- aggregate(myEpw.Cls[,"epw.hor.ir.sky"],list(myEpw.Cls[,"date"]),mean,na.rm=TRUE)
					  names(Tmp.HorIrSky)  <- c("date","HorIrSky")


						Tmp.DirNormRad <- aggregate(myEpw.Cls[,"epw.direct.norm.rad"],list(myEpw.Cls[,"date"]),mean,na.rm=TRUE)
					  names(Tmp.DirNormRad)<- c("date","DirNormRad")

						Tmp.DiffHorRad <- aggregate(myEpw.Cls[,"epw.diffuse.hor.rad"],list(myEpw.Cls[,"date"]),mean,na.rm=TRUE)
					  names(Tmp.DiffHorRad)<- c("date","DiffHorRad")


						Tmp.WindDirect <- aggregate(myEpw.Cls[,"epw.wind.direct"],list(myEpw.Cls[,"date"]),mean,na.rm=TRUE)
					  names(Tmp.WindDirect)<- c("date","WindDirect")

						Tmp.WindSpeed  <- aggregate(myEpw.Cls[,"epw.wind.speed"],list(myEpw.Cls[,"date"]),mean,na.rm=TRUE)
					  names(Tmp.WindSpeed) <- c("date","WindSpeed")
						...
						Tmp.Weather    <- merge(Tmp.Weather, Tmp.AtmPres)
						Tmp.Weather    <- merge(Tmp.Weather, Tmp.HorIrSky)
						Tmp.Weather    <- merge(Tmp.Weather, Tmp.DirNormRad)
						Tmp.Weather    <- merge(Tmp.Weather, Tmp.DiffHorRad)
						Tmp.Weather    <- merge(Tmp.Weather, Tmp.WindDirect)
						Tmp.Weather    <- merge(Tmp.Weather, Tmp.WindSpeed)
						...
						#
						# July 13, 2015: de-normalize the energy consumption data
						#
						myResults.Work[,grep("hourGJ",names(myResults.Work),value=TRUE)] <- myResults.Work[,grep("hourGJ",names(myResults.Work),value=TRUE)] * (max.GJ - min.GJ) + min.GJ
						cat(paste("60CC. [",this.CZ,"]-[",this.EEM.name,"]-[",this.subset,"]: normalize the energy consumption data back to original units before output.\n",sep=""))
						cat(paste("60CC. [",this.CZ,"]-[",this.EEM.name,"]-[",this.subset,"]: normalize the energy consumption data back to original units before output.\n",sep=""),file=FL.LOG,append=TRUE)
						...
						#
						# July 8, 2015: output the daily files with all environmental variables
						#
						FL.Daily.CSV <- paste(Path.CZ.OUT,paste("Daily_",this.CZ,"_",this.EEM.name,"_",this.subset,".csv",sep=""),sep="/")
						FL.Daily.RDS <- paste(Path.CZ.OUT,paste("Daily_",this.CZ,"_",this.EEM.name,"_",this.subset,".rds",sep=""),sep="/")
						if  (file.exists(FL.Daily.CSV))       {print(paste(FL.Daily.CSV,       "exist. Delete it!"));file.remove(FL.Daily.CSV)}
						if  (file.exists(FL.Daily.RDS))       {print(paste(FL.Daily.RDS,       "exist. Delete it!"));file.remove(FL.Daily.RDS)}
						saveRDS(myResults.Work,file=FL.Daily.RDS)
						
						#
						# July 13, 2015: prepare a data file for Luke Gosink
						#
						FL.4LukeGosink02.CSV <- paste(Path.CZ.OUT,"Data_Prepared_for_LukeGosink_02.csv",sep="/")
						if  (file.exists(FL.4LukeGosink02.CSV)){print(paste(FL.4LukeGosink02.CSV,"exist. Delete it!"));file.remove(FL.4LukeGosink02.CSV)}
						cat(paste("daily format,",sep=""),file=FL.4LukeGosink02.CSV,append=TRUE)
						write.table(myResults.Work,file=FL.4LukeGosink02.CSV,sep=",",row.names=TRUE,col.names=TRUE,append=TRUE)

						#
						# Write to csv file
						#			
						cat(paste("Daily_",this.CZ,"_",this.EEM.name,"_",this.subset,".csv,",sep=""),file=FL.Daily.CSV,append=TRUE)
						write.table(myResults.Work,file=FL.Daily.CSV,sep=",",row.names=TRUE,col.names=TRUE,append=TRUE)
						cat(paste("60CCC. [",this.CZ,"]: output the weather data.\n",sep=""))
						cat(paste("60CCC. [",this.CZ,"]: output the weather data.\n",sep=""),file=FL.LOG,append=TRUE)
...
cat(paste("\nPepare_JPG_Figures_for_Presentation_Restart.R is finished successfully at ",End.time,"!\n",sep=" "))
cat(paste("\nPepare_JPG_Figures_for_Presentation_Restart.R is finished successfully at ",End.time,"!\n",sep=" "),file=FL.LOG,append=TRUE)


cat(paste("\nProcessing time for [Pepare_JPG_Figures_for_Presentation_Restart.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "))
cat(paste("\nProcessing time for [Pepare_JPG_Figures_for_Presentation_Restart.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "),file=FL.LOG,append=TRUE)
