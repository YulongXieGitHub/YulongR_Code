#
# 08_DataMining_LargeOffice_Kmean_with_NbClust_kmeans.R 
# April 3, 2015
...
Path.OUT <- paste(Path.Sim,"08_DataMining_LargeOffice_Kmean_with_NbClust_kmeans",sep="/")
...
FL.LOG <- paste(Path.OUT,"08_DataMining_LargeOffice_Kmean_with_NbClust_kmeans.log",sep="/")
...
{	
	...
	# for (this.idx in seq(1,length(EEMs.name)))
	{
		...
		myData.Fake[myData.Fake[,"week.idx"] %in% c(3,4,7,8,11,12,15,16,19,20,23,24,27,28,31,32,35,36,39,40,43,44,47,48,51,52),] <- myData.base[myData.base[,"week.idx"] %in% c(3,4,7,8,11,12,15,16,19,20,23,24,27,28,31,32,35,36,39,40,43,44,47,48,51,52),]
		...
		cat(paste("21. [",this.CZ,"]-[",this.EEM.name,"]: to avoid worngly taking existing data frame, delete them first..\n",sep=""),file=FL.LOG,append=TRUE)	
 		  ...
 		  for (this.subset in c("all","WeekDay","July","July-August"))
 		{
			...
			}else if (this.subset == "July")
			{
				...
				min.nc <- 2
				max.nc <- 20
				if (this.CZ == "SanFrancisco"){max.nc <- 7}
				if (this.CZ == "SanFrancisco" & this.idx == 6){max.nc <- 4}
	      ...
	      row.names(myData.dayMapping) <- myData.dayMapping[,"date"]
			...
			{ 
				...
				# if(this.CZ == "SanFrancisco" & this.subset == "WeekDay" & this.idx == 1){no.cls.best <- 4}
				  if(this.CZ == "SanFrancisco" & this.subset == "July"    & this.idx == 6){no.cls.best <- 4}
				if(this.CZ == "SanFrancisco" & this.subset == "all"     & this.idx == 6){no.cls.best <- 14}
...
cat(paste("\n08_DataMining_LargeOffice_Kmean_with_NbClust_kmeans.R is finished successfully at ",End.time,"!\n",sep=" "))
cat(paste("\n08_DataMining_LargeOffice_Kmean_with_NbClust_kmeans.R is finished successfully at ",End.time,"!\n",sep=" "),file=FL.LOG,append=TRUE)


cat(paste("\nProcessing time for [08_DataMining_LargeOffice_Kmean_with_NbClust_kmeans.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "))
cat(paste("\nProcessing time for [08_DataMining_LargeOffice_Kmean_with_NbClust_kmeans.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "),file=FL.LOG,append=TRUE)
