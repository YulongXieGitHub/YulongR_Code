#
# 04_DataMining_LargeOffice_Kmean_with_NbClust_average.R 
# 
...
Path.OUT <- paste(Path.Sim,"04_DataMining_LargeOffice_Kmean_with_NbClust_average",sep="/")
...
FL.LOG <- paste(Path.OUT,"04_DataMining_LargeOffice_Kmean_with_NbClust_average.log",sep="/")
...
{	
	...
	# for (this.idx in seq(1,length(EEMs.name)))
	{
		...
		pdf(file = FL.OUT_Dummy.PDF,paper="special", width=17, height=11,bg = "transparent")
 		  ...
 		  for (this.subset in c("artificial1","artificial2","artificial3","July","July-August","WeekDay","Sunday","Saturday","Holiday"))
 		{
	      ...
	      row.names(myData.dayMapping) <- myData.dayMapping[,"date.new"]
			...
			{ 
				...
				NbClust.model <- NbClust(myData.Work, diss=NULL,distance="euclidean",min.nc = 2,max.nc = 20,method=this.method.4.NbClust,index="all")
				...
				if (length(grep("artificial",this.subset)))
				{
					...
					arrays.clusters.sub <- arrays.clusters[,c("Hartigan","Duda","PseudoT2","Beale","TraceW")]
				      ...
				      # no.cls.best <- which.max(table(NbClust.model$Best.nc["Number_clusters",]))
					no.cls.best <- max(arrays.clusters.sub["Number_clusters",])					
...
cat(paste("\n04_DataMining_LargeOffice_Kmean_with_NbClust_average.R is finished successfully at ",End.time,"!\n",sep=" "))
cat(paste("\n04_DataMining_LargeOffice_Kmean_with_NbClust_average.R is finished successfully at ",End.time,"!\n",sep=" "),file=FL.LOG,append=TRUE)


cat(paste("\nProcessing time for [04_DataMining_LargeOffice_Kmean_with_NbClust_average.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "))
cat(paste("\nProcessing time for [04_DataMining_LargeOffice_Kmean_with_NbClust_average.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "),file=FL.LOG,append=TRUE)
