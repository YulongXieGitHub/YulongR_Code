#
# 11_DataMining_LargeOffice_PAM_with_NbClust_kmeans.R 
# April 3, 2015
...
Path.OUT <- paste(Path.Sim,"11_DataMining_LargeOffice_PAM_with_NbClust_kmeans",sep="/")
...
FL.LOG <- paste(Path.OUT,"11_DataMining_LargeOffice_PAM_with_NbClust_kmeans.log",sep="/")
...
for (this.CZ in CZ.arrays[c(7,4,8,11)])	#for (this.CZ in CZ.arrays)  c("SanFrancisco","Albuquerque")  c(7,9,4,8,3,2,5,6,10,11,12,13,14,15,16,1)
{	
	...
	# for (this.idx in seq(1,length(EEMs.name)))
	{
		...
		cat(paste("21. [",this.CZ,"]-[",this.EEM.name,"]: to avoid worngly taking existing data frame, delete them first..\n",sep=""),file=FL.LOG,append=TRUE)	
 		...
 		# for (this.subset in c("artificial1","artificial2","artificial3","July","July-August","July-Weekday","July-August-Weekday","WeekDay","Sunday","Saturday","Holiday"))
 		  count.subset <- 0
 		  for (this.subset in c("all","WeekDay"))
 		{
			idx.name          <- "all"
...
cat(paste("\n11_DataMining_LargeOffice_PAM_with_NbClust_kmeans.R is finished successfully at ",End.time,"!\n",sep=" "))
cat(paste("\n11_DataMining_LargeOffice_PAM_with_NbClust_kmeans.R is finished successfully at ",End.time,"!\n",sep=" "),file=FL.LOG,append=TRUE)


cat(paste("\nProcessing time for [11_DataMining_LargeOffice_PAM_with_NbClust_kmeans.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "))
cat(paste("\nProcessing time for [11_DataMining_LargeOffice_PAM_with_NbClust_kmeans.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "),file=FL.LOG,append=TRUE)
