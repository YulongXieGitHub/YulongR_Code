...
# Path.Out     <- "C:/YuLong_Projects/FY2012_MORiver/DataAnalysis/7h_prepare_TLC_data"
...
# 
# FL.LOG       <- paste(Path.log,"7h_prepare_TLC_data.log",sep="/")	
# FL.SUM.cat   <- paste(Path.Out,"7h_prepare_TLC_data_categoricalSum.csv",sep="/")
# FL.SUM.num   <- paste(Path.Out,"7h_prepare_TLC_data_numericSum.csv",sep="/")
# FL.Summary   <- paste(Path.Out,"7h_prepare_TLC_data_summary.csv",sep="/")
# FL.missing   <- paste(Path.Out,"7h_prepare_TLC_data_missing.csv",sep="/")
...
FL.RESL.OUT <- paste(Path.Out,"7h_prepare_TLC_data.csv",sep="/")
FL.LOG      <- paste(Path.log,"7h_prepare_TLC_data.log",sep="/")	
FL.PDF      <- paste(Path.Out,"7h_prepare_TLC_data.pdf",sep="/")	
FL.SUM.cat  <- paste(Path.Out,"7h_prepare_TLC_data_cat.sum",sep="/")
FL.SUM.num  <- paste(Path.Out,"7h_prepare_TLC_data_num.sum",sep="/")
...
# -------------------------------------------------------------------------------------------------
# Reassign "Tax_part_sz" field: if I keep three categories, it will not work together with "SY".  So the number of categories matters!!!!
# -------------------------------------------------------------------------------------------------
...
var.X.cat <- c("SY","part","rock","macro.type","MicroClass")
var.Y     <- c("Pal_cnt")
