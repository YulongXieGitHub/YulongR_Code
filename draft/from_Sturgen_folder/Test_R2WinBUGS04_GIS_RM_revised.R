#
# Test_R2WinBUGS04_GIS_RM_revised.R
#
...
Path.Out     <- "C:/YuLong_Projects/FY2012_MORiver/DataAnalysis/Test_R2WinBUGS04_GIS_RM_Revised"
...
FL.Data.OUT <- paste(Path.Out,"Test_R2WinBUGS04_GIS_RM_revised_data.csv",sep="/")
FL.RESL.OUT <- paste(Path.Out,"Test_R2WinBUGS04_GIS_RM_revised_results.csv",sep="/")
FL.MODEL    <- paste(Path.Out,"Test_R2WinBUGS04_GIS_RM_revised_model.Rdata",sep="/")

FL.PostDist.CSV <- paste(Path.Out,"Test_R2WinBUGS04_GIS_RM_revised_PostDist.csv",sep="/")
FL.PostDist.OBJ <- paste(Path.Out,"Test_R2WinBUGS04_GIS_RM_revised_PostDist.Rdata",sep="/")


FL.LOG      <- paste(Path.log,"Test_R2WinBUGS04_GIS_RM_revised.log",sep="/")	
FL.PDF      <- paste(Path.Out,"Test_R2WinBUGS04_GIS_RM_revised.pdf",sep="/")	
FL.SUM.cat  <- paste(Path.Out,"Test_R2WinBUGS04_GIS_RM_revised_cat.sum",sep="/")
FL.SUM.num  <- paste(Path.Out,"Test_R2WinBUGS04_GIS_RM_revised_num.sum",sep="/")
...
# -------------------------------------------------------------------------------------------------
# output the other format used by WinBUGS or BUGS: [Test_R2WinBUGS04_GIS_RM_revised_data.csv]
# -------------------------------------------------------------------------------------------------
...
modelstring = "
	model{
		...
		for (j in 1:1){a[j]  ~ dnorm(0,1.0E-4)}
		for (j in 1:1){b[j]  ~ dnorm(0,1.0E-4)}
...
dataList = list(
		   ...
		   GIS_RM         = as.vector(myData.reduced[1:600,"VF_wid"]),	   
...
cat(paste("\nTest_R2WinBUGS04_GIS_RM_revised.R is finished successfully at ",End.time,"!\n",sep=" "))
cat(paste("\nTest_R2WinBUGS04_GIS_RM_revised.R is finished successfully at ",End.time,"!\n",sep=" "),file=FL.LOG,append=TRUE)

cat(paste("\nProcessing time for [Test_R2WinBUGS04_GIS_RM_revised.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "))
cat(paste("\nProcessing time for [Test_R2WinBUGS04_GIS_RM_revised.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "),file=FL.LOG,append=TRUE)
