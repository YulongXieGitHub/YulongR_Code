#
# 0_Bayesian_GLM_Hierarchical_revised.R
#
...
Path.Out     <- "C:/YuLong_Projects/FY2012_MORiver/DataAnalysis/0_Bayesian_GLM_Hierarchical_revised"
...
FL.Data.OUT <- paste(Path.Out,"0_Bayesian_GLM_Hierarchical_data.csv",sep="/")
FL.RESL.OUT <- paste(Path.Out,"0_Bayesian_GLM_Hierarchical_results.csv",sep="/")
FL.MODEL    <- paste(Path.Out,"0_Bayesian_GLM_Hierarchical_model.Rdata",sep="/")

FL.PostDist.CSV <- paste(Path.Out,"0_Bayesian_GLM_Hierarchical_PostDist.csv",sep="/")
FL.PostDist.OBJ <- paste(Path.Out,"0_Bayesian_GLM_Hierarchical_PostDist.Rdata",sep="/")


FL.LOG      <- paste(Path.log,"0_Bayesian_GLM_Hierarchical.log",sep="/")	
FL.PDF      <- paste(Path.Out,"0_Bayesian_GLM_Hierarchical.pdf",sep="/")	
FL.SUM.cat  <- paste(Path.Out,"0_Bayesian_GLM_Hierarchical_cat.sum",sep="/")
FL.SUM.num  <- paste(Path.Out,"0_Bayesian_GLM_Hierarchical_num.sum",sep="/")
...
#
# fit a zero inflated Poisson to get parameters to be used for initialization
#
fm_zip <- zeroinfl(ofp ~ hosp + numchron +  gendermale + school + privinsyes | hosp + numchron +  gendermale + school + privinsyes ,data = dt2, dist = "poisson")
...
modelstring = "
	model{
		...
		for (j in 1:5){a[j]  ~ dnorm(muA,tauA)}
		for (j in 1:5){b[j]  ~ dnorm(muB,tauB)}
...
genInitList <- function(nPred=nPredictors) {
	...
	list(
		a0 = df.coef.zero[1,1], 
		b0 = df.coef.count[1,1],
		a  = df.coef.zero[-1,1],
		b  = df.coef.count[-1,1],
		muA  = mean(df.coef.zero[-1,1]),
		muB  = mean(df.coef.count[-1,1]),
		tauA = 1/sd(df.coef.zero[-1,1])^2,
		tauB = 1/sd(df.coef.count[-1,1])^2	
	)
}
...
#------------------------------------------------------------------------------
samplesSet( c("a0","b0","a","b","muA","tauA","muB","tauB") )
nPerChain = 50000
thinStep = 1
...
#------------------------------------------------------------------------------
fname <- "0_Bayesian_GLM_Hierarchical"
...
if ( checkConvergence ) {
	...
	muASum  = plotChains( "muA"  , saveplots=T , filenameroot=fname,path.pdf=Path.Out )
	dev.copy2pdf(file=paste(Path.Out,paste(fname,"_ZeroInflation_muA.pdf",sep=""),sep="/"))
	
	muBSum  = plotChains( "muB"  , saveplots=T , filenameroot=fname,path.pdf=Path.Out )
	dev.copy2pdf(file=paste(Path.Out,paste(fname,"_Poisson_muB.pdf",sep=""),sep="/"))
	
	tauASum  = plotChains( "tauA"  , saveplots=T , filenameroot=fname,path.pdf=Path.Out )
	dev.copy2pdf(file=paste(Path.Out,paste(fname,"_ZeroInflation_tauA.pdf",sep=""),sep="/"))
	
	tauBSum  = plotChains( "tauB"  , saveplots=T , filenameroot=fname,path.pdf=Path.Out )
	dev.copy2pdf(file=paste(Path.Out,paste(fname,"_Poisson_tauB.pdf",sep=""),sep="/"))	
}
...
muASample  <- matrix( samplesSample( "muA"))
muBSample  <- matrix( samplesSample( "muB" ))
tauASample <- matrix( samplesSample( "tauA"))
tauBSample <- matrix( samplesSample( "tauB"))
