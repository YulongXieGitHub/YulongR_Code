#
# 14_VariableSelection_Subset7_FINAL_2014March7_check_lowerStream_meso_other.R
#
...
Path.OUT <- "C:/YuLong_Projects/FY2012_MORiver/DataAnalysis/14_VariableSelection_Subset7_FINAL_2014March7_checl_meso_OTHER_lowerStream"
s
...
FL.LOG    <- paste(Path.log,"14_VariableSelection_Subset7_FINAL_2014March7_check_lowerStream_meso_other.log",sep="/")	
...
FL.RESL.OUT <- paste(Path.OUT,"14_VariableSelection_Subset7_FINAL_2014March7_check_lowerStream_meso_other.csv",sep="/")
...
#
FL_subset7.OUT <- paste(Path.OUT,"2014_March7_lower_logistic_actuallyUsed.csv",sep="/")

write.table(myData.Sub,file=FL_subset7.OUT,sep=",",row.names=FALSE,col.name=TRUE)
...
FL_subset7.OUT  <- paste(Path.OUT,"2014_March7_lower_logistic_dataPreparartion.csv",sep="/")
load(FL.IN)
myData.Lower.Logistic <- subset(myData.All,MA=="Lower")
write.table(myData.Lower.Logistic,file=FL_subset7.OUT,sep=",",row.names=FALSE,col.name=TRUE)




#	if(idx.subset == 7)	# Logistic Regression for Lower River
#	{
#		# [Subset 7 Logistic Regression on Lower River]: define an empty array to hold model summary
#		model.count <- 0
#		summary.out <- c()
#		
...
#
#		# ---------------------------------------------------------------------------------
#		# [Subset 7 Logistic Regression on Lower River]: 1. model with 1 variable (one variable)
#		# ---------------------------------------------------------------------------------
#		save.command <- ""
...
#
#		# ---------------------------------------------------------------------------------
#		# [Subset 7 Logistic Regression on Lower River]: 2. model with 2 variables: "SY" is best single
#		# ---------------------------------------------------------------------------------
#		no.tmp  <- length(c(X.var.num,X.var.cat))
#		idx.tmp <- 0
#		for (X.var.rm in grep("SY",c(X.var.num,X.var.cat),value=TRUE,invert=TRUE))		# all variables other than "SY"
#		{
#			idx.tmp <- idx.tmp + 1
#			idx.var.rm <- match(X.var.rm,names(myData.Work))				# the index of the variable to be removed in the data frame
#			if(!(is.na(idx.var.rm)))
#			{
#				model.count  <- model.count + 1
#				myData.Tmp   <- myData.Work[,c("binary","SY",X.var.rm)]	# two variable combinations: "SY" and the other
#				model.Tmp    <- glm(binary ~ .,data=myData.Tmp,family=binomial("logit"))
#				model.ID     <- X.var.rm
...
#	
#		# ---------------------------------------------------------------------------------
#		# [Subset 7 Logistic Regression on Lower River]: 3. model with 3 variables: ["SY"] & ["Temp"] and another one (three variables): "Temp" is te second best single with "SY"
#		# ---------------------------------------------------------------------------------
#		no.tmp  <- length(c(X.var.num,X.var.cat))
#		idx.tmp <- 0
#		for (X.var.rm in grep("Temp",grep("SY",c(X.var.num,X.var.cat),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE))		# all variables other than "Temp", "SY"
#		{
#			idx.tmp <- idx.tmp + 1
#			idx.var.rm <- match(X.var.rm,names(myData.Work))							# the index of the variable to be removed in the data frame
#			if(!(is.na(idx.var.rm)))
#			{
#				model.count  <- model.count + 1
#				myData.Tmp   <- myData.Work[,c("binary","SY","Temp",X.var.rm)]			# three variable combinations: "SY", "Temp" and the other
#				model.Tmp    <- glm(binary ~ .,data=myData.Tmp,family=binomial("logit"))
#				model.ID     <- X.var.rm
#				model.string <- paste("Model",model.count," with ",X.var.rm," & SY/Temp\n",sep=" ")
#				model.no.var <- dim(myData.Tmp)[2] - 1
#				model.rm.var <- paste("all but SY/Temp & ",X.var.rm,sep=" ")
#
#				model.formula       <- summary(model.Tmp)$call
#				model.coef          <- as.data.frame(summary(model.Tmp)$coef)
#				model.sum           <- c(summary(model.Tmp)$df.null,		 # null.df        
...
#	
#		# ---------------------------------------------------------------------------------
#		# [Subset 7 Logistic Regression on Lower River]: 4. model with 4 variables: ["Segment"], ["SY"] & ["Temp"] and another one (three variables):  "Segment" is the third best single with "SY", "Temp"
#		# ---------------------------------------------------------------------------------
#		no.tmp  <- length(c(X.var.num,X.var.cat))
#		idx.tmp <- 0
#		for (X.var.rm in grep("Seg",grep("Temp",grep("SY",c(X.var.num,X.var.cat),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE))		# all variables other than "Seg", "Temp", "SY"
#		{
#			idx.tmp <- idx.tmp + 1
#			idx.var.rm <- match(X.var.rm,names(myData.Work))							# the index of the variable to be removed in the data frame
#			if(!(is.na(idx.var.rm)))
#			{
#				model.count  <- model.count + 1
#				myData.Tmp   <- myData.Work[,c("binary","SY","Temp","Seg",X.var.rm)]			# three variable combinations: "SY", "Temp" and the other
#				model.Tmp    <- glm(binary ~ .,data=myData.Tmp,family=binomial("logit"))
#				model.ID     <- X.var.rm
#				model.string <- paste("Model",model.count," with ",X.var.rm," & SY/Temp/Seg\n",sep=" ")
#				model.no.var <- dim(myData.Tmp)[2] - 1
#				model.rm.var <- paste("all but SY/Temp/Seg & ",X.var.rm,sep=" ")
#
#				model.formula       <- summary(model.Tmp)$call
#				model.coef          <- as.data.frame(summary(model.Tmp)$coef)
#				model.sum           <- c(summary(model.Tmp)$df.null,		 # null.df        
...
#
#		# ---------------------------------------------------------------------------------
#		# [Subset 7 Logistic Regression on Lower River]: 5. model with 5 variables: ["Seg"], ["Segment"], ["SY"] & ["GIS_RM"] and another one (three variables): ["Seg"] is the fourth best single with "SY", "GIS_RM", "Segment"
#		# ---------------------------------------------------------------------------------
#		no.tmp  <- length(c(X.var.num,X.var.cat))
#		idx.tmp <- 0
#		for (X.var.rm in grep("GIS_RM",grep("Seg",grep("Temp",grep("SY",c(X.var.num,X.var.cat),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE))		# all variables other than  "Seg", "Seg", "GIS_RM", "SY",
#		{
#			idx.tmp <- idx.tmp + 1
#			idx.var.rm <- match(X.var.rm,names(myData.Work))									# the index of the variable to be removed in the data frame
#			if(!(is.na(idx.var.rm)))
#			{
#				model.count  <- model.count + 1
#				myData.Tmp   <- myData.Work[,c("binary","SY","Temp","Seg","GIS_RM",X.var.rm)]			# five variable combinations: "Seg", "Seg","SY", "GIS_RM" and the other
#				model.Tmp    <- glm(binary ~ .,data=myData.Tmp,family=binomial("logit"))
#				model.ID     <- X.var.rm
#				model.string <- paste("Model",model.count," with ",X.var.rm," &  SY/Temp/Seg/GIS_RM\n",sep=" ")
#				model.no.var <- dim(myData.Tmp)[2] - 1
#				model.rm.var <- paste("all but  SY/Temp/Seg/GIS_RM & ",X.var.rm,sep=" ")
#
#				model.formula       <- summary(model.Tmp)$call
#				model.coef          <- as.data.frame(summary(model.Tmp)$coef)
#				model.sum           <- c(summary(model.Tmp)$df.null,		 # null.df        
...
#				
#		# ---------------------------------------------------------------------------------
#		# [Subset 7 Logistic Regression on Lower River]: 6. model with 6 variables: ["SY"], ["Seg"], ["Segment"], ["SY"] & ["GIS_RM"] and another one (three variables):  ["SY"] is the fifth best single with "SY", "GIS_RM", "Segment", ["Seg"]
#		# ---------------------------------------------------------------------------------
#		no.tmp  <- length(c(X.var.num,X.var.cat))
#		idx.tmp <- 0
#		for (X.var.rm in grep("new_meso",grep("GIS_RM",grep("Seg",grep("Temp",grep("SY",c(X.var.num,X.var.cat),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE))		# all variables other than  "SY", "Seg", "Seg", "GIS_RM", "SY",
#		{
#			idx.tmp <- idx.tmp + 1
#			idx.var.rm <- match(X.var.rm,names(myData.Work))										# the index of the variable to be removed in the data frame
#			if(!(is.na(idx.var.rm)))
#			{
#				model.count  <- model.count + 1
#				myData.Tmp   <- myData.Work[,c("binary","SY","Temp","Seg","GIS_RM","new_meso",X.var.rm)]			# six variable combinations: "SY", "Seg", "Seg","SY", "GIS_RM" and the other
#				model.Tmp    <- glm(binary ~ .,data=myData.Tmp,family=binomial("logit"))
#				model.ID     <- X.var.rm
#				model.string <- paste("Model",model.count," with ",X.var.rm," & SY/Temp/Seg/GIS_RM/new_meso\n",sep=" ")
#				model.no.var <- dim(myData.Tmp)[2] - 1
#				model.rm.var <- paste("all but SY/Temp/Seg/GIS_RM/new_meso & ",X.var.rm,sep=" ")
#
#				model.formula       <- summary(model.Tmp)$call
#				model.coef          <- as.data.frame(summary(model.Tmp)$coef)
#				model.sum           <- c(summary(model.Tmp)$df.null,		 # null.df        
...
#				
#		# ---------------------------------------------------------------------------------
#		# [Subset 7 Logistic Regression on Lower River]: 7. model with 7 variables: ["Sand_per"], ["SY"], ["Seg"], ["Segment"], ["SY"] & ["GIS_RM"] and another one (three variables):  "Sand_per" is the sixth best single with "SY", "GIS_RM", "Segment", ["Seg"], ["SY"]
#		# ---------------------------------------------------------------------------------
#		no.tmp  <- length(c(X.var.num,X.var.cat))
#		idx.tmp <- 0
#		for (X.var.rm in  grep("Rel_dsch_AY",grep("new_meso",grep("GIS_RM",grep("Seg",grep("Temp",grep("SY",c(X.var.num,X.var.cat),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE))		# all variables other than  "Sand_per", "SY", "Seg", "Seg", "GIS_RM", "SY",
#		{
#			idx.tmp <- idx.tmp + 1
#			idx.var.rm <- match(X.var.rm,names(myData.Work))										# the index of the variable to be removed in the data frame
#			if(!(is.na(idx.var.rm)))
#			{
#				model.count  <- model.count + 1
#				myData.Tmp   <- myData.Work[,c("binary","SY","Temp","Seg","GIS_RM","new_meso","Rel_dsch_AY",X.var.rm)]			# seven variable combinations: "Sand_per", "SY", "Seg", "Seg","SY", "GIS_RM" and the other
#				model.Tmp    <- glm(binary ~ .,data=myData.Tmp,family=binomial("logit"))
#				model.ID     <- X.var.rm
#				model.string <- paste("Model",model.count," with ",X.var.rm," & SY/Temp/Seg/GIS_RM/new_meso/Rel_dsch_AY\n",sep=" ")
#				model.no.var <- dim(myData.Tmp)[2] - 1
#				model.rm.var <- paste("all but SY/Temp/Seg/GIS_RM//new_meso/Rel_dsch_AY & ",X.var.rm,sep=" ")
#
#				model.formula       <- summary(model.Tmp)$call
#				model.coef          <- as.data.frame(summary(model.Tmp)$coef)
#				model.sum           <- c(summary(model.Tmp)$df.null,		 # null.df        
...
#
#		# ---------------------------------------------------------------------------------
#		# [Subset 7 Logistic Regression on Lower River]: 8. model with 8 variables: ["Temp"], ["Sand_per"], ["SY"], ["Seg"], ["Segment"], ["Stk_130_cnt"] & ["GIS_RM"] and another one (three variables):  "Temp" is the seventh best single with "Stk_130_cnt", "GIS_RM", "Segment", ["Seg"], ["SY"], ["Sand_per"]
#		# ---------------------------------------------------------------------------------
#		no.tmp  <- length(c(X.var.num,X.var.cat))
#		idx.tmp <- 0
#		for (X.var.rm in  grep("Rel_dsch_WY",grep("Rel_dsch_AY",grep("new_meso",grep("GIS_RM",grep("Seg",grep("Temp",grep("SY",c(X.var.num,X.var.cat),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE))		# all variables other than  "Temp","Sand_per", "SY", "Seg", "Seg", "GIS_RM", "SY",
#		{
#			idx.tmp <- idx.tmp + 1
#			idx.var.rm <- match(X.var.rm,names(myData.Work))										# the index of the variable to be removed in the data frame
#			if(!(is.na(idx.var.rm)))
#			{
#				model.count  <- model.count + 1
#				myData.Tmp   <- myData.Work[,c("binary","SY","Temp","Seg","GIS_RM","new_meso","Rel_dsch_AY","Rel_dsch_WY",X.var.rm)]			# eight variable combinations: "Temp","Sand_per", "SY", "Seg", "Seg","SY", "GIS_RM" and the other
#				model.Tmp    <- glm(binary ~ .,data=myData.Tmp,family=binomial("logit"))
#				model.ID     <- X.var.rm
#				model.string <- paste("Model",model.count," with ",X.var.rm," & SY/Temp/Seg/GIS_RM/new_meso/Rel_dsch_AY/Rel_dsch_WY\n",sep=" ")
#				model.no.var <- dim(myData.Tmp)[2] - 1
#				model.rm.var <- paste("all but SY/Temp/Seg/GIS_RM/new_meso/Rel_dsch_AY/Rel_dsch_WY & ",X.var.rm,sep=" ")
#
#				model.formula       <- summary(model.Tmp)$call
#				model.coef          <- as.data.frame(summary(model.Tmp)$coef)
#				model.sum           <- c(summary(model.Tmp)$df.null,		 # null.df        
...
#
#		# ---------------------------------------------------------------------------------
#		# [Subset 7 Logistic Regression on Lower River]: 9. model with 9 variables: ["new_part"], ["Temp"], ["Sand_per"], ["SY"], ["Seg"], ["Segment"], ["SY"] & ["GIS_RM"] and another one (three variables):  "new_part" is the seventh best single with "SY", "GIS_RM", "Segment", ["Seg"], ["SY"], ["Sand_per"], ["Temp"]
#		# ---------------------------------------------------------------------------------
#		no.tmp  <- length(c(X.var.num,X.var.cat))
#		idx.tmp <- 0
#		for (X.var.rm in  grep("Season",grep("Rel_dsch_WY",grep("Rel_dsch_AY",grep("new_meso",grep("GIS_RM",grep("Seg",grep("Temp",grep("SY",c(X.var.num,X.var.cat),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE))		# all variables other than  "Temp","Sand_per", "SY", "Seg", "Seg", "GIS_RM", "SY",
#		{
#			idx.tmp <- idx.tmp + 1
#			idx.var.rm <- match(X.var.rm,names(myData.Work))										# the index of the variable to be removed in the data frame
#			if(!(is.na(idx.var.rm)))
#			{
#				model.count  <- model.count + 1
#				myData.Tmp   <- myData.Work[,c("binary","SY","Temp","Seg","GIS_RM","new_meso","Rel_dsch_AY","Rel_dsch_WY","Season",X.var.rm)]			# eight variable combinations: "Temp","Sand_per", "SY", "Seg", "Seg","SY", "GIS_RM" and the other
#				model.Tmp    <- glm(binary ~ .,data=myData.Tmp,family=binomial("logit"))
#				model.ID     <- X.var.rm
#				model.string <- paste("Model",model.count," with ",X.var.rm," & SY/Temp/Seg/GIS_RM/new_meso/Rel_dsch_WAY/Rel_dsch_WY/Season\n",sep=" ")
#				model.no.var <- dim(myData.Tmp)[2] - 1
#				model.rm.var <- paste("all but SY/Temp/Seg/GIS_RM/new_meso/Rel_dsch_WAY/Rel_dsch_WY/Season & ",X.var.rm,sep=" ")
#
#				model.formula       <- summary(model.Tmp)$call
#				model.coef          <- as.data.frame(summary(model.Tmp)$coef)
#				model.sum           <- c(summary(model.Tmp)$df.null,		 # null.df        
...
#
#		# ---------------------------------------------------------------------------------
#		# [Subset 7 Logistic Regression on Lower River]: 10. model with 10 variables: ["T_dist_up"], ["new_part"], ["Temp"], ["Sand_per"], ["SY"], ["Seg"], ["Segment"], ["SY"] & ["GIS_RM"] and another one (three variables):  "T_dist_up" is the seventh best single with "SY", "GIS_RM", "Segment", ["Seg"], ["SY"], ["Sand_per"], ["Temp"], ["new_part"
#		# ---------------------------------------------------------------------------------
#		no.tmp  <- length(c(X.var.num,X.var.cat))
#		idx.tmp <- 0
#		for (X.var.rm in   grep("Stk_24_cnt",grep("Season",grep("Rel_dsch_WY",grep("Rel_dsch_AY",grep("new_meso",grep("GIS_RM",grep("Seg",grep("Temp",grep("SY",c(X.var.num,X.var.cat),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE))		# all variables other than  "Temp","Sand_per", "SY", "Seg", "Seg", "GIS_RM", "SY",
#		{
#			idx.tmp <- idx.tmp + 1
#			idx.var.rm <- match(X.var.rm,names(myData.Work))										# the index of the variable to be removed in the data frame
#			if(!(is.na(idx.var.rm)))
#			{
#				model.count  <- model.count + 1
#				myData.Tmp   <- myData.Work[,c("binary","SY","Temp","Seg","GIS_RM","new_meso","Rel_dsch_AY","Rel_dsch_WY","Season","Stk_24_cnt",X.var.rm)]			# eight variable combinations: "Temp","Sand_per", "SY", "Seg", "Seg","SY", "GIS_RM" and the other
#				model.Tmp    <- glm(binary ~ .,data=myData.Tmp,family=binomial("logit"))
#				model.ID     <- X.var.rm
#				model.string <- paste("Model",model.count," with ",X.var.rm," & SY/Temp/Seg/GIS_RM/new_meso/Rel_dsch_WAY/Rel_dsch_WY/Season/Stk_24_cnt\n",sep=" ")
#				model.no.var <- dim(myData.Tmp)[2] - 1
#				model.rm.var <- paste("all but SY/Temp/Seg/GIS_RM/new_meso/Rel_dsch_WAY/Rel_dsch_WY/Season/Stk_24_cnt & ",X.var.rm,sep=" ")
#
#				model.formula       <- summary(model.Tmp)$call
#				model.coef          <- as.data.frame(summary(model.Tmp)$coef)
#				model.sum           <- c(summary(model.Tmp)$df.null,		 # null.df        
...
#		
#		# ---------------------------------------------------------------------------------
#		# [Subset 7 Logistic Regression on Lower River]: 11. model with 11 variables: ["Depth_mean", ["T_dist_up"], ["new_part"], ["Temp"], ["Sand_per"], ["SY"], ["Seg"], ["Segment"], ["SY"] & ["GIS_RM"] and another one (three variables):  "Depth_Mean" is the seventh best single with "SY", "GIS_RM", "Segment", ["Seg"], ["SY"], ["Sand_per"], ["Temp"], ["new_part", "T_dist_up"
#		# ---------------------------------------------------------------------------------
#		no.tmp  <- length(c(X.var.num,X.var.cat))
#		idx.tmp <- 0
#		for (X.var.rm in   grep("VF_wid",grep("Stk_24_cnt",grep("Season",grep("Rel_dsch_WY",grep("Rel_dsch_AY",grep("new_meso",grep("GIS_RM",grep("Seg",grep("Temp",grep("SY",c(X.var.num,X.var.cat),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE))		# all variables other than  "Temp","Sand_per", "SY", "Seg", "Seg", "GIS_RM", "SY",
#		{
#			idx.tmp <- idx.tmp + 1
#			idx.var.rm <- match(X.var.rm,names(myData.Work))										# the index of the variable to be removed in the data frame
#			if(!(is.na(idx.var.rm)))
#			{
#				model.count  <- model.count + 1
#				myData.Tmp   <- myData.Work[,c("binary","SY","Temp","Seg","GIS_RM","new_meso","Rel_dsch_AY","Rel_dsch_WY","Season","Stk_24_cnt","VF_wid",X.var.rm)]			# eight variable combinations: "Temp","Sand_per", "SY", "Seg", "Seg","SY", "GIS_RM" and the other
#				model.Tmp    <- glm(binary ~ .,data=myData.Tmp,family=binomial("logit"))
#				model.ID     <- X.var.rm
#				model.string <- paste("Model",model.count," with ",X.var.rm," & SY/Temp/Seg/GIS_RM/new_meso/Rel_dsch_WAY/Rel_dsch_WY/Season/Stk_24_cnt/VF_wid\n",sep=" ")
#				model.no.var <- dim(myData.Tmp)[2] - 1
#				model.rm.var <- paste("all but SY/Temp/Seg/GIS_RM/new_meso/Rel_dsch_WAY/Rel_dsch_WY/Season/Stk_24_cnt/VF_wid & ",X.var.rm,sep=" ")
#
#				model.formula       <- summary(model.Tmp)$call
#				model.coef          <- as.data.frame(summary(model.Tmp)$coef)
#				model.sum           <- c(summary(model.Tmp)$df.null,		 # null.df        
...
#
#		# ---------------------------------------------------------------------------------
#		# [Subset 7 Logistic Regression on Lower River]: 12. model with 12 variables: [GIS_RM], ["Depth_mean", ["T_dist_up"], ["new_part"], ["Temp"], ["Sand_per"], ["SY"], ["macro.type"], ["Segment"], ["Stk_120_cnt"] & ["GIS_RM"] and another one (three variables):  "" is the seventh best single with "Stk_120_cnt", "GIS_RM", "Segment", ["macro.type"], ["SY"], ["Sand_per"], ["Temp"], ["new_part", "T_dist_up", "Depth_Mean"
#		# ---------------------------------------------------------------------------------
#		no.tmp  <- length(c(X.var.num,X.var.cat))
#		idx.tmp <- 0
#		for (X.var.rm in   grep("new_part",grep("VF_wid",grep("Stk_24_cnt",grep("Season",grep("Rel_dsch_WY",grep("Rel_dsch_AY",grep("new_meso",grep("GIS_RM",grep("Seg",grep("Temp",grep("SY",c(X.var.num,X.var.cat),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE))		# all variables other than  "Temp","Sand_per", "SY", "macro.type", "Seg", "GIS_RM", "Stk_120_cnt",
#		{
#			idx.tmp <- idx.tmp + 1
#			idx.var.rm <- match(X.var.rm,names(myData.Work))										# the index of the variable to be removed in the data frame
#			if(!(is.na(idx.var.rm)))
#			{
#				model.count  <- model.count + 1
#				myData.Tmp   <- myData.Work[,c("binary","SY","Temp","Seg","GIS_RM","new_meso","Rel_dsch_AY","Rel_dsch_WY","Season","Stk_24_cnt","VF_wid","new_part",X.var.rm)]			# eight variable combinations: "Temp","Sand_per", "SY", "macro.type", "Seg","Stk_120_cnt", "GIS_RM" and the other
#				model.Tmp    <- glm(binary ~ .,data=myData.Tmp,family=binomial("logit"))
#				model.ID     <- X.var.rm
#				model.string <- paste("Model",model.count," with ",X.var.rm," & SY/Temp/Seg/GIS_RM/new_meso/Rel_dsch_WAY/Rel_dsch_WY/Season/Stk_24_cnt/VF_wid/new_part\n",sep=" ")
#				model.no.var <- dim(myData.Tmp)[2] - 1
#				model.rm.var <- paste("all but SY/Temp/Seg/GIS_RM/new_meso/Rel_dsch_WAY/Rel_dsch_WY/Season/Stk_24_cnt/VF_wid/new_part & ",X.var.rm,sep=" ")
#
#				model.formula       <- summary(model.Tmp)$call
#				model.coef          <- as.data.frame(summary(model.Tmp)$coef)
#				model.sum           <- c(summary(model.Tmp)$df.null,		 # null.df        
...
#Diff.time
#
#cat(paste("\n14_VariableSelection_Subset7_FINAL_2014March7_check_lowerStream_meso_other.R is finished successfully at ",End.time,"!\n",sep=" "))
#cat(paste("\n14_VariableSelection_Subset7_FINAL_2014March7_check_lowerStream_meso_other.R is finished successfully at ",End.time,"!\n",sep=" "),file=FL.LOG,append=TRUE)
#
#cat(paste("\nProcessing time for [14_VariableSelection_Subset7_FINAL_2014March7_check_lowerStream_meso_other.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "))
#cat(paste("\nProcessing time for [14_VariableSelection_Subset7_FINAL_2014March7_check_lowerStream_meso_other.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "),file=FL.LOG,append=TRUE)
#
##
