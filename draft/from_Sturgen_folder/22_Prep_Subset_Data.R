#
# 22_Prep_Subset_Data.R
#
# August 27, 2013
# noice difference in the number of observation used for exploratory anal;ysis in "12_Prep_Subset_Data.R" and in modeling 14 and 15 series such as "14_VariableSelection_Subset1_pscl_FINAL.R"  and "15_GenerateData_for_OpenBugs_subset1_FINAL.R")
# 
# The differnce comes from the number of variables kept for determing the NA observatiosn in the two sets of scripts.  
# Subset 1: The total observation is 8166  and the retained observation should be 7882.  
# Subset 2: The total observation is 576   and the retained observation should be 465. 
#
# Subset 4: The total observation is 1809  and thre are 7 NA in CPUE.calc and 7 zerso in SUPPORT.  The retained number of obsrvatiosn should be 1642 (instead of 1649).
#
# Subset 5: The total observation is 5427  and the retained observation should be 5297. 
#
# Subset 6: The total observation is 8742  and there is no need to use SUPPORT.calc.  there are 31 NA in SUPPORT.calc, so the retained number of observations should be 8378 instead of 8347.
# Subset 7: The total observation is 23514 and there is no need to use SUPPORT.calc.  there are 24 NA in SUPPORT.calc, so the retained number of observations should be 22673 instead of 22649.
#
#
# August 15, 2013
...
# June 24, 2013
#
# This is a revision of "8_Prep_Subset_Data.R".
#
#
...
Path.Out     <- "C:/YuLong_Projects/FY2012_MORiver/DataAnalysis/22_Prep_Subset_Data"
...
FL.LOG       <- paste(Path.log,"22_Prep_Subset_Data.log",                                     sep="/")	
FL.Data.OBJ  <- paste(Path.Out,"22_Prep_Subset_Data.Rdata",                                   sep="/")
...
for (idx.subset in c(1,2,3,4,5,6,7,8))
{
	...
	# ##########################################################################################################################################
	# ##########################################################################################################################################
	# August 15, 2013: for each subset set, only keep those variables we are going use. And delete any rows with missing data in any variables
	...
	if (!(is.na(new_y)))
	{
		var.to.use <- c(var.to.use,new_y,"SUPPORT.calc")		
	}
	...
	for (var in grep("[^Pal_cnt]",grep("[^binary]",names(myData.Sub),value=TRUE,perl=TRUE),value=TRUE,perl=TRUE))	# field other than "binary" and "Pal_cnt" in [myData.Sub]
	{
		...
		{	
			...
			plot.binary <- barchart(myData.Sub.freq[,var] ~ myData.Sub.freq[,"Freq"], data = myData.Sub.freq,groups = myData.Sub.freq[,"binary"],layout = c(1,1),stack = TRUE,auto.key = list(points = FALSE, rectangles = TRUE, space = "top"),xlab = "Freq",ylab = var,horizontal=TRUE,scales="free",between = list(x=0.5,y=0.5)) 

			# Defaults to FALSE if x is a factor or shingle, TRUE otherwise.
			if (idx.subset >= 8)
			{
				plot(plot.all,   split=c(1,1,2,3))
				plot(plot.lower, split=c(1,2,2,3),newpage=FALSE)
				plot(plot.upper, split=c(2,2,2,3),newpage=FALSE)
				plot(plot.binary,split=c(1,3,2,3),newpage=FALSE)
			}else{
				plot(plot.all,   split=c(1,1,1,2))
				plot(plot.binary,split=c(1,2,1,2),newpage=FALSE)
			}
			...
			if(!(is.na(new_y)))
			{
				...
				command.string  <- paste("plot_new1 <- bwplot(",new_y,"   ~ ",var,",data=myData.tmp_new1,      notch=TRUE,outline=FALSE,range=0,main=\"",paste(subset.section,subset.gear,sep="-")," (all data)\",  col=c(\"red\",\"blue\",\"cyan\",\"magenta\",\"green\",\"brown\",\"yellow\",\"pink\",\"purple\"),xlab=\"",var,"\",ylab=\"",new_y,"\")",sep="")
				eval(parse(text=command.string))
				
				myData.tmp_new2 <- myData.Sub[!(is.na(myData.Sub[,var])) & myData.Sub[,"binary"] == "yes" ,c(var,new_y)]
				command.string  <- paste("plot_new2 <- bwplot(",new_y,"   ~ ",var,",data=myData.tmp_new2,      notch=TRUE,outline=FALSE,range=0,main=\"",paste(subset.section,subset.gear,sep="-")," (presence)\",  col=c(\"red\",\"blue\",\"cyan\",\"magenta\",\"green\",\"brown\",\"yellow\",\"pink\",\"purple\"),xlab=\"",var,"\",ylab=\"",new_y,"\")",sep="")
				eval(parse(text=command.string))	
		...
		# continuous variables		
			...
			if(!(is.na(new_y)))
			{
				...
				command.string <- paste("plot_new4 <- xyplot(myData.tmp_new4[,\"",new_y,"\"] ~ myData.tmp_new4[,\"",var,"\"],data=myData.tmp_new4,main=\"",paste(subset.section,subset.gear,sep="-")," (all data)\",xlab=\"",var,"\",ylab=\"",new_y,"\",pch=16,cex=0.5,col=\"red\")",sep="")
				eval(parse(text=command.string))
				
				
				myData.tmp_new5 <- myData.Sub[!(is.na(myData.Sub[,var])) & myData.Sub[,"binary"] == "yes" ,c(var,new_y)]
				command.string <- paste("plot_new5 <- xyplot(myData.tmp_new5[,\"",new_y,"\"] ~ myData.tmp_new5[,\"",var,"\"],data=myData.tmp_new5,main=\"",paste(subset.section,subset.gear,sep="-")," (presence)\",xlab=\"",var,"\",ylab=\"",new_y,"\",pch=16,cex=0.5,col=\"blue\")",sep="")
				eval(parse(text=command.string))
...
#
save(myData.Upper_Active,myData.Upper_TLC,myData.Lower_Active,myData.Lower_TLC,myData.Lower_GN,myData.Upper_Logistic,myData.Lower_Logistic,myData.All,col.title.Upper_Active,col.title.Upper_TLC,col.title.Lower_Active,
     col.title.Lower_TLC,col.title.Lower_GN,col.title.Upper_Logistic,col.title.Lower_Logistic,col.title.All,col.class.Upper_Active,col.class.Upper_TLC,col.class.Lower_Active,col.class.Lower_TLC,col.class.Lower_GN,col.class.Upper_Logistic,col.class.Lower_Logistic,col.class.All,
     myData.Upper_Active.for.plot,myData.Upper_TLC.for.plot,myData.Lower_Active.for.plot,myData.Lower_TLC.for.plot,myData.Lower_GN.for.plot,myData.Upper_Logistic.for.plot,myData.Lower_Logistic.for.plot,myData.All,
     var.Upper_Active.for.plot,var.Upper_TLC.for.plot,var.Lower_Active.for.plot,var.Lower_TLC.for.plot,var.Lower_GN.for.plot,var.Upper_Logistic.for.plot,var.Lower_Logistic.for.plot,var.All.for.plot,
     file=FL.Data.OBJ)	
...
cat(paste("\n22_Prep_Subset_Data.R is finished successfully at ",End.time,"!\n",sep=" "))
cat(paste("\n22_Prep_Subset_Data.R is finished successfully at ",End.time,"!\n",sep=" "),file=FL.LOG,append=TRUE)

cat(paste("\nProcessing time for [22_Prep_Subset_Data.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "))
cat(paste("\nProcessing time for [22_Prep_Subset_Data.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "),file=FL.LOG,append=TRUE)
