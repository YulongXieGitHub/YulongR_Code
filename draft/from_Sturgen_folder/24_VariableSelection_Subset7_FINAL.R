#
# 24_VariableSelection_Subset7_final.R
#
# August 27, 2013
#
...
FL.LOG    <- paste(Path.log,"24_VariableSelection_Subset7_final.log",sep="/")	
...
FL.RESL.OUT <- paste(Path.OUT,"24_VariableSelection_Subset7_final.csv",sep="/")
...
for (idx.subset in c(7))
{
	...
	# subset 7: Logistic Lower [myData.Lower_Logistic]
		...
		Y.var  <- "binary"
		var.rm.fix <- c("Gear","D_dist_dn","D_dist_near","MA","Lith_1","Lith_2","Lith_desc","NFHAP","Tax_part_sz","Turb","Meso","WaterVel","Fish_cnt","CPUE","CPUA","Stk_12_dist","Stk_24_dist","Stk_36_dist","Stk_60_dist","Stk_120_dist","gear.type2","CPUA.calc","CPUE.calc","Comp_per","Sand_per","Clay_per","new_lith","new_part","gear.type1")
		var.rm     <- c("Gear","D_dist_dn","D_dist_near","MA","Lith_1","Lith_2","Lith_desc","NFHAP","Tax_part_sz","Turb","Meso","WaterVel","Fish_cnt","CPUE","CPUA","Stk_12_dist","Stk_24_dist","Stk_36_dist","Stk_60_dist","Stk_120_dist","gear.type2","CPUA.calc","CPUE.calc","Comp_per","Sand_per","Clay_per","new_lith","new_part","gear.type1")
	...
	{
		idxCol.2.subset <- match(c(var.rm.fix,"SUPPORT.calc"),names(myData.Sub))	
	}else{
	...
	}else if (idx.subset == 8)	# Logistic Regression for the Lower River Stream
	{
	}
}
...
cat(paste("\n24_VariableSelection_Subset7_final.R is finished successfully at ",End.time,"!\n",sep=" "))
cat(paste("\n24_VariableSelection_Subset7_final.R is finished successfully at ",End.time,"!\n",sep=" "),file=FL.LOG,append=TRUE)

cat(paste("\nProcessing time for [24_VariableSelection_Subset7_final.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "))
cat(paste("\nProcessing time for [24_VariableSelection_Subset7_final.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "),file=FL.LOG,append=TRUE)
