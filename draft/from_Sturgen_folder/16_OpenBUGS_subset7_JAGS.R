...
FL.LOG    <- paste(Path.log,"16_OpenBUGS_subset7_JAGS.log",sep="/")	
...
"model{
	...
	for (j in 1:",no.var,"){a[j]  ~ dnorm(0,1.0E-6)}
			...
			cat(paste("8. Data has been prepared and loaded for JAGS!\n"))
			cat(paste("8. Data has been prepared and loaded for JAGS!\n"),file=FL.LOG,append=TRUE)
			...
			cat(paste("10. Specify for JAGS\n"))
			cat(paste("10. Specify for JAGS!\n"),file=FL.LOG,append=TRUE)


			# Create, initialize, and adpat the model
			cat( "Create, initialize, and adpat the model...\n" )
			jagsModel <- jags.model(file = FL.MODEL,data=dataList , n.chains=nChains , n.adapt=adaptSteps )	# fail if I delete the initialization list
			cat(paste("11. initial JAGS\n"))
			cat(paste("11. Specify JAGS!\n"),file=FL.LOG,append=TRUE)
			...
			cat(paste("12. run JAGS\n"))
			cat(paste("12. run JAGS!\n"),file=FL.LOG,append=TRUE)
			
			save.image(file=FL.OBJ)
			cat(paste("13. Save JAGS\n"))
			cat(paste("13. Save JAGS!\n"),file=FL.LOG,append=TRUE)				
...
cat(paste("\n16_OpenBUGS_subset7_JAGS.R is finished successfully at ",End.time,"!\n",sep=" "))
cat(paste("\n16_OpenBUGS_subset7_JAGS.R is finished successfully at ",End.time,"!\n",sep=" "),file=FL.LOG,append=TRUE)

cat(paste("\nProcessing time for [16_OpenBUGS_subset7_JAGS.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "))
cat(paste("\nProcessing time for [16_OpenBUGS_subset7_JAGS.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "),file=FL.LOG,append=TRUE)
