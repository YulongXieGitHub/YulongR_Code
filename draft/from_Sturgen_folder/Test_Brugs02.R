...
Path.Out     <- "C:/YuLong_Projects/FY2012_MORiver/DataAnalysis/Test_Brugs02"
...
FL.Data.OUT <- paste(Path.Out,"Test_Brugs02_data.csv",sep="/")
FL.RESL.OUT <- paste(Path.Out,"Test_Brugs02_results.csv",sep="/")
FL.MODEL    <- paste(Path.Out,"Test_Brugs02_model.Rdata",sep="/")

FL.PostDist.CSV <- paste(Path.Out,"Test_Brugs02_PostDist.csv",sep="/")
FL.PostDist.OBJ <- paste(Path.Out,"Test_Brugs02_PostDist.Rdata",sep="/")


FL.LOG      <- paste(Path.log,"Test_Brugs02.log",sep="/")	
FL.PDF      <- paste(Path.Out,"Test_Brugs02.pdf",sep="/")	
FL.SUM.cat  <- paste(Path.Out,"Test_Brugs02_cat.sum",sep="/")
FL.SUM.num  <- paste(Path.Out,"Test_Brugs02_num.sum",sep="/")
...
# -------------------------------------------------------------------------------------------------
# output the other format used by WinBUGS or BUGS: [Test_Brugs02_data.csv]
# -------------------------------------------------------------------------------------------------
...
modelstring = "
	model{
		...
		for (j in 1:13){a[j]  ~ dnorm(0,1.0E-6)}
		for (j in 1:13){b[j]  ~ dnorm(0,1.0E-6)}
...
modelstring = "
	model{
		...
		for (j in 1:7){a[j]  ~ dnorm(0,1.0E-4)}
		for (j in 1:7){b[j]  ~ dnorm(0,1.0E-4)}
...
datalist = list(
		   Pal_cnt        = as.vector(myData.reduced[,"Pal_cnt"]),
		   D_dist_up      = as.vector(myData.reduced[,"D_dist_up"]),
		   Chan_wid       = as.vector(myData.reduced[,"Chan_wid"]),
		   T_dist_up      = as.vector(myData.reduced[,"T_dist_up"]),
		   Grade_10_RM    = as.vector(myData.reduced[,"Grade_10_RM"]),
		   VF_wid         = as.vector(myData.reduced[,"VF_wid"]),		   
		   Temp           = as.vector(myData.reduced[,"Temp"]),
		   Depth_Mean     = as.vector(myData.reduced[,"Depth_Mean"]),	   
...
predictorNames = c("D_dist_up","Chan_wid","T_dist_up","Grade_10_RM","VF_wid","Temp","Depth_Mean")
...
cat(paste("3. Model has been compiled!\n"))
...
#------------------------------------------------------------------------------
 genInitList1 <- function(nPred=nPredictors) {					#
 	list(									#
 		a0 = -0.06, 							#
 		b0 =  1.19,							#
 		a  = c(-6.5712,-0.4831,0.4302,-04708,-0.3367,0.7129,-0.3173),			#
 		b  = c(-0.744121,-0.237194,0.001871,-0.004010,0.094706,0.054689,0.070450)				#
 	)									#
 }										#
...
cat(paste("4. Data has been initialized!\n"))
cat(paste("4. Data has been initialized!\n"),file=FL.LOG,append=TRUE)
...
cat(paste("5a. MCMC has been burning in!\n"))
cat(paste("5a. MCMC has been burning in!\n"),file=FL.LOG,append=TRUE)
...
cat(paste("5b. Parameter monitor is set!\n"))
cat(paste("5b. Parameter monitor is set!\n"),file=FL.LOG,append=TRUE)

# it seems like "dic" function has to be placed after sample function
  dicSet()
cat(paste("5c. Dic is set!\n"))
cat(paste("5c. Dic is set!\n"),file=FL.LOG,append=TRUE)
...
cat(paste("6a. MCMC has been updated!\n"))
cat(paste("6a. MCMC has been updated!\n"),file=FL.LOG,append=TRUE)

  dicStats()
cat(paste("6b. DIC are extracted!\n"))
cat(paste("6b. DIC are extracted!\n"),file=FL.LOG,append=TRUE)
...
cat(paste("7. MCMC chain traces have been plotted!\n"))
cat(paste("7. MCMC chain traces have been plotted!\n"),file=FL.LOG,append=TRUE)
...
cat(paste("8. MCMC parameters of posterior distrbution have been extracted and saved!\n"))
cat(paste("8. MCMC parameters of posterior distrbution have been extracted and saved!\n"),file=FL.LOG,append=TRUE)
...
cat(paste("9. MCMC parameters coorelation have been plotted!\n"))
cat(paste("9. MCMC parameters coorelation have been plotted!\n"),file=FL.LOG,append=TRUE)
...
cat(paste("10. MCMC parameters posterior distribution have been plotted!\n"))
cat(paste("10. MCMC parameters posterior distribution have been plotted!\n"),file=FL.LOG,append=TRUE)
...
no.data       <- dim(myData.reduced)[1]
no.postSample <- dim(a0Sample)[1] / no.thin
xPostPred = cbind(
		   as.vector(myData.reduced[,"D_dist_up"]),
		   as.vector(myData.reduced[,"Chan_wid"]),
		   as.vector(myData.reduced[,"T_dist_up"]),
		   as.vector(myData.reduced[,"Grade_10_RM"]),
		   as.vector(myData.reduced[,"VF_wid"])
		   )
...
cat(paste("11. A variety of quantities have been calculated for each observation based on the posterior distribution!\n"))
cat(paste("11. A variety of quantities have been calculated for each observation based on the posterior distribution!\n"),file=FL.LOG,append=TRUE)
...
#
table.summary <- cbind(observed.count   = myData.reduced[,"Pal_cnt"],
...
cat(paste("12. Processing of Posterior Distribution are finished!\n"))
cat(paste("12. Processing of Posterior Distribution are finished!\n"),file=FL.LOG,append=TRUE)
...
cat(paste("\nTest_Brugs02.R is finished successfully at ",End.time,"!\n",sep=" "))
cat(paste("\nTest_Brugs02.R is finished successfully at ",End.time,"!\n",sep=" "),file=FL.LOG,append=TRUE)

cat(paste("\nProcessing time for [Test_Brugs02.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "))
cat(paste("\nProcessing time for [Test_Brugs02.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "),file=FL.LOG,append=TRUE)
