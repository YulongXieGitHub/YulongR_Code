...
Path.Out     <- "C:/YuLong_Projects/FY2012_MORiver/DataAnalysis/8_Prep_Subset_Data"
...
FL.LOG       <- paste(Path.log,"8_Prep_Subset_Data.log",               sep="/")	
FL.Data.OBJ  <- paste(Path.Out,"8_Prep_Subset_Data.Rdata",             sep="/")
...
for (idx.subset in c(1,2,3,4,5,6,7,8))
{
	...
	FL.Plots.PDF <- paste(Path.Out,paste("8_indVar_Gear(",subset.gear,")_River(",subset.section,")_plot.pdf",sep=""),sep="/")	
	FL.Data.Sum  <- paste(Path.Out,paste("8_indVar_Gear(",subset.gear,")_River(",subset.section,")_data.csv",sep=""),sep="/")	
	...
	# 3. count summary: 
	# -------------------------------------------------------------------------------------------------	
	FL.Count.Summary.csv <- paste(Path.Out,paste("8_indVar_Gear(",subset.gear,")_River(",subset.section,")_Count_Summary.csv",sep=""),sep="/")
	...
	# -------------------------------------------------------------------------------------------------	
	FL.Binary.Summary.csv <- paste(Path.Out,paste("8_indVar_Gear(",subset.gear,")_River(",subset.section,")_Binary_Summary.csv",sep=""),sep="/")
	...
	#
	FL.Count_CPUE_CPUA.PDF <- paste(Path.Out,paste("8_indVar_Gear(",subset.gear,")_River(",subset.section,")_CPUE_CPUA_Count.pdf",sep=""),sep="/")	
...
cat(paste("\n8_Prep_Subset_Data.R is finished successfully at ",End.time,"!\n",sep=" "))
cat(paste("\n8_Prep_Subset_Data.R is finished successfully at ",End.time,"!\n",sep=" "),file=FL.LOG,append=TRUE)

cat(paste("\nProcessing time for [8_Prep_Subset_Data.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "))
cat(paste("\nProcessing time for [8_Prep_Subset_Data.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "),file=FL.LOG,append=TRUE)
