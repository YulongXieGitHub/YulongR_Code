...
#
# The same subset of data can be treated with either pscl, OpenBUGS, WinBUGS or BRUGS.  So need to set up a method string
#
method.string <- "BRUGS"
...
Path.OUT <- "C:/YuLong_Projects/FY2012_MORiver/DataAnalysis/16_OpenBUGS"
...
FL.LOG    <- paste(Path.log,"16_OpenBUGS_subset7_Brugs.log",sep="/")	
...
for (idx.subset in c(7))
{
	...
	Path.OUT.subset <- paste(Path.OUT,paste(method.string,paste("subset",idx.subset,sep=""),subset.section,subset.gear,sep="_"),sep="/")
	...
	if(idx.subset == 7)	# PSCL Lower TLC
	{
		...
		for (model4test in seq(from=1,to=1,by=1))
		{
			if (model4test == 1)
			{
				list.var <- c("SY","Temp","Seg","GIS_RM","new_meso","Rel_dsch_AY","Rel_dsch_WY","Season","Comp_per","VF_wid","new_part")
				A <- c(-5.739053756,0.446121721,0.846033637,0.473889624,0.759361612,0.2841103,-0.309689904,1.06806343,1.299621509,1.943461427,2.352906442,2.422443809,0.970622475,0.576169949,0.602471516,-0.435334929,0.468496656,-0.201352386,0.196115116,-0.591189372) 
			}
			...
			FL.OBJ     <- paste(Path.OUT.var,paste("RData_Subset_",idx.subset,"_",subset.section,"_",subset.gear,"_",method.string,".RData",sep=""),sep="/")
			FL.SAM     <- paste(Path.OUT.var,paste("Sample_",idx.subset,"_",subset.section,"_",subset.gear,"_",method.string,".RData",sep=""),sep="/")
			...
			if  (file.exists(FL.SAM))  {print(paste(FL.SAM,"exist.Delete it!")); file.remove(FL.SAM)}
			...
			myData.Tmp     <- cbind(myData.Y,myData.X)	# note we do have support for logistic regression
		  names(myData.Tmp)    <- c("binary",names(myData.X))	# note we do have support for logistic regression
			myData.support <- SUPPORT.calc
			cat(paste("4c. data ready for Bugs!\n",sep=""))
			cat(paste("4c. data ready for Bugs!\n",sep=""),file=FL.LOG,append=TRUE)										

			#
			# for logistic regression: convert binary to 0/1
			#
			binary.tmp <- rep(0,dim(myData.Tmp)[1])
			binary.tmp[myData.Tmp[,"binary"] == "yes"] <- 1
			myData.Tmp[,"binary"] <- binary.tmp			
			cat(paste("4d. convert [yes/no] of \"binary\" to 0/1 for OpenBUGS!\n",sep=""))
			cat(paste("4d. convert [yes/no] of \"binary\" to 0/1 for OpenBUGS!\n",sep=""),file=FL.LOG,append=TRUE)										
...
"model{
	for (i in 1:n)					
	{
		...
		mu[i] <-    1 / (1+exp(-(a0                      + 	\n"	
				...
				for (idx.var in X.var.kept)
				{
					...
					if(idx.count == no.var)
					{
						part2 <- paste(part2,"\t\ta[",idx.count,"]*",idx.var,"[i]\n",   sep="")
					}else{
						part2 <- paste(part2,"\t\ta[",idx.count,"]*",idx.var,"[i]                      + \n",sep="")
					}
				}
	...
	# normal distributions are assumed for all the parameters of logistic regression
	...
	for (j in 1:",no.var,"){a[j]  ~ dnorm(0,1.0E-6)}
			...
			modelstring = paste(part1,part2,part3)
			...
			command.string <- paste("dataList = list(",sep="")
			for (var in names(myData.Tmp))
			{
				command.string <- paste(command.string,"\"",var,"\"=as.vector(myData.Tmp[,\"",var,"\"]),",sep="")
			}
			command.string <- paste(command.string,"n = ",no.obs,")",sep="") 


			eval(parse(text=command.string))

			cat(paste("8. Data has been prepared and loaded for BRUGS!\n"))
			cat(paste("8. Data has been prepared and loaded for BRUGS!\n"),file=FL.LOG,append=TRUE)


			modelData( bugsData( dataList ) )
			cat(paste("8. Data has been prepared and loaded!\n"))
			cat(paste("8. Data has been prepared and loaded!\n"),file=FL.LOG,append=TRUE)

			# ------------------------------------------------------------------------------
			# define variable names
			# ------------------------------------------------------------------------------
			predictedName  = "Pal_cnt"

			idx.count <- 0
			tmp.part1 <- paste("predictorNames <- c(",sep="")
			tmp.part2 <- ""
			for (var in X.var.kept)
			{
				idx.count <- idx.count + 1
				if(idx.count == no.var)
				{
					tmp.part2 <- paste(tmp.part2,"\"",var,"\")",sep="")
				}else{
					tmp.part2 <- paste(tmp.part2,"\"",var,"\",",sep="")
				}
			}
			command.string <- paste(tmp.part1,tmp.part2,sep="")
			eval(parse(text=command.string))
			nPredictors <- length(predictorNames)
			cat(paste("9. cccccccccccccccccccc\n"))
			cat(paste("9. cccccccccccccccccccc!\n"),file=FL.LOG,append=TRUE)
			...
			cat(paste("10. Model has been compiled!\n"))
			...
			cat(paste("11. Specify initial values!\n"))
			cat(paste("11. Specify initial values!\n"),file=FL.LOG,append=TRUE)
			...
			cat(paste("12. Data has been initialized!\n"))
			cat(paste("12. Data has been initialized!\n"),file=FL.LOG,append=TRUE)
			...
			cat(paste("13. MCMC has been burning in!\n"))
			cat(paste("13. MCMC has been burning in!\n"),file=FL.LOG,append=TRUE)
			...
			#------------------------------------------------------------------------------
			samplesSet( c("a0","a","deviance") )
			nPerChain = 50000
			thinStep = 100
			modelUpdate( nPerChain , thin=thinStep )
			cat(paste("14. MCMC has been updated!\n"))
			cat(paste("14. MCMC has been updated!\n"),file=FL.LOG,append=TRUE)
			...
			if ( checkConvergence ) {
				a0Sum = plotChains( "a0" , saveplots=T , filenameroot=fname,path.pdf=Path.OUT.var )
				dev.copy2pdf(file=paste(Path.OUT.var,paste(fname,"_ZeroInflation_a0.pdf",sep=""),sep="/"))

				# b0Sum = plotChains( "b0" , saveplots=T , filenameroot=fname,path.pdf=Path.OUT.var )
				# dev.copy2pdf(file=paste(Path.OUT.var,paste(fname,"_Poisson_b0.pdf",sep=""),sep="/"))

				aSum  = plotChains( "a"  , saveplots=T , filenameroot=fname,path.pdf=Path.OUT.var )
				dev.copy2pdf(file=paste(Path.OUT.var,paste(fname,"_ZeroInflation_a.pdf",sep=""),sep="/"))

				# bSum  = plotChains( "b"  , saveplots=T , filenameroot=fname,path.pdf=Path.OUT.var )
				# dev.copy2pdf(file=paste(Path.OUT.var,paste(fname,"_Poisson_b.pdf",sep=""),sep="/"))	
			}
			cat(paste("15. MCMC chain traces have been plotted!\n"))
			cat(paste("15. MCMC chain traces have been plotted!\n"),file=FL.LOG,append=TRUE)
			...
			save( a0Sample,           aSample, file=FL.SAM)
		      # save( a0Sample, b0Sample, aSample,bSample, file=FL.SAM)
			cat(paste("16. MCMC parameters of posterior distrbution have been extracted and saved!\n"))
			cat(paste("16. MCMC parameters of posterior distrbution have been extracted and saved!\n"),file=FL.LOG,append=TRUE)
			...
			if ( nPredictors <= nPredictors ) { # don't display if too many predictors
			    ...
			    dev.copy2pdf(file=paste(Path.OUT.var,paste(fname,"_Pairs_ZeroInflation.pdf",sep=""),sep="/"))

			    # pairs(cbind(b0Sample[thinIdx,],bSample[thinIdx,]),labels=c("intercept",predictorNames),main=paste("Poisson Parameters",sep=""))      
			    # dev.copy2pdf(file=paste(Path.OUT.var,paste(fname,"_Pairs_Poisson.pdf",sep=""),sep="/"))
			}
			cat(paste("17. MCMC parameters of posterior distrbution have been plotted!\n"))
			cat(paste("17. MCMC parameters of posterior distrbution have been plotted!\n"),file=FL.LOG,append=TRUE)
			
			save.image(file=FL.OBJ)
			cat(paste("18. Save BRUGS\n"))
			cat(paste("18. Save BRUGS!\n"),file=FL.LOG,append=TRUE)				
...
cat(paste("\n16_OpenBUGS_subset7_Brugs.R is finished successfully at ",End.time,"!\n",sep=" "))
cat(paste("\n16_OpenBUGS_subset7_Brugs.R is finished successfully at ",End.time,"!\n",sep=" "),file=FL.LOG,append=TRUE)

cat(paste("\nProcessing time for [16_OpenBUGS_subset7_Brugs.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "))
cat(paste("\nProcessing time for [16_OpenBUGS_subset7_Brugs.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "),file=FL.LOG,append=TRUE)
