#
# 25_GenerateData_for_OpenBugs_subset7_Final.R
#
...
Path.OUT <- "C:/YuLong_Projects/FY2012_MORiver/DataAnalysis/25_GenerateData_for_OpenBugs_Final"
...
FL.LOG    <- paste(Path.log,"25_GenerateData_for_OpenBugs.log",sep="/")	
...
for (idx.subset in c(7))
{
	...
	# subset 7: Logistic Lower [myData.Lower_Logistic]
		...
		Y.var  <- "binary"
		var.rm.fix <- c("Gear","D_dist_dn","D_dist_near","MA","Lith_1","Lith_2","Lith_desc","NFHAP","Tax_part_sz","Turb","Meso","WaterVel","Fish_cnt","CPUE","CPUA","Stk_12_dist","Stk_24_dist","Stk_36_dist","Stk_60_dist","Stk_120_dist","gear.type2","CPUA.calc","CPUE.calc","Comp_per","Sand_per","Clay_per","new_lith","new_part")	# August 27, 2013: include "gear.type1"
		var.rm     <- c("Gear","D_dist_dn","D_dist_near","MA","Lith_1","Lith_2","Lith_desc","NFHAP","Tax_part_sz","Turb","Meso","WaterVel","Fish_cnt","CPUE","CPUA","Stk_12_dist","Stk_24_dist","Stk_36_dist","Stk_60_dist","Stk_120_dist","gear.type2","CPUA.calc","CPUE.calc","Comp_per","Sand_per","Clay_per","new_lith","new_part")	# August 27, 2013: include "gear.type1"
	...
	if(idx.subset >=6)	# do not use "SUPPORT.calc"
	{
		idxCol.2.subset <- match(c(var.rm,"SUPPORT.calc"),names(myData.Sub))	
	}else{
		idxCol.2.subset <- match(var.rm,names(myData.Sub))
	}
	...
	idxCol.2.rm <- match(c(var.rm,"SUPPORT.calc"),names(myData.Sub))
	...
	idx.2.rm <- match(c(var.rm,"SUPPORT.calc"),col.title.4.sub)
	...
	cat("[col.title.reduced]\n")
		
	#	
	X.var.cat    <- grep("binary",col.title.reduced[col.class.reduced == "factor"],value=TRUE,invert=TRUE)
	X.var.num    <- grep("Pal_cnt",col.title.reduced[col.class.reduced != "factor"],value=TRUE,invert=TRUE)
      # X.var.offset <- "SUPPORT.calc"
        cat("[X.var.cat] & [X.var.num]\n")
	...
	cat("[myData.X.num] & [myData.X.cat] & [myData.Y]\n")
	...
	for (i in names(myData.X.cat))
	{
		...
		}else{								# 2013 August 27: if it is already a factor but subsetting may have incomplete levels, reset the levels to the available levels
			myData.X.cat[,i] <- factor(myData.X.cat[,i])
		}
	}
	...
	if(idx.subset == 7)	# PSCL Lower TLC
	{
		...
		for (model4test in seq(from=1,to=1,by=1))
		{
			if (model4test == 1)
			{
				list.var <- c("gear.type1","Seg","SY","GIS_RM","new_meso","Stk_24_cnt","VF_wid")
				A <- c(-5.305583188,-1.518961817,1.42372981,-0.971789165,1.353913905,1.955654931,2.248450875,2.568554799,2.623246331,0.358831336,0.887896317,0.602449513,0.592449482,0.684996743,0.976331015,0.453913988,0.137859287,0.164680567)
			}
...
cat(paste("\n25_GenerateData_for_OpenBugs.R is finished successfully at ",End.time,"!\n",sep=" "))
cat(paste("\n25_GenerateData_for_OpenBugs.R is finished successfully at ",End.time,"!\n",sep=" "),file=FL.LOG,append=TRUE)

cat(paste("\nProcessing time for [25_GenerateData_for_OpenBugs.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "))
cat(paste("\nProcessing time for [25_GenerateData_for_OpenBugs.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "),file=FL.LOG,append=TRUE)
