#
# 15_GenerateData_for_OpenBugs_subset7_Final.R
#
...
for (idx.subset in c(7))
{
	...
	if(idx.subset == 7)	# PSCL Lower TLC
	{
		...
		for (model4test in seq(from=1,to=1,by=1))
		{
			if (model4test == 1)
			{
				list.var <- c("SY","Temp","Seg","GIS_RM","new_meso","Rel_dsch_AY","Rel_dsch_WY","Season","Stk_24_cnt","VF_wid","new_part")
				A <- c(-6.025946808,0.447388875,0.841869868,0.464378917,0.741884535,0.276946583,-0.32746582,1.163944508,1.659647901,2.218979546,2.721457909,3.007541974,1.17290996,0.550894352,0.599000009,-0.429797055,0.467067121,0.097563299,0.170823638,-0.427889296)
