...
FL.Vars.PDF  <- paste(Path.Out,"2_Chk_April5_2013Data_Vars.pdf",sep="/")	
FL.Count.PDF <- paste(Path.Out,"2_Chk_April5_2013Data_Count.pdf",sep="/")	
FL.SUM.cat   <- paste(Path.Out,"2_Chk_April5_2013Data_cat.sum",sep="/")
FL.SUM.num   <- paste(Path.Out,"2_Chk_April5_2013Data_num.sum",sep="/")
...
if  (file.exists(FL.Vars.PDF)) {print(paste(FL.Vars.PDF, "exist.Delete it!")); file.remove(FL.Vars.PDF)}
if  (file.exists(FL.Count.PDF)){print(paste(FL.Count.PDF,"exist.Delete it!")); file.remove(FL.Count.PDF)}
...
pdf(file = FL.Vars.PDF,         paper="a4r",width=0,height=0)	


# read April 8, 2013 data 
row.titles  <- c("ID",     "SY",     "FieldOffice","Project","UniqueID","Gear",  "Season","Bend",   "BendRN","BendRiverMile","Near_NHD2rm_FID","Near_NHD2RM_dist_m","NHD2_RM","X1960BendID","RM1960_RM","UpDamDist","UpDamNm","DnDamDist","DnDamNm","Ch_W_Full","Ch_W_NoIsl","Braiding","UpTrib","UpTribDist","DnTrib","DTribDist","Reach", "Mean_z", "Max_z",  "Min_z",  "Grade10RM","GradeBend","Rocktype1","Rocktype2","Lithology","NFHAP_Scr","MedFlow","MeanFlow","taxorder","taxpartsiz","comppct_r","sandtotal_r","claytotal_r","frag3to10_r","VF_width","Segment","nPage",  "TotalPages","SetDate",  "Replicate","ReplicateRN","SubSample","SubSamplePass","SubSampleN","Biologist","Recorder", "CheckBy",  "Temp",   "Turbidity","Conductivity","DO",     "Distance","Width",  "NetRiverMile","StructureNumber","USGS",  "RiverStage","Discharge","U1",       "U2",       "U3",       "U4",       "U5",       "U6",       "U7",       "Macro", "MacroSW","Meso",  "MesoSW","Micro", "StartTime","StopTime", "DecimalTimeDifference","StartLatitude","StopLatitude","StartLongitude","StopLongitude","Depth1", "Depth2", "Depth3", "Velocity02or06_1","Velocity02or06_2","Velocity02or06_3","Velocity08_1","Velocity08_2","Velocity08_3","VelocityBot1","VelocityBot2","VelocityBot3","WaterVel","HabitatRN","Cobble", "silt",   "Sand",   "Gravel", "Organic","QC",       "Comments", "MappingBox","Total_Fish_Count","Pallid_Only_Count","CPUE",   "CPU_Area","Alt_Pallid_Only_Count","Alt_CPUE","Alt_CPU_Area","gear.type1","gear.type2")
colClasses  <- c("integer","integer","factor",     "integer","integer", "factor","factor","integer","factor","numeric",      "integer",        "numeric",           "numeric","integer",    "numeric",  "numeric",  "factor", "numeric",  "factor", "numeric",  "numeric",   "integer", "factor","numeric",   "factor","numeric",  "factor","numeric","numeric","numeric","numeric",  "numeric",  "factor",   "factor",   "factor",   "factor",   "numeric","numeric", "factor",  "factor",    "numeric",  "numeric",    "numeric",    "numeric",    "numeric", "integer","integer","integer",   "factor",   "logic",    "logic",      "integer",  "integer",      "character", "character","character","character","numeric","numeric",  "numeric",     "numeric","numeric", "numeric","numeric",     "factor",         "factor","numeric",   "numeric",  "character","character","character","character","character","character","character","factor","factor", "factor","factor","factor","character","character","numeric",              "numeric",      "numeric",     "numeric",       "numeric",      "numeric","numeric","numeric","numeric",         "numeric",         "numeric",         "numeric",     "numeric",     "numeric",     "numeric",     "numeric",     "numeric",     "numeric", "character","numeric","numeric","numeric","numeric","numeric","character","character","character", "integer",         "inetger",          "numeric","numeric", "integer",              "integer", "numeric",     "factor",    "factor")
used.4.plot <- c( FALSE,    TRUE,     FALSE,        FALSE,    FALSE,     TRUE,    TRUE,    TRUE,     FALSE,   TRUE,           FALSE,            FALSE,               FALSE,    FALSE,        TRUE,       TRUE,       FALSE,    FALSE,      FALSE,    TRUE,       TRUE,        TRUE,      FALSE,   TRUE,        FALSE,   TRUE,       TRUE,    TRUE,     TRUE,     TRUE,     TRUE,       TRUE,       TRUE,       TRUE,       TRUE,       TRUE,       TRUE,     TRUE,      TRUE,      TRUE,        TRUE,       TRUE,         TRUE,         TRUE,         TRUE,      TRUE,     FALSE,    FALSE,       FALSE,      FALSE,      FALSE,        FALSE,      FALSE,          FALSE,       FALSE,      FALSE,      FALSE,      TRUE,     FALSE,      FALSE,         FALSE,    TRUE,      FALSE,    FALSE,         FALSE,            FALSE,   FALSE,       FALSE,      FALSE,      FALSE,      FALSE,      FALSE,      FALSE,      FALSE,      FALSE,      TRUE,    FALSE,    TRUE,    FALSE,   TRUE,    FALSE,      FALSE,      FALSE,                  FALSE,          FALSE,         FALSE,           FALSE,          TRUE,     TRUE,     TRUE,     FALSE,             FALSE,             FALSE,             FALSE,         FALSE,         FALSE,         FALSE,         FALSE,         FALSE,         TRUE,      FALSE,      FALSE,    FALSE,    FALSE,    FALSE,    FALSE,    FALSE,      FALSE,      FALSE,       TRUE,              TRUE,               TRUE,     TRUE,      FALSE,                  FALSE,     FALSE,         TRUE,        TRUE)
used.4.anal <- c( FALSE,    TRUE,     FALSE,        FALSE,    FALSE,     TRUE,    TRUE,    TRUE,     FALSE,   TRUE,           FALSE,            FALSE,               FALSE,    TRUE,         TRUE,       TRUE,       FALSE,    TRUE,       FALSE,    TRUE,       TRUE,        TRUE,      FALSE,   TRUE,        FALSE,   TRUE,       TRUE,    TRUE,     TRUE,     TRUE,     TRUE,       TRUE,       TRUE,       TRUE,       TRUE,       TRUE,       TRUE,     TRUE,      TRUE,      TRUE,        TRUE,       TRUE,         TRUE,         TRUE,         TRUE,      TRUE,     FALSE,    FALSE,       FALSE,      FALSE,      FALSE,        FALSE,      FALSE,          FALSE,       FALSE,      FALSE,      FALSE,      TRUE,     TRUE,       FALSE,         FALSE,    TRUE,      FALSE,    TRUE,          FALSE,            FALSE,   FALSE,       FALSE,      FALSE,      FALSE,      FALSE,      FALSE,      FALSE,      FALSE,      FALSE,      TRUE,    FALSE,    TRUE,    FALSE,   TRUE,    FALSE,      FALSE,      FALSE,                  FALSE,          FALSE,         FALSE,           FALSE,          TRUE,     TRUE,     TRUE,     FALSE,             TRUE,              FALSE,             FALSE,         TRUE,          FALSE,         FALSE,         TRUE,          FALSE,         TRUE,      FALSE,      FALSE,    FALSE,    FALSE,    FALSE,    FALSE,    FALSE,      FALSE,      FALSE,       TRUE,              TRUE,               TRUE,     TRUE,      FALSE,                  FALSE,     FALSE,         TRUE,        TRUE)
...
# out[ut the number of data/missing data
...
cor.cef.all   <- cor(myData.subset[,c("Depth1","Depth2","Depth3")])
cor.cef.upper <- cor(myData.subset[myData.subset[,"Reach"] == "Upper",c("Depth1","Depth2","Depth3")])
cor.cef.lower <- cor(myData.subset[myData.subset[,"Reach"] == "Lower",c("Depth1","Depth2","Depth3")])
...
# summary 
# -------------------------------------------------------------------------------------------------
myData.sub <- melt(myData,id.var=c("SY","Reach","Season","Bend","Macro","Meso","Segment","Gear","gear.type1","gear.type2"),measure.var="Pallid_Only_Count")
...
sum.Macro.Count   <- as.data.frame(cast(myData.sub,Macro      ~ value,fun=length,margins = c("grand_row","grand_col")))			# count distribution across Macro [Macro]
...
write.table(sum.Macro.Count,file=FL.Summary,sep=",",row.names=FALSE,col.names=TRUE,append=TRUE)						# write [sum.Macro.Count] out
...
# ........ [Macro] vs [Meso] ......................
cat("\n\nCount distribution in [Macro] and [Meso]\n")
cat("\n\nCount distribution in [Macro] and [Meso]\n",file=FL.Summary,append=TRUE)
sum.Macro.Meso.Count <- cast(myData.sub,Macro ~ value | Meso,fun=length)								# count distribution across Macro [Macro] and Meso [Meso]
for (idx in names(sum.Macro.Meso.Count))
{
	cat(paste("Count distribution in [Macro] and [",idx,"]\n"),file=FL.Summary,append=TRUE)
	write.table(data.frame(sum.Macro.Meso.Count[idx]),file=FL.Summary,sep=",",row.names=FALSE,col.names=TRUE,append=TRUE)		# write [sum.Macro.Meso.Count] out
}
...
cat(paste("count summary has been done for some single variables and some two variable combinations!\n",sep=""))



# -------------------------------------------------------------------------------------------------
# add a presence/absence variable for Pallid Sturgeon Count
# -------------------------------------------------------------------------------------------------
...
x.vars.num <- c("UpDamDist","Ch_W_Full","Ch_W_NoIsl","Braiding","UpTribDist","DTribDist","Mean_z", "Max_z",  "Min_z",  "Grade10RM","GradeBend","MedFlow","MeanFlow","comppct_r","sandtotal_r","claytotal_r","frag3to10_r","VF_width","Segment","Temp",   "Distance","Depth1", "Depth2", "Depth3", "WaterVel")

y.vars.cat <- "binary"
x.vars.cat <- c("Gear",  "Season","Bend",  "Reach", "Rocktype1","Rocktype2","Lithology","NFHAP_Scr","taxorder","taxpartsiz","Segment","Macro", "Meso",  "Micro", "gear.type1","gear.type2")

myData.work <- myData.sup[,c(x.vars.num,x.vars.cat,y.vars.num,y.vars.cat)]


#
# check correlation of continusous variables versus the presence/absence of Pallid Sturgeon
#
...
{
	command.string <- paste("plot.obj1 <- xyplot(",y.vars.num," ~ ",var.num,", data = myData.work,xlab=\"",var.num,"\",ylab=\"",y.vars.num,"\",main=paste(\"",y.vars.num,"\",\" versus \",\"",var.num,"\",sep=\"\"),type=\"p\",pch=16,col=\"red\")",sep="")
	eval(parse(text=command.string))
	# plot(plot.obj1)


	command.string <- paste("plot.obj2 <- bwplot(",var.num," ~ as.factor(",y.vars.num,"), data = myData.work,xlab=\"",y.vars.num,"\",ylab=\"",var.num,"\",main=paste(\"",y.vars.num,"\",\" versus \",\"",var.num,"\",sep=\"\"),col=\"blue\")",sep="")
	eval(parse(text=command.string))
	# plot(plot.obj2)
...
dev.off()
#
# check the correlation of discrete variables versus the presence/absence of Pallid Sturgeon
#
...
for (var.cat in x.vars.cat)
{
	...
	cat(paste(var.cat," versus ",y.vars.cat,",",sep=""),file=FL.Summary,append=TRUE)
