...
Path.Out     <- "C:/YuLong_Projects/FY2012_MORiver/DataAnalysis/Test_JAGS03"
...
FL.Data.OUT <- paste(Path.Out,"Test_JAGS03_data.csv",sep="/")
FL.RESL.OUT <- paste(Path.Out,"Test_JAGS03_results.csv",sep="/")
FL.MODEL    <- paste(Path.Out,"Test_JAGS03_model.Rdata",sep="/")

FL.PostDist.CSV <- paste(Path.Out,"Test_JAGS03_PostDist.csv",sep="/")
FL.PostDist.OBJ <- paste(Path.Out,"Test_JAGS03_PostDist.Rdata",sep="/")


FL.LOG      <- paste(Path.log,"Test_JAGS03.log",sep="/")	
FL.PDF      <- paste(Path.Out,"Test_JAGS03.pdf",sep="/")	
FL.SUM.cat  <- paste(Path.Out,"Test_JAGS03_cat.sum",sep="/")
FL.SUM.num  <- paste(Path.Out,"Test_JAGS03_num.sum",sep="/")
...
# -------------------------------------------------------------------------------------------------
# output the other format used by WinBUGS or BUGS: [Test_JAGS03_data.csv]
# -------------------------------------------------------------------------------------------------
...
#### to:
#### 
#### initsFunction = function() {list( mu=muInit , sigma = sigmaInit 
####                   , y=yInit 
####                   )}
...
jagsModel    <- jags.model("model.txt",data=dataList , inits=initsList ,                         n.chains=nChains , n.adapt=adaptSteps )	# fail if I delete the initialization list
...
#------------------------------------------------------------------------------
 genInitList1 <- function(nPred=nPredictors) {					#
 	list(									#
 		a0 = -0.06, 							#
 		b0 =  1.19,							#
 		a  = c(-0.82,-1.24, 0.65,-0.08,-1.15,-0.82,-1.24, 0.65,-0.08,-1.15,-0.82,-1.24),			#
 		b  = c( 0.21, 0.13,-0.08, 0.02, 0.13, 0.21, 0.13,-0.08, 0.02, 0.13, 0.21, 0.13)				#
 	)									#
 }										#
 genInitList2 <- function(nPred=nPredictors) {					#
 	list(									#
 		a0 =  0.06, 							#
 		b0 =  0.89,							#
 		a  = c(-0.52,-1.00, 0.45, 0.08,-0.15,-0.52,-1.00, 0.45, 0.08,-0.15,-0.52,-1.00),			#
 		b  = c( 0.11,-0.13, 0.08, 1.02, 1.13, 0.11,-0.13, 0.08, 1.02, 1.13, 0.11,-0.13)				#
 	)									#
 }										#
...
cat(paste("5. MCMC has been burning in!\n"))
cat(paste("5. MCMC has been burning in!\n"),file=FL.LOG,append=TRUE)
...
cat(paste("6. MCMC has been updated!\n"))
cat(paste("6. MCMC has been updated!\n"),file=FL.LOG,append=TRUE)
...
cat(paste("\nTest_JAGS03.R is finished successfully at ",End.time,"!\n",sep=" "))
cat(paste("\nTest_JAGS03.R is finished successfully at ",End.time,"!\n",sep=" "),file=FL.LOG,append=TRUE)

cat(paste("\nProcessing time for [Test_JAGS03.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "))
cat(paste("\nProcessing time for [Test_JAGS03.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "),file=FL.LOG,append=TRUE)
