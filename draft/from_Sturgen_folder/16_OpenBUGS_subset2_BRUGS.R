...
FL.LOG    <- paste(Path.log,"16_OpenBUGS_subset2_Brugs.log",sep="/")	
...
for (idx.subset in c(2))
{
	...
	if(idx.subset == 2)	# PSCL Lower TLC
	{
		...
		for (model4test in seq(from=1,to=1,by=1))
		{
			if (model4test == 1)
			{
				list.var <- c("GIS_RM","VF_wid","T_dist_near","Rel_dsch_AY","Stk_60_cnt")	
				B <- c(-1.065,-0.481,0.473,0.282,0.403,0.233) 
				A <- c(0.031,0.184,1.249,-0.129,0.366,0.469) 
			}
			...
			myData.Tmp     <- cbind(myData.Y,myData.X,SUPPORT.calc)	# note we do have support for logistic regression
		  names(myData.Tmp)    <- c("Pal_cnt",names(myData.X),"support")	# note we do have support for logistic regression
...
"model{
	...
	for (j in 1:",no.var,"){b[j]  ~ dnorm(0,1.0E-6)}
			...
			# INIT
			# ------------------------------------------------------------------------------------------------
			modelCheck.flag <- modelCheck(file = FL.MODEL )
			cat(paste("5. Model has been checked for [",list.var,"]!\n"))
			cat(paste("5. Model has been checked for [",list.var,"]!\n"),file=FL.LOG,append=TRUE)
			...
			modelData.flag <- modelData( bugsData( dataList ) )
			...
			#------------------------------------------------------------------------------
			nChains = 3
			modelCompile.flag <- modelCompile( numChains = nChains )
			...
			#------------------------------------------------------------------------------
			BurnInSteps = 50000
			modelBurn.flag <- modelUpdate( BurnInSteps )
			...
			#------------------------------------------------------------------------------
			samplesSet( c("a0","a","b0","b") )
			nPerChain = 50000
			thinStep = 100
			modelUpdate.flag <- modelUpdate( nPerChain , thin=thinStep )
...
cat(paste("\n16_OpenBUGS_subset2_Brugs.R is finished successfully at ",End.time,"!\n",sep=" "))
cat(paste("\n16_OpenBUGS_subset2_Brugs.R is finished successfully at ",End.time,"!\n",sep=" "),file=FL.LOG,append=TRUE)

cat(paste("\nProcessing time for [16_OpenBUGS_subset2_Brugs.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "))
cat(paste("\nProcessing time for [16_OpenBUGS_subset2_Brugs.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "),file=FL.LOG,append=TRUE)
