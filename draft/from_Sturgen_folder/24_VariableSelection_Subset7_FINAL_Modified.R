#
# 24_VariableSelection_Subset7_final_Modified.R
#
# August 27, 2013
#
...
FL.LOG    <- paste(Path.log,"24_VariableSelection_Subset7_final_Modified.log",sep="/")	
...
FL.RESL.OUT <- paste(Path.OUT,"24_VariableSelection_Subset7_final_Modified.csv",sep="/")
...
for (idx.subset in c(7))
{
	...
	if(idx.subset == 7)	# Logistic Regression for Lower River
	{
		...
		# ---------------------------------------------------------------------------------
		# [Subset 7 Logistic Regression on Lower River]: 2. model with 2 variables: "gear.type1" is best single
		# ---------------------------------------------------------------------------------
		no.tmp  <- length(c(X.var.num,X.var.cat))
		idx.tmp <- 0
		for (X.var.rm in grep("gear.type1",c(X.var.num,X.var.cat),value=TRUE,invert=TRUE))		# all variables other than "gear.type1"
		{
			idx.tmp <- idx.tmp + 1
			idx.var.rm <- match(X.var.rm,names(myData.Work))				# the index of the variable to be removed in the data frame
			if(!(is.na(idx.var.rm)))
			{
				model.count  <- model.count + 1
				myData.Tmp   <- myData.Work[,c("binary","gear.type1",X.var.rm)]	# two variable combinations: "gear.type1" and the other
				model.Tmp    <- glm(binary ~ .,data=myData.Tmp,family=binomial("logit"))
				model.ID     <- X.var.rm
				model.string <- paste("Model",model.count," with ",X.var.rm," & gear.type1\n",sep=" ")
				model.no.var <- dim(myData.Tmp)[2] - 1
				model.rm.var <- paste("all but gear.type1 & ",X.var.rm,sep=" ")
		...
		# ---------------------------------------------------------------------------------
		# [Subset 7 Logistic Regression on Lower River]: 3. model with 3 variables: "gear.type1" is best single
		# ---------------------------------------------------------------------------------
		no.tmp  <- length(c(X.var.num,X.var.cat))
		idx.tmp <- 0
		for (X.var.rm in grep("Seg",grep("gear.type1",c(X.var.num,X.var.cat),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE))		# all variables other than "gear.type1"
		{
			idx.tmp <- idx.tmp + 1
			idx.var.rm <- match(X.var.rm,names(myData.Work))				# the index of the variable to be removed in the data frame
			if(!(is.na(idx.var.rm)))
			{
				model.count  <- model.count + 1
				myData.Tmp   <- myData.Work[,c("binary","gear.type1","Seg",X.var.rm)]	# two variable combinations: "gear.type1" and the other
				model.Tmp    <- glm(binary ~ .,data=myData.Tmp,family=binomial("logit"))
				model.ID     <- X.var.rm
				model.string <- paste("Model",model.count," with ",X.var.rm," & gear.type1/Seg\n",sep=" ")
				model.no.var <- dim(myData.Tmp)[2] - 1
				model.rm.var <- paste("all but gear.type1/Seg & ",X.var.rm,sep=" ")
		...
		# ---------------------------------------------------------------------------------
		# [Subset 7 Logistic Regression on Lower River]: 4. model with 4 variables: "gear.type1" is best single
		# ---------------------------------------------------------------------------------
		no.tmp  <- length(c(X.var.num,X.var.cat))
		idx.tmp <- 0
		for (X.var.rm in grep("SY",grep("Seg",grep("gear.type1",c(X.var.num,X.var.cat),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE))		# all variables other than "gear.type1"
		{
			idx.tmp <- idx.tmp + 1
			idx.var.rm <- match(X.var.rm,names(myData.Work))				# the index of the variable to be removed in the data frame
			if(!(is.na(idx.var.rm)))
			{
				model.count  <- model.count + 1
				myData.Tmp   <- myData.Work[,c("binary","gear.type1","Seg","SY",X.var.rm)]	# two variable combinations: "gear.type1" and the other
				model.Tmp    <- glm(binary ~ .,data=myData.Tmp,family=binomial("logit"))
				model.ID     <- X.var.rm
				model.string <- paste("Model",model.count," with ",X.var.rm," & gear.type1/Seg/SY\n",sep=" ")
				model.no.var <- dim(myData.Tmp)[2] - 1
				model.rm.var <- paste("all but gear.type1/Seg/SY & ",X.var.rm,sep=" ")
		...
		# ---------------------------------------------------------------------------------
		# [Subset 7 Logistic Regression on Lower River]: 5. model with 5 variables: "gear.type1" is best single
		# ---------------------------------------------------------------------------------
		no.tmp  <- length(c(X.var.num,X.var.cat))
		idx.tmp <- 0
		for (X.var.rm in grep("GIS_RM",grep("SY",grep("Seg",grep("gear.type1",c(X.var.num,X.var.cat),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE))		# all variables other than "gear.type1"
		{
			idx.tmp <- idx.tmp + 1
			idx.var.rm <- match(X.var.rm,names(myData.Work))				# the index of the variable to be removed in the data frame
			if(!(is.na(idx.var.rm)))
			{
				model.count  <- model.count + 1
				myData.Tmp   <- myData.Work[,c("binary","gear.type1","Seg","SY","GIS_RM",X.var.rm)]	# two variable combinations: "gear.type1" and the other
				model.Tmp    <- glm(binary ~ .,data=myData.Tmp,family=binomial("logit"))
				model.ID     <- X.var.rm
				model.string <- paste("Model",model.count," with ",X.var.rm," & gear.type1/Seg/SY/GIS_RM\n",sep=" ")
				model.no.var <- dim(myData.Tmp)[2] - 1
				model.rm.var <- paste("all but gear.type1/Seg/SY/GIS_RM & ",X.var.rm,sep=" ")
		...
		# ---------------------------------------------------------------------------------
		# [Subset 7 Logistic Regression on Lower River]: 6. model with 6 variables: "gear.type1" is best single
		# ---------------------------------------------------------------------------------
		no.tmp  <- length(c(X.var.num,X.var.cat))
		idx.tmp <- 0
		for (X.var.rm in grep("new_meso",grep("GIS_RM",grep("SY",grep("Seg",grep("gear.type1",c(X.var.num,X.var.cat),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE))		# all variables other than "gear.type1"
		{
			idx.tmp <- idx.tmp + 1
			idx.var.rm <- match(X.var.rm,names(myData.Work))				# the index of the variable to be removed in the data frame
			if(!(is.na(idx.var.rm)))
			{
				model.count  <- model.count + 1
				myData.Tmp   <- myData.Work[,c("binary","gear.type1","Seg","SY","GIS_RM","new_meso",X.var.rm)]	# two variable combinations: "gear.type1" and the other
				model.Tmp    <- glm(binary ~ .,data=myData.Tmp,family=binomial("logit"))
				model.ID     <- X.var.rm
				model.string <- paste("Model",model.count," with ",X.var.rm," & gear.type1/Seg/SY/GIS_RM/new_meso\n",sep=" ")
				model.no.var <- dim(myData.Tmp)[2] - 1
				model.rm.var <- paste("all but gear.type1/Seg/SY/GIS_RM/new_meso & ",X.var.rm,sep=" ")
		...
		# ---------------------------------------------------------------------------------
		# [Subset 7 Logistic Regression on Lower River]: 7. model with 7 variables: "gear.type1" is best single
		# ---------------------------------------------------------------------------------
		no.tmp  <- length(c(X.var.num,X.var.cat))
		idx.tmp <- 0
		for (X.var.rm in grep("Stk_24_cnt",grep("new_meso",grep("GIS_RM",grep("SY",grep("Seg",grep("gear.type1",c(X.var.num,X.var.cat),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE))		# all variables other than "gear.type1"
		{
			idx.tmp <- idx.tmp + 1
			idx.var.rm <- match(X.var.rm,names(myData.Work))				# the index of the variable to be removed in the data frame
			if(!(is.na(idx.var.rm)))
			{
				model.count  <- model.count + 1
				myData.Tmp   <- myData.Work[,c("binary","gear.type1","Seg","SY","GIS_RM","new_meso","Stk_24_cnt",X.var.rm)]	# two variable combinations: "gear.type1" and the other
				model.Tmp    <- glm(binary ~ .,data=myData.Tmp,family=binomial("logit"))
				model.ID     <- X.var.rm
				model.string <- paste("Model",model.count," with ",X.var.rm," & gear.type1/Seg/SY/GIS_RM/new_meso/Stk_24_cnt\n",sep=" ")
				model.no.var <- dim(myData.Tmp)[2] - 1
				model.rm.var <- paste("all but gear.type1/Seg/SY/GIS_RM/new_meso/Stk_24_cnt & ",X.var.rm,sep=" ")
		...
		# ---------------------------------------------------------------------------------
		# [Subset 7 Logistic Regression on Lower River]: 8. model with 8 variables: "gear.type1" is best single
		# ---------------------------------------------------------------------------------
		no.tmp  <- length(c(X.var.num,X.var.cat))
		idx.tmp <- 0
		for (X.var.rm in grep("VF_wid",grep("Stk_24_cnt",grep("new_meso",grep("GIS_RM",grep("SY",grep("Seg",grep("gear.type1",c(X.var.num,X.var.cat),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE))		# all variables other than "gear.type1"
		{
			idx.tmp <- idx.tmp + 1
			idx.var.rm <- match(X.var.rm,names(myData.Work))				# the index of the variable to be removed in the data frame
			if(!(is.na(idx.var.rm)))
			{
				model.count  <- model.count + 1
				myData.Tmp   <- myData.Work[,c("binary","gear.type1","Seg","SY","GIS_RM","new_meso","Stk_24_cnt","VF_wid",X.var.rm)]	# two variable combinations: "gear.type1" and the other
				model.Tmp    <- glm(binary ~ .,data=myData.Tmp,family=binomial("logit"))
				model.ID     <- X.var.rm
				model.string <- paste("Model",model.count," with ",X.var.rm," & gear.type1/Seg/SY/GIS_RM/new_meso/Stk_24_cnt/VF_wid\n",sep=" ")
				model.no.var <- dim(myData.Tmp)[2] - 1
				model.rm.var <- paste("all but gear.type1/Seg/SY/GIS_RM/new_meso/Stk_24_cnt/VF_wid & ",X.var.rm,sep=" ")
...
cat(paste("\n24_VariableSelection_Subset7_final_Modified.R is finished successfully at ",End.time,"!\n",sep=" "))
cat(paste("\n24_VariableSelection_Subset7_final_Modified.R is finished successfully at ",End.time,"!\n",sep=" "),file=FL.LOG,append=TRUE)

cat(paste("\nProcessing time for [24_VariableSelection_Subset7_final_Modified.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "))
cat(paste("\nProcessing time for [24_VariableSelection_Subset7_final_Modified.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "),file=FL.LOG,append=TRUE)
