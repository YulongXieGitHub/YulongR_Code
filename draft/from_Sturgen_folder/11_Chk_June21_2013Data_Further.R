#
# 11_Chk_June21_2013Data_Further.R
#
# June 24, 2013
#
...
Path.Out     <- "C:/YuLong_Projects/FY2012_MORiver/DataAnalysis/11_Chk_June21_2013Data_Further"
...
FL.LOG       <- paste(Path.log,"11_Chk_June21_2013Data_Further.log",               sep="/")	
FL.Data.OUT  <- paste(Path.Out,"MoRiver_Variables_2013May31_revisedOn_2013June21_YLX_app.csv",sep="/")
FL.SUM.cat   <- paste(Path.Out,"11_Chk_June21_2013Data_Further_categoricalSum.csv",sep="/")
FL.SUM.num   <- paste(Path.Out,"11_Chk_June21_2013Data_Further_numericSum.csv",    sep="/")
FL.Summary   <- paste(Path.Out,"11_Chk_June21_2013Data_Further_summary.csv",       sep="/")
FL.missing   <- paste(Path.Out,"11_Chk_June21_2013Data_Further_missing.csv",       sep="/")
...
#
# check the observations for each site
#
sum.overall <- 0
for (seg.idx in sort(unique(myData.raw[,"Seg"])))
{
	for (bend.idx in sort(unique(myData.raw[,"Bend"])))
	{
		index <- myData.raw[,"Seg"] == seg.idx & myData.raw[,"Bend"] == bend.idx
		if (sum(index) > 0)
		{
			myData.this <- myData.raw[index,]
			myData.dist <- as.data.frame(cast(SY~Season~Gear,data=myData.this[,c("SY","Season","Gear","Pal_cnt")],fun=length))
			no.sample <- sum(myData.dist)
			sum.overall <- sum.overall +no.sample
			cat(paste("Seg",seg.idx,"-","Bend",bend.idx,"(",no.sample,"),",sep=""),file=FL.Data.OUT,append=TRUE)
			write.table(myData.dist,file=FL.Data.OUT,row.names=TRUE,col.names=TRUE,sep=",",append=TRUE)
			cat("\n",file=FL.Data.OUT,append=TRUE)
		}
	}
}
if (sum.overall != dim(myData.raw)[1])
{
	cat("Something wrong!\n")
	die;
}else{
...
# FL_Total_cnt_vs_Pal_cnt.sum <- paste(Path.Out,"11_Chk_June21_2013Data_Further_Gear_CPUE_CPUA.csv",sep="/")	
#
# Total Fish Catch vs Pallid Fish Catch
#
FL_Total_cnt_vs_Pal_cnt.pdf <- paste(Path.Out,"11_Chk_June21_2013Data_Further_Tot_Cnt_vs_Pal_Cnt.pdf",sep="/")	
...
# FL_CPUE_CPUA.sum <- paste(Path.Out,"11_Chk_June21_2013Data_Further_Gear_CPUE_CPUA.csv",sep="/")	
# FL_CPUE_CPUA.pdf <- paste(Path.Out,"11_Chk_June21_2013Data_Further_Gear_CPUE_CPUA.pdf",sep="/")	
# *********************************************************************************************************************
#
...
# -------------------------------------------------------------------------------------------------
# open pdf file for outputting plots
FL.stock.PDF <- paste(Path.Out,"11_Chk_June21_2013Data_Further_stock_var.pdf",sep="/")	
...
# -------------------------------------------------------------------------------------------------
# open pdf file for outputting plots
FL.microHabitat <- paste(Path.Out,"11_Chk_June21_2013Data_Further_microHabitat.pdf",sep="/")	
...
# -------------------------------------------------------------------------------------------------
FL.sandClay <- paste(Path.Out,"11_Chk_June21_2013Data_Further_sandClay.pdf",sep="/")	
...
# -------------------------------------------------------------------------------------------------
# 2a. check each individual variable: [11_Chk_June21_2013Data_Further_indVar.pdf] and [11_Chk_June21_2013Data_Further_num.sum]
# -------------------------------------------------------------------------------------------------
# open pdf file for outputting plots
FL.indVar.PDF <- paste(Path.Out,"11_Chk_June21_2013Data_Further_indVar.pdf",sep="/")	
...
# -------------------------------------------------------------------------------------------------
# 2b. check the correlation between small group of variables: [11_Chk_June21_2013Data_Further_indVar.pdf]
# -------------------------------------------------------------------------------------------------
...
# -------------------------------------------------------------------------------------------------
# 3. check correlation between numeric covariates: [11_Chk_June21_2013Data_Further_Correlation.pdf]
# -------------------------------------------------------------------------------------------------
FL.Correlation.PDF  <- paste(Path.Out,"11_Chk_June21_2013Data_Further_Correlation.pdf",sep="/")	
...
# -------------------------------------------------------------------------------------------------
# 4. count summary: FL.Summary == [11_Chk_June21_2013Data_Further_summary.csv]
# -------------------------------------------------------------------------------------------------
...
# -------------------------------------------------------------------------------------------------
# 6. check correlation of continusous variables vs the [count] of Pallid Sturgeon: [11_Chk_June21_2013Data_Further_Count_numVar.pdf]
# -------------------------------------------------------------------------------------------------
FL.Count_num.PDF  <- paste(Path.Out,"11_Chk_June21_2013Data_Further_Count_numVar.pdf",sep="/")	
...
# -------------------------------------------------------------------------------------------------
# 7. check the correlation of discrete variables vs the [count] of Pallid Sturgeon: FL.Summary == [11_Chk_June21_2013Data_Further_summary.csv]
# -------------------------------------------------------------------------------------------------
# FL.Count_cat.PDF  <- paste(Path.Out,"11_Chk_June21_2013Data_Further_Count_catVar.pdf",sep="/")	
...
# Use [melt] and [cast] to do this
myData.tmp <- melt(myData,id.var=c("Gear", "Season","Bend","SY","Lith_1","Lith_2","Lith_desc","NFHAP","Tax_ord","Tax_part_sz","Seg","gear.type1","gear.type2","macro.type","MicroClass","MA"),measure.var="binary")

# gear - Reach
sum.tmp1 <- as.data.frame(cast(myData.tmp,Gear ~ value,fun=length,margins = c("grand_row","grand_col")))
cat("\nCount distribution in Gears\n",file="tbd.csv",append=TRUE)
write.table(sum.tmp1,file="tbd.csv",sep=",",row.names=FALSE,col.names=TRUE,append=TRUE)						

sum.tmp2 <- cast(myData.tmp,Gear ~ value | MA,fun=length)								
for (idx in names(sum.tmp2))
{
	cat(paste("\nCount distribution in terms of [Gear] and [",idx,"]\n"),file="tbd.csv",append=TRUE)
	write.table(data.frame(sum.tmp2[idx]),file="tbd.csv",sep=",",row.names=FALSE,col.names=TRUE,append=TRUE)		
}


# gear.type1 - Reach
sum.tmp1 <- as.data.frame(cast(myData.tmp,gear.type1 ~ value,fun=length,margins = c("grand_row","grand_col")))
cat("\nCount distribution in gear.type1s\n",file="tbd.csv",append=TRUE)
write.table(sum.tmp1,file="tbd.csv",sep=",",row.names=FALSE,col.names=TRUE,append=TRUE)						

sum.tmp2 <- cast(myData.tmp,gear.type1 ~ value | MA,fun=length)								
for (idx in names(sum.tmp2))
{
	cat(paste("\nCount distribution in terms of [gear.type1] and [",idx,"]\n"),file="tbd.csv",append=TRUE)
	write.table(data.frame(sum.tmp2[idx]),file="tbd.csv",sep=",",row.names=FALSE,col.names=TRUE,append=TRUE)		
}


# gear.type2 - Reach
sum.tmp1 <- as.data.frame(cast(myData.tmp,gear.type2 ~ value,fun=length,margins = c("grand_row","grand_col")))
cat("\nCount distribution in gear.type2s\n",file="tbd.csv",append=TRUE)
write.table(sum.tmp1,file="tbd.csv",sep=",",row.names=FALSE,col.names=TRUE,append=TRUE)						

sum.tmp2 <- cast(myData.tmp,gear.type2 ~ value | MA,fun=length)								
for (idx in names(sum.tmp2))
{
	cat(paste("\nCount distribution in terms of [gear.type2] and [",idx,"]\n"),file="tbd.csv",append=TRUE)
	write.table(data.frame(sum.tmp2[idx]),file="tbd.csv",sep=",",row.names=FALSE,col.names=TRUE,append=TRUE)		
}


# macro.type - Reach
sum.tmp1 <- as.data.frame(cast(myData.tmp,macro.type ~ value,fun=length,margins = c("grand_row","grand_col")))
cat("\nCount distribution in macro.types\n",file="tbd.csv",append=TRUE)
write.table(sum.tmp1,file="tbd.csv",sep=",",row.names=FALSE,col.names=TRUE,append=TRUE)						

sum.tmp2 <- cast(myData.tmp,macro.type ~ value | MA,fun=length)								
for (idx in names(sum.tmp2))
{
	cat(paste("\nCount distribution in terms of [macro.type] and [",idx,"]\n"),file="tbd.csv",append=TRUE)
	write.table(data.frame(sum.tmp2[idx]),file="tbd.csv",sep=",",row.names=FALSE,col.names=TRUE,append=TRUE)		
}
...
# -------------------------------------------------------------------------------------------------
# 8. check correlation of continusous variables vs the [count] of Pallid Sturgeon: 11_Chk_June21_2013Data_Further_Binary_numVar.pdf; 	FL.Summary == [11_Chk_June21_2013Data_Further_summary.csv]
# -------------------------------------------------------------------------------------------------
FL.binary_num.PDF  <- paste(Path.Out,"11_Chk_June21_2013Data_Further_Binary_numVar.pdf",sep="/")	
...
# -------------------------------------------------------------------------------------------------
# FL.binary_cat.PDF  <- paste(Path.Out,"11_Chk_June21_2013Data_Further_Binary_catVar.pdf",sep="/")	
...
# 
FL.binary_cat.csv <- paste(Path.Out,"11_Chk_June21_2013Data_Further_Binary_catVar.csv",sep="/")
