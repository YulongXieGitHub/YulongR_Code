...
# eliminate all stuff
#
# myData.sub <- melt(myData.reduced,id.var=grep("[^Pal_cnt]",names(myData.reduced),value=TRUE),measure.var="Pal_cnt")
# cast(myData.sub,Tax_part_sz ~ value | rock,fun=length)
# 
# ------------------------------------------------------------
# June 4, 2013:
# The 7 series test lead the following observations:
# 1. the number of groups of the categorical variables cannot be too large: Lith_1 --> rock, "Tax_part_sz" --> "part"
# 2. re-name the categorical variables when subsetting them because some categories may drop but still remain in the level list.
# 3. variable with too many missing may not be able to be included such as "WaterVel".
# 
...
# Path.Out     <- "C:/YuLong_Projects/FY2012_MORiver/DataAnalysis/7i_prepare_TLC_data"
...
# 
# FL.LOG       <- paste(Path.log,"7i_prepare_TLC_data.log",sep="/")	
# FL.SUM.cat   <- paste(Path.Out,"7i_prepare_TLC_data_categoricalSum.csv",sep="/")
# FL.SUM.num   <- paste(Path.Out,"7i_prepare_TLC_data_numericSum.csv",sep="/")
# FL.Summary   <- paste(Path.Out,"7i_prepare_TLC_data_summary.csv",sep="/")
# FL.missing   <- paste(Path.Out,"7i_prepare_TLC_data_missing.csv",sep="/")
...
FL.RESL.OUT <- paste(Path.Out,"7i_prepare_TLC_data.csv",sep="/")
FL.LOG      <- paste(Path.log,"7i_prepare_TLC_data.log",sep="/")	
FL.PDF      <- paste(Path.Out,"7i_prepare_TLC_data.pdf",sep="/")	
FL.SUM.cat  <- paste(Path.Out,"7i_prepare_TLC_data_cat.sum",sep="/")
FL.SUM.num  <- paste(Path.Out,"7i_prepare_TLC_data_num.sum",sep="/")
...
myData.reduced <- myData.raw[Lower.TLC,]

# -------------------------------------------------------------------------------------------------
# Reassign "Lith_1" field
# -------------------------------------------------------------------------------------------------
myData.reduced[,"rock"] <- rep("shale",dim(myData.reduced)[1])
myData.reduced[myData.reduced[,"Lith_1"] == "clay or mud","rock"] <- "clay"

# -------------------------------------------------------------------------------------------------
# Reassign "Tax_part_sz" field: Cannot have "WaterVel" in the model due to the number of missing values.
# -------------------------------------------------------------------------------------------------
myData.reduced[,"part"] <- rep("clayey",dim(myData.reduced)[1])
myData.reduced[myData.reduced[,"Tax_part_sz"] == "coarse-silty" | myData.reduced[,"Tax_part_sz"] == "coarse-silty over clayey" ,"part"] <- "coarse"
...
# var.X.cat <- c("Lith_1","Tax_part_sz","SY","Seg","macro.type")
var.X.cat <- c("SY","part","rock","macro.type","MicroClass","Seg")
var.Y     <- c("Pal_cnt")

myData.X.num <- myData.reduced[,var.X.num,drop=FALSE]
myData.X.cat <- myData.reduced[,var.X.cat,drop=FALSE] 
myData.Y     <- myData.reduced[,var.Y,drop=FALSE] 
...
# ---------------------------------------------------------
# delete NA data before modeling
# ---------------------------------------------------------
A <- apply(myData.TLC.lower,1,function(x){sum(is.na(x))})
idx.NA <- seq(1,dim(myData.TLC.lower)[1])[A>0]
myData.TLC.lower <- myData.TLC.lower[-idx.NA,]
idx.lines <- dim(myData.TLC.lower)[1] + 2
...
par(mfrow = c(2,2))
 hist(estimation.Poisson[3:idx.lines,"Pal_cnt"],nclass=100,xlab="Pal_cnt",ylab="frequency",main="[fm_pois]: distribution of observed count")
 hist(estimation.Poisson[3:idx.lines,"mu"], nclass=100,xlab="mu", ylab="frequency",main="[fm_pois]: distribution of expected count")

 plot(estimation.Poisson[3:idx.lines,"Pal_cnt"],estimation.Poisson[3:idx.lines,"mu"],type="p",pch=16,cex=0.5,xlab="Pal_cnt count",ylab="expected",main="[fm_Poisson]: model fitting")
...
par(mfrow = c(2,2))
 hist(estimation.QuasiPoisson[3:idx.lines,"Pal_cnt"],nclass=100,xlab="Pal_cnt",ylab="frequency",main="[fm_qpois]: distribution of observed count")
 hist(estimation.QuasiPoisson[3:idx.lines,"mu"], nclass=100,xlab="mu", ylab="frequency",main="[fm_qpois]: distribution of expected count")

 plot(estimation.QuasiPoisson[3:idx.lines,"Pal_cnt"],estimation.QuasiPoisson[3:idx.lines,"mu"],type="p",pch=16,cex=0.5,xlab="Pal_cnt count",ylab="expected",main="[fm_QuasiPoisson]: model fitting")
 abline(a=0,b=1,col="red")
...
par(mfrow = c(2,2))
 hist(estimation.nbin[3:idx.lines,"Pal_cnt"],nclass=100,xlab="Pal_cnt",ylab="frequency",main="[fm_nbin]: distribution of observed count")
 hist(estimation.nbin[3:idx.lines,"mu"], nclass=100,xlab="mu", ylab="frequency",main="[fm_nbin]: distribution of expected count")

 plot(estimation.nbin[3:idx.lines,"Pal_cnt"],estimation.nbin[3:idx.lines,"mu"],type="p",pch=16,cex=0.5,xlab="Pal_cnt count",ylab="expected",main="[fm_nbin]: model fitting")
 abline(a=0,b=1,col="red")
...
par(mfrow = c(2,2))
 hist(estimation.zinb0[3:idx.lines,"Pal_cnt"],nclass=100,xlab="Pal_cnt",ylab="frequency",main="[fm_zinb0]: distribution of observed count")
 hist(estimation.zinb0[3:idx.lines,"mu"], nclass=100,xlab="mu", ylab="frequency",main="[fm_zinb0]: distribution of expected count")
                    
 hist(estimation.zinb0[3:idx.lines,"est.zero.infl"],nclass=100,xlab="p.0infl",ylab="frequency",main="[fm_zinb0]:distribution of probability of inflated zero")
 plot(estimation.zinb0[3:idx.lines,"Pal_cnt"],estimation.zinb0[3:idx.lines,"mu"],type="p",pch=16,cex=0.5,xlab="Pal_cnt count",ylab="expected",main="[fm_zinb0]:model fitting (Zero-Inflated)")
 abline(a=0,b=1,col="red")
