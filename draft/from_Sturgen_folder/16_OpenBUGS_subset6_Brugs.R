...
FL.LOG    <- paste(Path.log,"16_OpenBUGS_subset6_Brugs.log",sep="/")	
...
for (idx.subset in c(6))
{
	...
	if(idx.subset == 6)	# PSCL Lower TLC
	{
		...
		for (model4test in seq(from=1,to=1,by=1))
		{
			if (model4test == 1)
			{
				list.var <- c("Stk_120_cnt","Rel_dsch_AY","macro.type","Season","Seg","SY","T_dist_up","Rel_dsch_WY")	
				A <- c(-2.681149672,0.11714891,0.014328101,-0.915131403,-0.928044306,-1.313893259,-0.719907529,-0.39233723,0.640979438,0.854522504,0.443732367,0.644356545,0.741137146,1.091427164,0.517374243,0.113161405,-0.274624853) 
			}
			...
			FL.OBJ     <- paste(Path.OUT.var,paste("Model_",idx.subset,"_",subset.section,"_",subset.gear,"_",method.string,".RData",sep=""),sep="/")
...
cat(paste("\n16_OpenBUGS_subset6_Brugs.R is finished successfully at ",End.time,"!\n",sep=" "))
cat(paste("\n16_OpenBUGS_subset6_Brugs.R is finished successfully at ",End.time,"!\n",sep=" "),file=FL.LOG,append=TRUE)

cat(paste("\nProcessing time for [16_OpenBUGS_subset6_Brugs.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "))
cat(paste("\nProcessing time for [16_OpenBUGS_subset6_Brugs.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "),file=FL.LOG,append=TRUE)
