#
# 25_GenerateData_for_OpenBugs_subset1_Final.R
#
...
for (idx.subset in c(1))
{
	...
	if(idx.subset == 1)	# PSCL Lower TLC
	{
		...
		# ---------------------------------------------------------------------------------
		for (model4test in seq(from=1,to=2,by=1))
		{
			if (model4test == 1)
			{
				list.var <- c("SY","macro.type","new_meso","Wet_wid","new_NFHAP","T_dist_dn","Stk_24_cnt","Grade_10_RM","Seg","D_dist_near","VF_wid","Depth_Mean","Clay_per","GIS_RM","Temp","Rel_dsch_WY","Braid")	
				B <- c(-107.653,47.333,29.758,31.551,38.394,24.066,-22.589,-11.453,-10.720,-21.874,59.388,2.501,-4.234,-6.378,-6.146,7.358,-3.359,-31.522,0.769,16.871,-2.530,0.877,3.444,-7.663,-2.608,4.516,1.883) 
				A <- c(-1.861,3.252,1.804,1.449,1.709,2.591,1.031,1.610,1.828,-0.360,2.985,0.183,0.012,-0.412,-0.327,0.166,-0.366,-1.870,-0.253,0.999,-0.361,0.087,0.142,0.344,-0.187,0.644,0.145) 
			}else if (model4test == 2)
			{
				list.var <- c("SY","macro.type","new_meso","Wet_wid","new_NFHAP","T_dist_dn")	
				B <- c(-94.56669908,21.85373611,21.83431181,20.36679112,28.8318737,19.85378739,-33.84642369,-26.04781887,-19.9321167,-26.66146298,53.95406911,2.48932462,-10.18132984,-15.06012725,-2.182238972) 
				A <- c(-2.149241363,1.821307683,1.805473401,0.926363624,1.273970417,2.435543317,1.135916576,1.979463504,2.595913219,0.838608107,2.220859743,0.049566957,-0.936749373,-1.210919498,-0.460657269) 			
			}
