#
# 5_prepare_TLC_data.R
#
# Data updated on May 14, 2013
...
# Path.Out     <- "C:/YuLong_Projects/FY2012_MORiver/DataAnalysis/5_prepare_TLC_data"
...
# 
# FL.LOG       <- paste(Path.log,"5_prepare_TLC_data.log",sep="/")	
# FL.SUM.cat   <- paste(Path.Out,"5_prepare_TLC_data_categoricalSum.csv",sep="/")
# FL.SUM.num   <- paste(Path.Out,"5_prepare_TLC_data_numericSum.csv",sep="/")
# FL.Summary   <- paste(Path.Out,"5_prepare_TLC_data_summary.csv",sep="/")
# FL.missing   <- paste(Path.Out,"5_prepare_TLC_data_missing.csv",sep="/")
...
FL.RESL.OUT <- paste(Path.Out,"5_prepare_TLC_data.csv",sep="/")

FL.LOG      <- paste(Path.log,"5_prepare_TLC_data.log",sep="/")	
FL.PDF      <- paste(Path.Out,"5_prepare_TLC_data.pdf",sep="/")	
FL.SUM.cat  <- paste(Path.Out,"5_prepare_TLC_data_cat.sum",sep="/")
FL.SUM.num  <- paste(Path.Out,"5_prepare_TLC_data_num.sum",sep="/")
...
used.4.anal <- c( FALSE,    TRUE,     FALSE,   FALSE,    FALSE,     FALSE,   TRUE,    FALSE,    FALSE,   FALSE,          FALSE,            FALSE,               FALSE,    FALSE,        FALSE,      TRUE,       FALSE,    TRUE,       FALSE,    TRUE,       FALSE,       FALSE,     FALSE,   TRUE,        FALSE,   TRUE,       FALSE,   FALSE,    FALSE,    FALSE,    TRUE,       FALSE,      FALSE,      FALSE,      FALSE,      FALSE,      FALSE,    FALSE,     FALSE,     FALSE,       TRUE,       TRUE,         TRUE,         FALSE,        TRUE,      TRUE,     FALSE,    FALSE,       FALSE,      FALSE,      FALSE,        FALSE,      FALSE,          FALSE,       FALSE,      FALSE,      FALSE,      TRUE,     FALSE,      FALSE,         FALSE,    FALSE,     FALSE,    FALSE,         FALSE,            FALSE,   FALSE,       FALSE,      FALSE,      FALSE,      FALSE,      FALSE,      FALSE,      FALSE,      FALSE,      FALSE,   FALSE,    FALSE,   FALSE,   FALSE,   TRUE,        FALSE,      FALSE,      FALSE,                  FALSE,          FALSE,         FALSE,           FALSE,          FALSE,    FALSE,    FALSE,    FALSE,             FALSE,             FALSE,             FALSE,         FALSE,         FALSE,         FALSE,         FALSE,         FALSE,         TRUE,      FALSE,      FALSE,    FALSE,    FALSE,    FALSE,    FALSE,    FALSE,      FALSE,      FALSE,       FALSE,             TRUE,               FALSE,    FALSE,     FALSE,                  FALSE,     FALSE,         FALSE,          TRUE,           FALSE,          TRUE,           FALSE,          TRUE,           FALSE,           TRUE,            FALSE,            TRUE,             FALSE,       FALSE,       TRUE)      
...
myData.work <-  myData.TLC.lower
 
myData.reduced <- myData.work[complete.cases(myData.work),]
...
fm_pois <- glm(Pallid_Only_Count ~ ., data = myData.reduced, family = poisson)
...
est.pois   <- cbind(Pallid_Only_Count        = myData.work[,"Pallid_Only_Count"],
                    eta        = est.pred,
                    mu         = est.mu,
                    lambda     = est.lambda,
...
par(mfrow = c(2,2))
 hist(estimation.Poisson[3:1984,"Pallid_Only_Count"],nclass=100,xlab="Pallid_Only_Count",ylab="frequency",main="[fm_pois]: distribution of observed count")
 hist(estimation.Poisson[3:1984,"mu"], nclass=100,xlab="mu", ylab="frequency",main="[fm_pois]: distribution of expected count")

 plot(estimation.Poisson[3:1984,"Pallid_Only_Count"],estimation.Poisson[3:1984,"mu"],type="p",pch=16,cex=0.5,xlab="Pallid_Only_Count count",ylab="expected",main="[fm_Poisson]: model fitting")
...
# *************************************************************************************************
rm(df.coef,df.sum)
fm_qpois <- glm(Pallid_Only_Count ~ ., data = myData.reduced, family = quasipoisson)
...
est.zero.p  <- dpois(0,est.lambda)	# the lambda defines the Posisson distribution and we can estimate the probability of zeros of each observation
est.qpois   <- cbind(Pallid_Only_Count        = myData.work[,"Pallid_Only_Count"],
                     eta        = est.pred,
                     mu         = est.mu,
                     lambda     = est.lambda,
                     est.zero.p = est.zero.p)
...
par(mfrow = c(2,2))
 hist(estimation.QuasiPoisson[3:1984,"Pallid_Only_Count"],nclass=100,xlab="Pallid_Only_Count",ylab="frequency",main="[fm_qpois]: distribution of observed count")
 hist(estimation.QuasiPoisson[3:1984,"mu"], nclass=100,xlab="mu", ylab="frequency",main="[fm_qpois]: distribution of expected count")

 plot(estimation.QuasiPoisson[3:1984,"Pallid_Only_Count"],estimation.QuasiPoisson[3:1984,"mu"],type="p",pch=16,cex=0.5,xlab="Pallid_Only_Count count",ylab="expected",main="[fm_QuasiPoisson]: model fitting")
 abline(a=0,b=1,col="red")
...
# *************************************************************************************************
rm(df.coef,df.sum)
fm_nbin <- glm.nb(Pallid_Only_Count ~ ., data = myData.reduced)
...
est.nbin    <- cbind(Pallid_Only_Count        = myData.work[,"Pallid_Only_Count"],
                     eta        = est.pred,
                     mu         = est.mu,
                     lambda     = est.lambda,
                     est.zero.p = est.zero.p)
...
par(mfrow = c(2,2))
 hist(estimation.nbin[3:1984,"Pallid_Only_Count"],nclass=100,xlab="Pallid_Only_Count",ylab="frequency",main="[fm_nbin]: distribution of observed count")
 hist(estimation.nbin[3:1984,"mu"], nclass=100,xlab="mu", ylab="frequency",main="[fm_nbin]: distribution of expected count")

 plot(estimation.nbin[3:1984,"Pallid_Only_Count"],estimation.nbin[3:1984,"mu"],type="p",pch=16,cex=0.5,xlab="Pallid_Only_Count count",ylab="expected",main="[fm_nbin]: model fitting")
 abline(a=0,b=1,col="red")
...
fm_zinb0 <- zeroinfl(Pallid_Only_Count ~ ., data = myData.reduced, dist = "negbin")
...
est.zinb0   	<- cbind(Pallid_Only_Count           = myData.work[,"Pallid_Only_Count"],
                         mu            = est.mu,
                         lambda        = est.lambda,
                         est.zero.p    = est.zero.p,
...
par(mfrow = c(2,2))
 hist(estimation.zinb0[3:1984,"Pallid_Only_Count"],nclass=100,xlab="Pallid_Only_Count",ylab="frequency",main="[fm_zinb0]: distribution of observed count")
 hist(estimation.zinb0[3:1984,"mu"], nclass=100,xlab="mu", ylab="frequency",main="[fm_zinb0]: distribution of expected count")
                    
 hist(estimation.zinb0[3:1984,"est.zero.infl"],nclass=100,xlab="p.0infl",ylab="frequency",main="[fm_zinb0]:distribution of probability of inflated zero")
 plot(estimation.zinb0[3:1984,"Pallid_Only_Count"],estimation.zinb0[3:1984,"mu"],type="p",pch=16,cex=0.5,xlab="Pallid_Only_Count count",ylab="expected",main="[fm_zinb0]:model fitting (Zero-Inflated)")
 abline(a=0,b=1,col="red")
...
list.2.keep <- c("SY","Season","UpDamDist","DnDamDist","Ch_W_Full","UpTribDist","DTribDist","Grade10RM","comppct_r","sandtotal_r","claytotal_r","VF_width", "Segment",  "Temp",  "MicroClass","WaterVel","Pallid_Only_Count","S_U10_D2_Cumu","S_U20_D4_Cumu","S_U30_D6_Cumu","S_U50_D10_Cumu","S_U100_D20_Cumu","macro.type","Lith_1","NFHAP_new","Tax_part_new", "Depth.mean")
list.2.keep <- c( "UpDamDist","Ch_W_Full","UpTribDist","Temp","Pallid_Only_Count", "Depth.mean")
myData.reduced <- myData.work[,list.2.keep]
...
# *************************************************************************************************
rm(df.coef.count,df.coef.zero,df.sum)
fm_zip0 <- zeroinfl(Pallid_Only_Count ~ ., data = myData.reduced, dist = "poisson")
...
est.zip0   	<- cbind(Pallid_Only_Count           = myData.work[,"Pallid_Only_Count"],
                         mu            = est.mu,
                         lambda        = est.lambda,
                         est.zero.p    = est.zero.p,
                         est.zero.infl = est.zero.infl,
...
par(mfrow = c(2,2))
 hist(estimation.zip0[3:1984,"Pallid_Only_Count"],nclass=100,xlab="Pallid_Only_Count",ylab="frequency",main="[fm_zinp0]: distribution of observed count")
 hist(estimation.zip0[3:1984,"mu"], nclass=100,xlab="mu", ylab="frequency",main="[fm_zinp0]: distribution of expected count")
                    
 hist(estimation.zip0[3:1984,"est.zero.infl"],nclass=100,xlab="p.0infl",ylab="frequency",main="[fm_zinp0]:distribution of probability of inflated zero")
 plot(estimation.zip0[3:1984,"Pallid_Only_Count"],estimation.zip0[3:1984,"mu"],type="p",pch=16,cex=0.5,xlab="Pallid_Only_Count count",ylab="expected",main="[fm_zinp0]:model fitting (Zero-Inflated)")
 abline(a=0,b=1,col="red")
...
# -------------------------------------------------------------------------------------------------
# open pdf file for outputting plots
FL.stock.PDF <- paste(Path.Out,"5_prepare_TLC_data_stock_dis.pdf",sep="/")	
...
# -------------------------------------------------------------------------------------------------
# open pdf file for outputting plots
FL.microHabitat <- paste(Path.Out,"5_prepare_TLC_data_microHabitat.pdf",sep="/")	
...
# -------------------------------------------------------------------------------------------------
FL.sandClay <- paste(Path.Out,"5_prepare_TLC_data_sandClay.pdf",sep="/")	
...
# -------------------------------------------------------------------------------------------------
# 2a. check each individual variable: [5_prepare_TLC_data_indVar.pdf] and [5_prepare_TLC_data_num.sum]
# -------------------------------------------------------------------------------------------------
# open pdf file for outputting plots
FL.indVar.PDF <- paste(Path.Out,"5_prepare_TLC_data_indVar.pdf",sep="/")	
...
# -------------------------------------------------------------------------------------------------
# 2b. check the correlation between small group of variables: [5_prepare_TLC_data_indVar.pdf]
# -------------------------------------------------------------------------------------------------
...
# -------------------------------------------------------------------------------------------------
# 3. check correlation between numeric covariates: [5_prepare_TLC_data_Correlation.pdf]
# -------------------------------------------------------------------------------------------------
FL.Correlation.PDF  <- paste(Path.Out,"5_prepare_TLC_data_Correlation.pdf",sep="/")	
...
# -------------------------------------------------------------------------------------------------
# 4. count summary: FL.Summary == [5_prepare_TLC_data_summary.csv]
# -------------------------------------------------------------------------------------------------
...
# -------------------------------------------------------------------------------------------------
# 6. check correlation of continusous variables vs the [count] of Pallid Sturgeon: [5_prepare_TLC_data_Count_numVar.pdf]
# -------------------------------------------------------------------------------------------------
FL.Count_num.PDF  <- paste(Path.Out,"5_prepare_TLC_data_Count_numVar.pdf",sep="/")	
...
# -------------------------------------------------------------------------------------------------
# 7. check the correlation of discrete variables vs the [count] of Pallid Sturgeon: FL.Summary == [5_prepare_TLC_data_summary.csv]
# -------------------------------------------------------------------------------------------------
# FL.Count_cat.PDF  <- paste(Path.Out,"5_prepare_TLC_data_Count_catVar.pdf",sep="/")	
...
# -------------------------------------------------------------------------------------------------
# 8. check correlation of continusous variables vs the [count] of Pallid Sturgeon: 5_prepare_TLC_data_Binary_numVar.pdf; 	FL.Summary == [5_prepare_TLC_data_summary.csv]
# -------------------------------------------------------------------------------------------------
FL.binary_num.PDF  <- paste(Path.Out,"5_prepare_TLC_data_Binary_numVar.pdf",sep="/")	
...
# -------------------------------------------------------------------------------------------------
# FL.binary_cat.PDF  <- paste(Path.Out,"5_prepare_TLC_data_Binary_catVar.pdf",sep="/")	
