...
# Path.Out     <- "C:/YuLong_Projects/FY2012_MORiver/DataAnalysis/7c_prepare_TLC_data"
...
# 
# FL.LOG       <- paste(Path.log,"7c_prepare_TLC_data.log",sep="/")	
# FL.SUM.cat   <- paste(Path.Out,"7c_prepare_TLC_data_categoricalSum.csv",sep="/")
# FL.SUM.num   <- paste(Path.Out,"7c_prepare_TLC_data_numericSum.csv",sep="/")
# FL.Summary   <- paste(Path.Out,"7c_prepare_TLC_data_summary.csv",sep="/")
# FL.missing   <- paste(Path.Out,"7c_prepare_TLC_data_missing.csv",sep="/")
...
FL.RESL.OUT <- paste(Path.Out,"7c_prepare_TLC_data.csv",sep="/")
FL.LOG      <- paste(Path.log,"7c_prepare_TLC_data.log",sep="/")	
FL.PDF      <- paste(Path.Out,"7c_prepare_TLC_data.pdf",sep="/")	
FL.SUM.cat  <- paste(Path.Out,"7c_prepare_TLC_data_cat.sum",sep="/")
FL.SUM.num  <- paste(Path.Out,"7c_prepare_TLC_data_num.sum",sep="/")
...
myData.TLC.lower <- cbind(myData.Y,myData.X.num.std)
...
# -------------------------------------------------------------------------------------------------
# open pdf file for outputting plots
FL.stock.PDF <- paste(Path.Out,"7c_prepare_TLC_data_stock_dis.pdf",sep="/")	
...
# -------------------------------------------------------------------------------------------------
# open pdf file for outputting plots
FL.microHabitat <- paste(Path.Out,"7c_prepare_TLC_data_microHabitat.pdf",sep="/")	
...
# -------------------------------------------------------------------------------------------------
FL.sandClay <- paste(Path.Out,"7c_prepare_TLC_data_sandClay.pdf",sep="/")	
...
# -------------------------------------------------------------------------------------------------
# 2a. check each individual variable: [7c_prepare_TLC_data_indVar.pdf] and [7c_prepare_TLC_data_num.sum]
# -------------------------------------------------------------------------------------------------
# open pdf file for outputting plots
FL.indVar.PDF <- paste(Path.Out,"7c_prepare_TLC_data_indVar.pdf",sep="/")	
...
# -------------------------------------------------------------------------------------------------
# 2b. check the correlation between small group of variables: [7c_prepare_TLC_data_indVar.pdf]
# -------------------------------------------------------------------------------------------------
...
# -------------------------------------------------------------------------------------------------
# 3. check correlation between numeric covariates: [7c_prepare_TLC_data_Correlation.pdf]
# -------------------------------------------------------------------------------------------------
FL.Correlation.PDF  <- paste(Path.Out,"7c_prepare_TLC_data_Correlation.pdf",sep="/")	
...
x.var  <- c("BendRiverMile")
	...
	for (y.var in y.vars)
	{
		myData.TLC.lower <- myData.raw[,c(x.var,y.var,"Reach")]
		command.string <- paste("plot.obj1 <- xyplot(",y.var," ~ ",x.var,", data = myData.TLC.lower,xlab=\"",x.var,"\",ylab=\"",y.var,"\",main=paste(\"(All) \",\"",y.var,"\",\" vs \",\"",x.var,"\",sep=\"\"),type=\"p\",pch=16,col=\"red\")",sep="")
		eval(parse(text=command.string))

		command.string <- paste("plot.obj2 <- xyplot(",y.var," ~ ",x.var,", data = myData.TLC.lower[myData.TLC.lower[,\"Reach\"]==\"Upper\",],xlab=\"",x.var,"\",ylab=\"",y.var,"\",main=paste(\"(Upper) \",\"",y.var,"\",\" vs \",\"",x.var,"\",sep=\"\"),type=\"p\",pch=16,col=\"blue\")",sep="")
		eval(parse(text=command.string))			

		command.string <- paste("plot.obj3 <- xyplot(",y.var," ~ ",x.var,", data = myData.TLC.lower[myData.TLC.lower[,\"Reach\"]==\"Lower\",],xlab=\"",x.var,"\",ylab=\"",y.var,"\",main=paste(\"(Lower) \",\"",y.var,"\",\" vs \",\"",x.var,"\",sep=\"\"),type=\"p\",pch=16,col=\"green\")",sep="")
		eval(parse(text=command.string))
...
# -------------------------------------------------------------------------------------------------
# 4. count summary: FL.Summary == [7c_prepare_TLC_data_summary.csv]
# -------------------------------------------------------------------------------------------------
...
myData.TLC.lower <- myData.sup[,c(x.vars.num,x.vars.cat,y.vars.num,y.vars.cat)]
...
# -------------------------------------------------------------------------------------------------
# 6. check correlation of continusous variables vs the [count] of Pallid Sturgeon: [7c_prepare_TLC_data_Count_numVar.pdf]
# -------------------------------------------------------------------------------------------------
FL.Count_num.PDF  <- paste(Path.Out,"7c_prepare_TLC_data_Count_numVar.pdf",sep="/")	
...
{
	# all data
	command.string <- paste("plot.obj1 <- xyplot(",y.vars.num," ~ ",var.num,", data = myData.TLC.lower,xlab=\"",var.num,"\",ylab=\"",y.vars.num,"\",main=paste(\"(All) \",\"",y.vars.num,"\",\" vs \",\"",var.num,"\",sep=\"\"),type=\"p\",pch=16,col=\"red\")",sep="")
	eval(parse(text=command.string))

	command.string <- paste("plot.obj2 <- bwplot(",var.num," ~ as.factor(",y.vars.num,"), data = myData.TLC.lower,xlab=\"",y.vars.num,"\",ylab=\"",var.num,"\",main=paste(\"(All) \",\"",y.vars.num,"\",\" vs \",\"",var.num,"\",sep=\"\"),col=\"red\")",sep="")
	eval(parse(text=command.string))

	# upper stream
	command.string <- paste("plot.obj3 <- xyplot(",y.vars.num," ~ ",var.num,", data = myData.TLC.lower[myData.TLC.lower[,\"Reach\"]==\"Upper\",],xlab=\"",var.num,"\",ylab=\"",y.vars.num,"\",main=paste(\"(Upper) \",\"",y.vars.num,"\",\" vs \",\"",var.num,"\",sep=\"\"),type=\"p\",pch=16,col=\"blue\")",sep="")
	eval(parse(text=command.string))

	command.string <- paste("plot.obj4 <- bwplot(",var.num," ~ as.factor(",y.vars.num,"), data = myData.TLC.lower[myData.TLC.lower[,\"Reach\"]==\"Upper\",],xlab=\"",y.vars.num,"\",ylab=\"",var.num,"\",main=paste(\"(Upper) \",\"",y.vars.num,"\",\" vs \",\"",var.num,"\",sep=\"\"),col=\"blue\")",sep="")
	eval(parse(text=command.string))
	
	# lower stream
	command.string <- paste("plot.obj5 <- xyplot(",y.vars.num," ~ ",var.num,", data = myData.TLC.lower[myData.TLC.lower[,\"Reach\"]==\"Lower\",],xlab=\"",var.num,"\",ylab=\"",y.vars.num,"\",main=paste(\"(Lower) \",\"",y.vars.num,"\",\" vs \",\"",var.num,"\",sep=\"\"),type=\"p\",pch=16,col=\"green\")",sep="")
	eval(parse(text=command.string))

	command.string <- paste("plot.obj6 <- bwplot(",var.num," ~ as.factor(",y.vars.num,"), data = myData.TLC.lower[myData.TLC.lower[,\"Reach\"]==\"Lower\",],xlab=\"",y.vars.num,"\",ylab=\"",var.num,"\",main=paste(\"(Lower) \",\"",y.vars.num,"\",\" vs \",\"",var.num,"\",sep=\"\"),col=\"green\")",sep="")
	eval(parse(text=command.string))
	...
	command.string <- paste("plot.obj <- xyplot(",y.vars.num," ~ ",var.num," | Segment, data = myData.TLC.lower,xlab=\"",var.num,"\",ylab=\"",y.vars.num,"\",main=paste(\"(All) \",\"",y.vars.num,"\",\" vs \",\"",var.num,"\",sep=\"\"),type=\"p\",pch=16,col=\"red\")",sep="")
	eval(parse(text=command.string))
	plot(plot.obj)

	# Segment-wise: boxplot plots
	command.string <- paste("plot.obj <- bwplot(",var.num," ~ as.factor(",y.vars.num,") | Segment, data = myData.TLC.lower,xlab=\"",y.vars.num,"\",ylab=\"",var.num,"\",main=paste(\"(All) \",\"",y.vars.num,"\",\" vs \",\"",var.num,"\",sep=\"\"),col=\"red\")",sep="")
	eval(parse(text=command.string))
	plot(plot.obj)
	
	# Macro-wise: scatter plots
	command.string <- paste("plot.obj <- xyplot(",y.vars.num," ~ ",var.num," | macro.type, data = myData.TLC.lower,xlab=\"",var.num,"\",ylab=\"",y.vars.num,"\",main=paste(\"(All) \",\"",y.vars.num,"\",\" vs \",\"",var.num,"\",sep=\"\"),type=\"p\",pch=16,col=\"blue\")",sep="")
	eval(parse(text=command.string))
	plot(plot.obj)

	# Macro-wise: boxplot plots
	command.string <- paste("plot.obj <- bwplot(",var.num," ~ as.factor(",y.vars.num,") | macro.type, data = myData.TLC.lower,xlab=\"",y.vars.num,"\",ylab=\"",var.num,"\",main=paste(\"(All) \",\"",y.vars.num,"\",\" vs \",\"",var.num,"\",sep=\"\"),col=\"blue\")",sep="")
	eval(parse(text=command.string))
	plot(plot.obj)
...
# -------------------------------------------------------------------------------------------------
# 7. check the correlation of discrete variables vs the [count] of Pallid Sturgeon: FL.Summary == [7c_prepare_TLC_data_summary.csv]
# -------------------------------------------------------------------------------------------------
# FL.Count_cat.PDF  <- paste(Path.Out,"7c_prepare_TLC_data_Count_catVar.pdf",sep="/")	
...
{
# 	# all data
# 	command.string <- paste("plot.obj1 <- xyplot(",y.vars.num," ~ ",var.cat,", data = myData.TLC.lower,xlab=\"",var.cat,"\",ylab=\"",y.vars.num,"\",main=paste(\"(All) \",\"",y.vars.num,"\",\" vs \",\"",var.cat,"\",sep=\"\"),type=\"p\",pch=16,col=\"red\")",sep="")
# 	eval(parse(text=command.string))
# 
# 	command.string <- paste("plot.obj2 <- bwplot(",var.cat," ~ as.factor(",y.vars.num,"), data = myData.TLC.lower,xlab=\"",y.vars.num,"\",ylab=\"",var.cat,"\",main=paste(\"(All) \",\"",y.vars.num,"\",\" vs \",\"",var.cat,"\",sep=\"\"),col=\"red\")",sep="")
# 	eval(parse(text=command.string))
# 
# 	# upper stream
# 	command.string <- paste("plot.obj3 <- xyplot(",y.vars.num," ~ ",var.cat,", data = myData.TLC.lower[myData.TLC.lower[,\"Reach\"]==\"Upper\",],xlab=\"",var.cat,"\",ylab=\"",y.vars.num,"\",main=paste(\"(Upper) \",\"",y.vars.num,"\",\" vs \",\"",var.cat,"\",sep=\"\"),type=\"p\",pch=16,col=\"blue\")",sep="")
# 	eval(parse(text=command.string))
# 
# 	command.string <- paste("plot.obj4 <- bwplot(",var.cat," ~ as.factor(",y.vars.num,"), data = myData.TLC.lower[myData.TLC.lower[,\"Reach\"]==\"Upper\",],xlab=\"",y.vars.num,"\",ylab=\"",var.cat,"\",main=paste(\"(Upper) \",\"",y.vars.num,"\",\" vs \",\"",var.cat,"\",sep=\"\"),col=\"blue\")",sep="")
# 	eval(parse(text=command.string))
# 	
# 	# lower stream
# 	command.string <- paste("plot.obj5 <- xyplot(",y.vars.num," ~ ",var.cat,", data = myData.TLC.lower[myData.TLC.lower[,\"Reach\"]==\"Lower\",],xlab=\"",var.cat,"\",ylab=\"",y.vars.num,"\",main=paste(\"(Lower) \",\"",y.vars.num,"\",\" vs \",\"",var.cat,"\",sep=\"\"),type=\"p\",pch=16,col=\"green\")",sep="")
# 	eval(parse(text=command.string))
# 
# 	command.string <- paste("plot.obj6 <- bwplot(",var.cat," ~ as.factor(",y.vars.num,"), data = myData.TLC.lower[myData.TLC.lower[,\"Reach\"]==\"Lower\",],xlab=\"",y.vars.num,"\",ylab=\"",var.cat,"\",main=paste(\"(Lower) \",\"",y.vars.num,"\",\" vs \",\"",var.cat,"\",sep=\"\"),col=\"green\")",sep="")
# 	eval(parse(text=command.string))
# 	
# 	plot(plot.obj1,split=c(1,1,2,3))
# 	plot(plot.obj2,split=c(2,1,2,3),newpage=FALSE)	
...
# 	
# 	# Segment-wise: scatter plots
# 	command.string <- paste("plot.obj <- xyplot(",y.vars.num," ~ ",var.cat," | Segment, data = myData.TLC.lower,xlab=\"",var.cat,"\",ylab=\"",y.vars.num,"\",main=paste(\"(All) \",\"",y.vars.num,"\",\" vs \",\"",var.cat,"\",sep=\"\"),type=\"p\",pch=16,col=\"red\")",sep="")
# 	eval(parse(text=command.string))
# 	plot(plot.obj)
# 
# 	# Segment-wise: boxplot plots
# 	command.string <- paste("plot.obj <- bwplot(",var.cat," ~ as.factor(",y.vars.num,") | Segment, data = myData.TLC.lower,xlab=\"",y.vars.num,"\",ylab=\"",var.cat,"\",main=paste(\"(All) \",\"",y.vars.num,"\",\" vs \",\"",var.cat,"\",sep=\"\"),col=\"red\")",sep="")
# 	eval(parse(text=command.string))
# 	plot(plot.obj)
# 	
# 	# Macro-wise: scatter plots
# 	command.string <- paste("plot.obj <- xyplot(",y.vars.num," ~ ",var.cat," | macro.type, data = myData.TLC.lower,xlab=\"",var.cat,"\",ylab=\"",y.vars.num,"\",main=paste(\"(All) \",\"",y.vars.num,"\",\" vs \",\"",var.cat,"\",sep=\"\"),type=\"p\",pch=16,col=\"blue\")",sep="")
# 	eval(parse(text=command.string))
# 	plot(plot.obj)
# 
# 	# Macro-wise: boxplot plots
# 	command.string <- paste("plot.obj <- bwplot(",var.cat," ~ as.factor(",y.vars.num,") | macro.type, data = myData.TLC.lower,xlab=\"",y.vars.num,"\",ylab=\"",var.cat,"\",main=paste(\"(All) \",\"",y.vars.num,"\",\" vs \",\"",var.cat,"\",sep=\"\"),col=\"blue\")",sep="")
# 	eval(parse(text=command.string))
# 	plot(plot.obj)
# 	
 	#
 	# make cross table
 	#
 	command.string <- paste("myData.tmp <- myData.TLC.lower[,c(\"",var.cat,"\",\"",y.vars.cat,"\")]",sep="")
 	eval(parse(text=command.string))
...
# -------------------------------------------------------------------------------------------------
# 8. check correlation of continusous variables vs the [count] of Pallid Sturgeon: 7c_prepare_TLC_data_Binary_numVar.pdf; 	FL.Summary == [7c_prepare_TLC_data_summary.csv]
# -------------------------------------------------------------------------------------------------
FL.binary_num.PDF  <- paste(Path.Out,"7c_prepare_TLC_data_Binary_numVar.pdf",sep="/")	
...
{
	# all data
	command.string <- paste("plot.obj1 <- bwplot(",var.num," ~ as.factor(",y.vars.cat,"), data = myData.TLC.lower,xlab=\"",y.vars.cat,"\",ylab=\"",var.num,"\",main=paste(\"(All) \",\"",y.vars.cat,"\",\" vs \",\"",var.num,"\",sep=\"\"),col=\"red\",scale=list(y=\"free\"))",sep="")
	eval(parse(text=command.string))

	# upper stream
	command.string <- paste("plot.obj2 <- bwplot(",var.num," ~ as.factor(",y.vars.cat,"), data = myData.TLC.lower[myData.TLC.lower[,\"Reach\"]==\"Upper\",],xlab=\"",y.vars.cat,"\",ylab=\"",var.num,"\",main=paste(\"(Upper) \",\"",y.vars.cat,"\",\" vs \",\"",var.num,"\",sep=\"\"),col=\"blue\",scale=list(y=\"free\"))",sep="")
	eval(parse(text=command.string))
	
	# lower stream
	command.string <- paste("plot.obj3 <- bwplot(",var.num," ~ as.factor(",y.vars.cat,"), data = myData.TLC.lower[myData.TLC.lower[,\"Reach\"]==\"Lower\",],xlab=\"",y.vars.cat,"\",ylab=\"",var.num,"\",main=paste(\"(Lower) \",\"",y.vars.cat,"\",\" vs \",\"",var.num,"\",sep=\"\"),col=\"green\",scale=list(y=\"free\"))",sep="")
	eval(parse(text=command.string))
	...
	# Segment-wise: boxplot plots
	command.string <- paste("plot.obj <- bwplot(",var.num," ~ as.factor(",y.vars.cat,") | Segment, data = myData.TLC.lower,xlab=\"",y.vars.cat,"\",ylab=\"",var.num,"\",main=paste(\"(All) \",\"",y.vars.cat,"\",\" vs \",\"",var.num,"\",sep=\"\"),col=\"red\",scale=list(y=\"free\"))",sep="")
	eval(parse(text=command.string))
	plot(plot.obj)
	
	# Macro-wise: boxplot plots
	command.string <- paste("plot.obj <- bwplot(",var.num," ~ as.factor(",y.vars.cat,") | macro.type, data = myData.TLC.lower,xlab=\"",y.vars.cat,"\",ylab=\"",var.num,"\",main=paste(\"(All) \",\"",y.vars.cat,"\",\" vs \",\"",var.num,"\",sep=\"\"),col=\"blue\",scale=list(y=\"free\"))",sep="")
	eval(parse(text=command.string))
	plot(plot.obj)
...
# -------------------------------------------------------------------------------------------------
# FL.binary_cat.PDF  <- paste(Path.Out,"7c_prepare_TLC_data_Binary_catVar.pdf",sep="/")	
...
# 
 ...
 {
# 	# all data
# 	command.string <- paste("plot.obj1 <- xyplot(",y.vars.cat," ~ ",var.cat,", data = myData.TLC.lower,xlab=\"",var.cat,"\",ylab=\"",y.vars.cat,"\",main=paste(\"(All) \",\"",y.vars.cat,"\",\" vs \",\"",var.cat,"\",sep=\"\"),type=\"p\",pch=16,col=\"red\")",sep="")
# 	eval(parse(text=command.string))
# 
# 	command.string <- paste("plot.obj2 <- bwplot(",var.cat," ~ as.factor(",y.vars.cat,"), data = myData.TLC.lower,xlab=\"",y.vars.cat,"\",ylab=\"",var.cat,"\",main=paste(\"(All) \",\"",y.vars.cat,"\",\" vs \",\"",var.cat,"\",sep=\"\"),col=\"red\")",sep="")
# 	eval(parse(text=command.string))
# 
# 	# upper stream
# 	command.string <- paste("plot.obj3 <- xyplot(",y.vars.cat," ~ ",var.cat,", data = myData.TLC.lower[myData.TLC.lower[,\"Reach\"]==\"Upper\",],xlab=\"",var.cat,"\",ylab=\"",y.vars.cat,"\",main=paste(\"(Upper) \",\"",y.vars.cat,"\",\" vs \",\"",var.cat,"\",sep=\"\"),type=\"p\",pch=16,col=\"blue\")",sep="")
# 	eval(parse(text=command.string))
# 
# 	command.string <- paste("plot.obj4 <- bwplot(",var.cat," ~ as.factor(",y.vars.cat,"), data = myData.TLC.lower[myData.TLC.lower[,\"Reach\"]==\"Upper\",],xlab=\"",y.vars.cat,"\",ylab=\"",var.cat,"\",main=paste(\"(Upper) \",\"",y.vars.cat,"\",\" vs \",\"",var.cat,"\",sep=\"\"),col=\"blue\")",sep="")
# 	eval(parse(text=command.string))
# 	
# 	# lower stream
# 	command.string <- paste("plot.obj5 <- xyplot(",y.vars.cat," ~ ",var.cat,", data = myData.TLC.lower[myData.TLC.lower[,\"Reach\"]==\"Lower\",],xlab=\"",var.cat,"\",ylab=\"",y.vars.cat,"\",main=paste(\"(Lower) \",\"",y.vars.cat,"\",\" vs \",\"",var.cat,"\",sep=\"\"),type=\"p\",pch=16,col=\"green\")",sep="")
# 	eval(parse(text=command.string))
# 
# 	command.string <- paste("plot.obj6 <- bwplot(",var.cat," ~ as.factor(",y.vars.cat,"), data = myData.TLC.lower[myData.TLC.lower[,\"Reach\"]==\"Lower\",],xlab=\"",y.vars.cat,"\",ylab=\"",var.cat,"\",main=paste(\"(Lower) \",\"",y.vars.cat,"\",\" vs \",\"",var.cat,"\",sep=\"\"),col=\"green\")",sep="")
# 	eval(parse(text=command.string))
# 	
# 	plot(plot.obj1,split=c(1,1,2,3))
# 	plot(plot.obj2,split=c(2,1,2,3),newpage=FALSE)	
...
# 	
# 	# Segment-wise: scatter plots
# 	command.string <- paste("plot.obj <- xyplot(",y.vars.cat," ~ ",var.cat," | Segment, data = myData.TLC.lower,xlab=\"",var.cat,"\",ylab=\"",y.vars.cat,"\",main=paste(\"(All) \",\"",y.vars.cat,"\",\" vs \",\"",var.cat,"\",sep=\"\"),type=\"p\",pch=16,col=\"red\")",sep="")
# 	eval(parse(text=command.string))
# 	plot(plot.obj)
# 
# 	# Segment-wise: boxplot plots
# 	command.string <- paste("plot.obj <- bwplot(",var.cat," ~ as.factor(",y.vars.cat,") | Segment, data = myData.TLC.lower,xlab=\"",y.vars.cat,"\",ylab=\"",var.cat,"\",main=paste(\"(All) \",\"",y.vars.cat,"\",\" vs \",\"",var.cat,"\",sep=\"\"),col=\"red\")",sep="")
# 	eval(parse(text=command.string))
# 	plot(plot.obj)
# 	
# 	# Macro-wise: scatter plots
# 	command.string <- paste("plot.obj <- xyplot(",y.vars.cat," ~ ",var.cat," | macro.type, data = myData.TLC.lower,xlab=\"",var.cat,"\",ylab=\"",y.vars.cat,"\",main=paste(\"(All) \",\"",y.vars.cat,"\",\" vs \",\"",var.cat,"\",sep=\"\"),type=\"p\",pch=16,col=\"blue\")",sep="")
# 	eval(parse(text=command.string))
# 	plot(plot.obj)
# 
# 	# Macro-wise: boxplot plots
# 	command.string <- paste("plot.obj <- bwplot(",var.cat," ~ as.factor(",y.vars.cat,") | macro.type, data = myData.TLC.lower,xlab=\"",y.vars.cat,"\",ylab=\"",var.cat,"\",main=paste(\"(All) \",\"",y.vars.cat,"\",\" vs \",\"",var.cat,"\",sep=\"\"),col=\"blue\")",sep="")
# 	eval(parse(text=command.string))
# 	plot(plot.obj)
# 	
 	#
