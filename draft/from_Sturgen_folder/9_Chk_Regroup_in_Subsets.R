...
Path.OUT <- "C:/YuLong_Projects/FY2012_MORiver/DataAnalysis/9_Chk_Regroup_in_Subsets"
...
FL.LOG    <- paste(Path.log,"9_Chk_Regroup_in_Subsets.log",sep="/")	
...
for (idx.subset in c(1,2,3,4,5,6,7,8))
{
	...
	FL.OUT.CSV <- paste(Path.OUT, paste("Subset_",subset.section,"_",subset.gear,".csv",sep=""),sep="/")
...
cat(paste("\n9_Chk_Regroup_in_Subsets.R is finished successfully at ",End.time,"!\n",sep=" "))
cat(paste("\n9_Chk_Regroup_in_Subsets.R is finished successfully at ",End.time,"!\n",sep=" "),file=FL.LOG,append=TRUE)

cat(paste("\nProcessing time for [9_Chk_Regroup_in_Subsets.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "))
cat(paste("\nProcessing time for [9_Chk_Regroup_in_Subsets.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "),file=FL.LOG,append=TRUE)
