#
# 25_GenerateData_for_OpenBugs_subset6_Final.R
#
...
for (idx.subset in c(6))
{
	...
	# subset 6: Logistic Upper [myData.Upper_Logistic]
		...
		Y.var  <- "binary"
		var.rm.fix <- c("Gear","MA","Lith_1","Lith_2","Lith_desc","NFHAP","Tax_part_sz","Turb","Meso","MicroClass","WaterVel","Fish_cnt","CPUE","CPUA","Stk_12_dist","Stk_24_dist","Stk_36_dist","Stk_60_dist","Stk_120_dist",                            "gear.type2","CPUA.calc","CPUE.calc","Comp_per","Sand_per","Clay_per","new_lith","new_part")	# August 27, 2013: include "gear.type1"
		var.rm     <- c("Gear","MA","Lith_1","Lith_2","Lith_desc","NFHAP","Tax_part_sz","Turb","Meso","MicroClass","WaterVel","Fish_cnt","CPUE","CPUA","Stk_12_dist","Stk_24_dist","Stk_36_dist","Stk_60_dist","Stk_120_dist",                            "gear.type2","CPUA.calc","CPUE.calc","Comp_per","Sand_per","Clay_per","new_lith","new_part")	# August 27, 2013: include "gear.type1"
	...
	if(idx.subset == 6)	# PSCL Lower TLC
	{
		...
		for (model4test in seq(from=1,to=1,by=1))
		{
			if (model4test == 1)
			{
				list.var <- c("Stk_120_cnt","gear.type1","Rel_dsch_AY","Seg","Season","macro.type","T_dist_up","SY")	
				A <- c(-2.356870486,0.194691205,1.102660522,-0.264025113,-0.36440211,0.635491736,0.848157465,-0.444366718,-1.042785789,-1.081262248,-1.458540879,-0.850612669,0.116000487,0.417647426,0.500220602,0.603911788,0.553624482,0.881608862) 
			}
