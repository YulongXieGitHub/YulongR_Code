#
# 24_VariableSelection_Subset6_final_Modified.R (include "gear.type1")
#
# August 27, 2013
#
...
FL.LOG    <- paste(Path.log,"24_VariableSelection_Subset6_final_Modified.log",sep="/")	
...
FL.RESL.OUT <- paste(Path.OUT,"24_VariableSelection_Subset6_final_Modified.csv",sep="/")
...
for (idx.subset in c(6))
{
	...
	if(idx.subset == 6)	# Logistic Regression for Upper River
	{
		...
		# ---------------------------------------------------------------------------------
		# [Subset 6 Logistic Regression on Upper River]: 3. model with 3 variables: ["Stk_120_cnt"] & ["gear.type1"] and another one (three variables): "gear.type1" is te second best single with "Stk_120_cnt"
		# ---------------------------------------------------------------------------------
		no.tmp  <- length(c(X.var.num,X.var.cat))
		idx.tmp <- 0
		for (X.var.rm in grep("gear.type1",grep("Stk_120_cnt",c(X.var.num,X.var.cat),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE))		# all variables other than "gear.type1", "Stk_120_cnt"
		{
			idx.tmp <- idx.tmp + 1
			idx.var.rm <- match(X.var.rm,names(myData.Work))							# the index of the variable to be removed in the data frame
			if(!(is.na(idx.var.rm)))
			{
				model.count  <- model.count + 1
				myData.Tmp   <- myData.Work[,c("binary","Stk_120_cnt","gear.type1",X.var.rm)]			# three variable combinations: "Stk_120_cnt", "gear.type1" and the other
				model.Tmp    <- glm(binary ~ .,data=myData.Tmp,family=binomial("logit"))
				model.ID     <- X.var.rm
				model.string <- paste("Model",model.count," with ",X.var.rm," & Stk_120_cnt/gear.type1\n",sep=" ")
				model.no.var <- dim(myData.Tmp)[2] - 1
				model.rm.var <- paste("all but Stk_120_cnt/gear.type1 & ",X.var.rm,sep=" ")
		...
		# ---------------------------------------------------------------------------------
		# [Subset 6 Logistic Regression on Upper River]: 4. model with 4 variables: ["Stk_120_cnt"] & ["gear.type1"] and another one (three variables): "gear.type1" is te second best single with "Stk_120_cnt"
		# ---------------------------------------------------------------------------------
		no.tmp  <- length(c(X.var.num,X.var.cat))
		idx.tmp <- 0
		for (X.var.rm in grep("Rel_dsch_AY",grep("gear.type1",grep("Stk_120_cnt",c(X.var.num,X.var.cat),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE))		# all variables other than "gear.type1", "Stk_120_cnt"
		{
			idx.tmp <- idx.tmp + 1
			idx.var.rm <- match(X.var.rm,names(myData.Work))							# the index of the variable to be removed in the data frame
			if(!(is.na(idx.var.rm)))
			{
				model.count  <- model.count + 1
				myData.Tmp   <- myData.Work[,c("binary","Stk_120_cnt","gear.type1","Rel_dsch_AY",X.var.rm)]			# three variable combinations: "Stk_120_cnt", "gear.type1" and the other
				model.Tmp    <- glm(binary ~ .,data=myData.Tmp,family=binomial("logit"))
				model.ID     <- X.var.rm
				model.string <- paste("Model",model.count," with ",X.var.rm," & Stk_120_cnt/gear.type1/Rel_dsch_AY\n",sep=" ")
				model.no.var <- dim(myData.Tmp)[2] - 1
				model.rm.var <- paste("all but Stk_120_cnt/gear.type1/Rel_dsch_AY & ",X.var.rm,sep=" ")
		...
		# ---------------------------------------------------------------------------------
		# [Subset 6 Logistic Regression on Upper River]: 5. model with 5 variables: ["Stk_120_cnt"] & ["gear.type1"] and another one (three variables): "gear.type1" is te second best single with "Stk_120_cnt"
		# ---------------------------------------------------------------------------------
		no.tmp  <- length(c(X.var.num,X.var.cat))
		idx.tmp <- 0
		for (X.var.rm in grep("Seg",grep("Rel_dsch_AY",grep("gear.type1",grep("Stk_120_cnt",c(X.var.num,X.var.cat),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE))		# all variables other than "gear.type1", "Stk_120_cnt"
		{
			idx.tmp <- idx.tmp + 1
			idx.var.rm <- match(X.var.rm,names(myData.Work))							# the index of the variable to be removed in the data frame
			if(!(is.na(idx.var.rm)))
			{
				model.count  <- model.count + 1
				myData.Tmp   <- myData.Work[,c("binary","Stk_120_cnt","gear.type1","Rel_dsch_AY","Seg",X.var.rm)]			# three variable combinations: "Stk_120_cnt", "gear.type1" and the other
				model.Tmp    <- glm(binary ~ .,data=myData.Tmp,family=binomial("logit"))
				model.ID     <- X.var.rm
				model.string <- paste("Model",model.count," with ",X.var.rm," & Stk_120_cnt/gear.type1/Seg\n",sep=" ")
				model.no.var <- dim(myData.Tmp)[2] - 1
				model.rm.var <- paste("all but Stk_120_cnt/gear.type1/Rel_dsch_AY/Seg & ",X.var.rm,sep=" ")
		...
		# ---------------------------------------------------------------------------------
		# [Subset 6 Logistic Regression on Upper River]: 6. model with 6 variables: ["Stk_120_cnt"] & ["gear.type1"] and another one (three variables): "gear.type1" is te second best single with "Stk_120_cnt"
		# ---------------------------------------------------------------------------------
		no.tmp  <- length(c(X.var.num,X.var.cat))
		idx.tmp <- 0
		for (X.var.rm in grep("Season",grep("Seg",grep("Rel_dsch_AY",grep("gear.type1",grep("Stk_120_cnt",c(X.var.num,X.var.cat),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE))		# all variables other than "gear.type1", "Stk_120_cnt"
		{
			idx.tmp <- idx.tmp + 1
			idx.var.rm <- match(X.var.rm,names(myData.Work))							# the index of the variable to be removed in the data frame
			if(!(is.na(idx.var.rm)))
			{
				model.count  <- model.count + 1
				myData.Tmp   <- myData.Work[,c("binary","Stk_120_cnt","gear.type1","Rel_dsch_AY","Seg","Season",X.var.rm)]			# three variable combinations: "Stk_120_cnt", "gear.type1" and the other
				model.Tmp    <- glm(binary ~ .,data=myData.Tmp,family=binomial("logit"))
				model.ID     <- X.var.rm
				model.string <- paste("Model",model.count," with ",X.var.rm," & Stk_120_cnt/gear.type1/Seg/Season\n",sep=" ")
				model.no.var <- dim(myData.Tmp)[2] - 1
				model.rm.var <- paste("all but Stk_120_cnt/gear.type1/Rel_dsch_AY/Seg/Season & ",X.var.rm,sep=" ")
		...
		# ---------------------------------------------------------------------------------
		# [Subset 6 Logistic Regression on Upper River]: 7. model with 7 variables: ["Stk_120_cnt"] & ["gear.type1"] and another one (three variables): "gear.type1" is te second best single with "Stk_120_cnt"
		# ---------------------------------------------------------------------------------
		no.tmp  <- length(c(X.var.num,X.var.cat))
		idx.tmp <- 0
		for (X.var.rm in grep("macro.type",grep("Season",grep("Seg",grep("Rel_dsch_AY",grep("gear.type1",grep("Stk_120_cnt",c(X.var.num,X.var.cat),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE))		# all variables other than "gear.type1", "Stk_120_cnt"
		{
			idx.tmp <- idx.tmp + 1
			idx.var.rm <- match(X.var.rm,names(myData.Work))							# the index of the variable to be removed in the data frame
			if(!(is.na(idx.var.rm)))
			{
				model.count  <- model.count + 1
				myData.Tmp   <- myData.Work[,c("binary","Stk_120_cnt","gear.type1","Rel_dsch_AY","Seg","Season","macro.type",X.var.rm)]			# three variable combinations: "Stk_120_cnt", "gear.type1" and the other
				model.Tmp    <- glm(binary ~ .,data=myData.Tmp,family=binomial("logit"))
				model.ID     <- X.var.rm
				model.string <- paste("Model",model.count," with ",X.var.rm," & Stk_120_cnt/gear.type1/Seg/Season/macro.type\n",sep=" ")
				model.no.var <- dim(myData.Tmp)[2] - 1
				model.rm.var <- paste("all but Stk_120_cnt/gear.type1/Rel_dsch_AY/Seg/Season/macro.type & ",X.var.rm,sep=" ")
		...
		# ---------------------------------------------------------------------------------
		# [Subset 6 Logistic Regression on Upper River]: 8. model with 8 variables: ["Stk_120_cnt"] & ["gear.type1"] and another one (three variables): "gear.type1" is te second best single with "Stk_120_cnt"
		# ---------------------------------------------------------------------------------
		no.tmp  <- length(c(X.var.num,X.var.cat))
		idx.tmp <- 0
		for (X.var.rm in grep("T_dist_up",grep("macro.type",grep("Season",grep("Seg",grep("Rel_dsch_AY",grep("gear.type1",grep("Stk_120_cnt",c(X.var.num,X.var.cat),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE))		# all variables other than "gear.type1", "Stk_120_cnt"
		{
			idx.tmp <- idx.tmp + 1
			idx.var.rm <- match(X.var.rm,names(myData.Work))							# the index of the variable to be removed in the data frame
			if(!(is.na(idx.var.rm)))
			{
				model.count  <- model.count + 1
				myData.Tmp   <- myData.Work[,c("binary","Stk_120_cnt","gear.type1","Rel_dsch_AY","Seg","Season","macro.type","T_dist_up",X.var.rm)]			# three variable combinations: "Stk_120_cnt", "gear.type1" and the other
				model.Tmp    <- glm(binary ~ .,data=myData.Tmp,family=binomial("logit"))
				model.ID     <- X.var.rm
				model.string <- paste("Model",model.count," with ",X.var.rm," & Stk_120_cnt/gear.type1/Seg/Season/macro.type/T_dist_up\n",sep=" ")
				model.no.var <- dim(myData.Tmp)[2] - 1
				model.rm.var <- paste("all but Stk_120_cnt/gear.type1/Rel_dsch_AY/Seg/Season/macro.type/T_dist_up & ",X.var.rm,sep=" ")
		...
		# ---------------------------------------------------------------------------------
		# [Subset 6 Logistic Regression on Upper River]: 9. model with 9 variables: ["Stk_120_cnt"] & ["gear.type1"] and another one (three variables): "gear.type1" is te second best single with "Stk_120_cnt"
		# ---------------------------------------------------------------------------------
		no.tmp  <- length(c(X.var.num,X.var.cat))
		idx.tmp <- 0
		for (X.var.rm in grep("SY",grep("T_dist_up",grep("macro.type",grep("Season",grep("Seg",grep("Rel_dsch_AY",grep("gear.type1",grep("Stk_120_cnt",c(X.var.num,X.var.cat),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE),value=TRUE,invert=TRUE))		# all variables other than "gear.type1", "Stk_120_cnt"
		{
			idx.tmp <- idx.tmp + 1
			idx.var.rm <- match(X.var.rm,names(myData.Work))							# the index of the variable to be removed in the data frame
			if(!(is.na(idx.var.rm)))
			{
				model.count  <- model.count + 1
				myData.Tmp   <- myData.Work[,c("binary","Stk_120_cnt","gear.type1","Rel_dsch_AY","Seg","Season","macro.type","T_dist_up","SY",X.var.rm)]			# three variable combinations: "Stk_120_cnt", "gear.type1" and the other
				model.Tmp    <- glm(binary ~ .,data=myData.Tmp,family=binomial("logit"))
				model.ID     <- X.var.rm
				model.string <- paste("Model",model.count," with ",X.var.rm," & Stk_120_cnt/gear.type1/Seg/Season/macro.type/T_dist_up/SY\n",sep=" ")
				model.no.var <- dim(myData.Tmp)[2] - 1
				model.rm.var <- paste("all but Stk_120_cnt/gear.type1/Rel_dsch_AY/Seg/Season/macro.type/T_dist_up/SY & ",X.var.rm,sep=" ")
...
cat(paste("\n24_VariableSelection_Subset6_final_Modified.R is finished successfully at ",End.time,"!\n",sep=" "))
cat(paste("\n24_VariableSelection_Subset6_final_Modified.R is finished successfully at ",End.time,"!\n",sep=" "),file=FL.LOG,append=TRUE)

cat(paste("\nProcessing time for [24_VariableSelection_Subset6_final_Modified.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "))
cat(paste("\nProcessing time for [24_VariableSelection_Subset6_final_Modified.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "),file=FL.LOG,append=TRUE)
