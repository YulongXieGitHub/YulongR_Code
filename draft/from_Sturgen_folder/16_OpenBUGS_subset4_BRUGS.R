...
FL.LOG    <- paste(Path.log,"16_OpenBUGS_subset4_BRUGS.log",sep="/")	
...
for (idx.subset in c(4))
{
	...
	if(idx.subset == 4)	# PSCL Lower TLC
	{
		...
		for (model4test in seq(from=1,to=1,by=1))
		{
			if (model4test == 1)
			{
				list.var <- c("GIS_RM","Stk_60_cnt","Season","T_dist_up")	
				B <- c(-2.475,0.810,0.150,-0.212,0.004) 
				A <- c(-10.270,13.028,1.347,-2.210,0.570) 
			}
...
"model{
	...
	for (j in 1:",no.var,"){b[j]  ~ dnorm(0,1.0E-6)}
			...
			#------------------------------------------------------------------------------
			samplesSet( c("a0","a","b0","b","deviance") )
			nPerChain = 50000
			thinStep = 100
...
cat(paste("\n16_OpenBUGS_subset4_BRUGS.R is finished successfully at ",End.time,"!\n",sep=" "))
cat(paste("\n16_OpenBUGS_subset4_BRUGS.R is finished successfully at ",End.time,"!\n",sep=" "),file=FL.LOG,append=TRUE)

cat(paste("\nProcessing time for [16_OpenBUGS_subset4_BRUGS.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "))
cat(paste("\nProcessing time for [16_OpenBUGS_subset4_BRUGS.R] is ",as.numeric(Diff.time, units="mins")," minutes\n",sep=" "),file=FL.LOG,append=TRUE)
