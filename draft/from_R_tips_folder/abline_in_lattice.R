...
#
data.gas.wide <- subset(data.gas.wide,month==9)
...
# C. histgram of O3 
# -------------------------------------------------------------------------------------------------
plot.obj1 <-  densityplot(~O3,     data=data.1hr.runAvg_O3_longFM,groups=site,plot.points=FALSE,endpoints = c(0,160),xlim = c(0,160), ylim = c(0,0.030),ref=TRUE,xlab=expression("O"[{3}] (ppbv)),auto.key=list(columns=3,col=c("red","blue","green")),col=c("red","blue","green"))
plot(plot.obj1)

plot.obj2 <-  densityplot(~inc.O3, data=data.1hr.runAvg_O3_longFM,groups=site,plot.points=FALSE,endpoints = c(0,10), xlim = c(0,10),  ylim = c(0,1.5),  ref=TRUE,xlab=expression(Delta(O[{3}])/Delta(t)),auto.key=list(columns=3,col=c("red","blue","green")),col=c("red","blue","green"))
plot(plot.obj2)
